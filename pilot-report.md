# SHA-256 Verification Pilot: Report on Phases 1-4 (Complete)
...
multi-block loop that constitutes the production-grade target.
...
- `arm/proofs/sha256_block_data_order_hw.ml` -- Multi-block correctness proof (fully machine-checked)

**Testing and benchmarks:**
- `tests/test.c` -- NIST known-answer + random tests
- `benchmarks/benchmark.c` -- Performance benchmarks
- `include/s2n-bignum.h` -- Updated header with new function declarations
- `arm/Makefile` -- Updated build rules
