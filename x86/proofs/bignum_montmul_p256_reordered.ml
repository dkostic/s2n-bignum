...
(* ========================================================================= *)
(* MULX-based Montgomery multiplication modulo p_256 (reordered).                        *)
(* ========================================================================= *)

needs "x86/proofs/base.ml";;

(**** print_literal_from_elf "x86/p256/bignum_montmul_p256_reordered.o";;
 ****)

let bignum_montmul_p256_reordered_mc =
  define_assert_from_elf "bignum_montmul_p256_reordered_mc" "x86/p256/bignum_montmul_p256_reordered.o"
[
...
let bignum_montmul_p256_reordered_tmc = define_trimmed "bignum_montmul_p256_reordered_tmc" bignum_montmul_p256_reordered_mc;;

let BIGNUM_MONTMUL_P256_REORDERED_EXEC = X86_MK_CORE_EXEC_RULE bignum_montmul_p256_reordered_tmc;;
...
let BIGNUM_MONTMUL_P256_REORDERED_CORRECT = time prove
 (`!z x y a b pc.
        nonoverlapping (word pc,0x242) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_montmul_p256_reordered_tmc) /\
  ...
  ASM_CASES_TAC `a * b <= 2 EXP 256 * p_256` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC BIGNUM_MONTMUL_P256_REORDERED_EXEC (1--114)] THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_MONTMUL_P256_REORDERED_EXEC (1--96) (1--96) THEN
  ABBREV_TAC
  ...
  X86_ACCSTEPS_TAC BIGNUM_MONTMUL_P256_REORDERED_EXEC [98;100;103;105;106] (97--114) THEN
...
let BIGNUM_MONTMUL_P256_REORDERED_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word_sub stackpointer (word 40),40))
            [(word pc,LENGTH bignum_montmul_p256_reordered_tmc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256_reordered_tmc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montmul_p256_reordered_tmc /\
                  read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montmul_p256_reordered_tmc BIGNUM_MONTMUL_P256_REORDERED_CORRECT
   `[RBX; R12; R13; R14; R15]` 40);;

let BIGNUM_MONTMUL_P256_REORDERED_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 40),48) /\
        ALL (nonoverlapping (word_sub stackpointer (word 40),40))
            [(word pc,LENGTH bignum_montmul_p256_reordered_mc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256_reordered_mc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montmul_p256_reordered_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x; y] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 40),40)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTMUL_P256_REORDERED_NOIBT_SUBROUTINE_CORRECT));;
...
let BIGNUM_AMONTMUL_P256_REORDERED_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 40),48) /\
        ALL (nonoverlapping (word_sub stackpointer (word 40),40))
            [(word pc,LENGTH bignum_montmul_p256_reordered_tmc); (x,8 * 4); (y,8 * 4)] /\
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 40),40)])`,
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montmul_p256_reordered_tmc BIGNUM_AMONTMUL_P256_CORRECT
   `[RBX; R12; R13; R14; R15]` 40);;
 ...
 (`!z x y a b pc stackpointer returnaddress.
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 40),40)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_AMONTMUL_P256_REORDERED_NOIBT_SUBROUTINE_CORRECT));;
