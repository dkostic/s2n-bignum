...
needs "x86/proofs/utils/gcm_simd_simplify.ml";; (* per-step SIMD refold *)
...
(* ------------------------------------------------------------------------- *)
(* Per-step SIMD-refold machinery (`SIMD_SIMPLIFY_CONV`,                     *)
(* `SIMD_SIMPLIFY_TAC`, `VPSHUFB_BYTEREV_128`, `VPALIGNR_8_SWAP_128`,        *)
(* `VPALIGNR_8_SWAP_128_VIA_ZX_256`, `WORD_ZX_ZX_128`,                        *)
(* `GHASH_ABBREV_STEP_TAC`) lives in `x86/proofs/utils/gcm_simd_simplify.ml`  *)
(* -- see that file for the rationale and the comment block explaining why   *)
(* each rewrite is needed.  Milestone 7's stitched-loop proof re-uses the    *)
(* same identifiers.                                                          *)
(* ------------------------------------------------------------------------- *)
