...
let sha256_block_data_order_nohw2_mc = define_from_elf
  "sha256_block_data_order_nohw2_mc"
  (file_on_path !load_path "arm/sha2/sha256_block_data_order_nohw2.o");;

let NOHW2_EXEC = ARM_MK_EXEC_RULE sha256_block_data_order_nohw2_mc;;
...
let SHA256_BLOCK_DATA_ORDER_NOHW2_CORRECT = prove
 (`!num_blocks (blocks:(int32 list) list)
  ...
  ENSURES_WHILE_UP_TAC `num_blocks:num` `pc + 0x14` `pc + 0x188`
    `\i s. aligned_bytes_loaded s (word pc) sha256_block_data_order_nohw2_mc /\
           read SP s = stackpointer /\
           read X0 s = state_ptr /\
  ...
  [(* ===== num_blocks <> 0 ===== *)
   ...
   (* ===== Body: invariant(ii) at pc+0x14 => invariant(ii+1) at pc+0x188 *)
   ALL_TAC;

   (* ===== Back-edge: invariant(ii) at pc+0x188 => invariant(ii) at pc+0x14 *)
   X_GEN_TAC `i:num` THEN STRIP_TAC THEN
   REWRITE_TAC[MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI] THEN
   SUBGOAL_THEN `num_blocks - i < 2 EXP 64` ASSUME_TAC THENL
    [ASM_ARITH_TAC; ALL_TAC] THEN
   VAL_INT64_TAC `num_blocks - i` THEN
   ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC NOHW2_EXEC [1] THEN
   ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];

   (* ===== Exit: invariant(num_blocks) at pc+0x188 => postcondition at pc+0x18c *)
   REWRITE_TAC[MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI;
               NONOVERLAPPING_CLAUSES; SUB_REFL] THEN
   VAL_INT64_TAC `0` THEN
   ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC NOHW2_EXEC (1--1) THEN
  ...
  ENSURES_SEQUENCE_TAC `pc + 0x30`
    `\s. aligned_bytes_loaded s (word pc) sha256_block_data_order_nohw2_mc /\
         read SP s = stackpointer /\
         read X0 s = state_ptr /\
         read X1 s = dptr_i /\
  ...
  [(* Phase A: load 16 words from dptr_i, REV, store to stackpointer *)
   ...
   ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
    [ARITH_TAC;
     ...
     ARM_STEPS_TAC NOHW2_EXEC (1--1) THEN
     ...
     ASSUME_TAC THENL
      [FIRST_X_ASSUM(fun th ->
       ...
       ASM_REWRITE_TAC[];
       ALL_TAC] THEN
     ARM_STEPS_TAC NOHW2_EXEC (1--6) THEN
     ENSURES_FINAL_STATE_TAC THEN
  ...
  [(* Phase B: extend message schedule on stack *)
   ...
   CONJ_TAC THENL
    [ENSURES_INIT_TAC "s0" THEN
     ARM_STEPS_TAC NOHW2_EXEC (1--2) THEN
     ENSURES_FINAL_STATE_TAC THEN
     ASM_REWRITE_TAC[];
     ALL_TAC] THEN

   ENSURES_WHILE_UP2_TAC `48` `pc + 0x38` `pc + 0x80`
    `\i s. aligned_bytes_loaded s (word pc) sha256_block_data_order_nohw2_mc /\
           read SP s = stackpointer /\
           read X0 s = state_ptr /\
           read X1 s = dptr_i /\
    ...
    [ARITH_TAC;
     ...
     STRIP_ASSUME_TAC THENL
      [REPEAT CONJ_TAC THEN FIRST_X_ASSUM MATCH_MP_TAC THEN
       ...
       ALL_TAC] THEN
     ARM_STEPS_TAC NOHW2_EXEC (1--18) THEN
     ENSURES_FINAL_STATE_TAC THEN
     ASM_REWRITE_TAC[] THEN
     REPEAT CONJ_TAC THENL
      [REWRITE_TAC[VAL_WORD_SUB_EQ_0; VAL_WORD_ADD; VAL_WORD; DIMINDEX_64] THEN
  ...
  [(* Phase C: load state into W4..W11 and save to W19..W26 *)
   ...
   ARM_STEPS_TAC NOHW2_EXEC (1--16) THEN
   ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  [(* Phase D: 64-round compression loop *)
   ...
   ENSURES_WHILE_UP2_TAC `64` `pc + 0xc4` `pc + 0x140`
    `\i s. aligned_bytes_loaded s (word pc) sha256_block_data_order_nohw2_mc /\
           read SP s = stackpointer /\
           read X0 s = state_ptr /\
           read X1 s = dptr_i /\
   ...
   ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
    [ARITH_TAC;
     ...
     ARM_STEPS_TAC NOHW2_EXEC (1--31) THEN
     ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
     (* Normalize word_ror (produced by shifted-register EOR) to       *)
     (* word_subword(word_join ...) form to match the postcondition.   *)
     SIMP_TAC[GSYM WORD_SUBWORD_JOIN_SELF; DIMINDEX_32; ARITH] THEN
     SIMP_TAC[WORD_ZX_ZX; DIMINDEX_32; DIMINDEX_64; LE_REFL; ARITH;
              WORD_ZX_TRIVIAL] THEN
  ...
  (* ===== Phase E: split at pc+0x160 ===== *)
  ENSURES_SEQUENCE_TAC `pc + 0x160`
    `\s. aligned_bytes_loaded s (word pc) sha256_block_data_order_nohw2_mc /\
         read SP s = stackpointer /\
         read X0 s = state_ptr /\
         read X1 s = dptr_i /\
  ...
  [(* Phase E: 8 ADDs for add-back *)
   ENSURES_INIT_TAC "s0" THEN
   ARM_STEPS_TAC NOHW2_EXEC (1--8) THEN
   ENSURES_FINAL_STATE_TAC THEN
   ASM_REWRITE_TAC[] THEN
   SIMP_TAC[WORD_ZX_ZX; DIMINDEX_32; DIMINDEX_64; LE_REFL; ARITH;
            WORD_ZX_TRIVIAL] THEN
  ...
  ENSURES_INIT_TAC "s0" THEN
  ARM_STEPS_TAC NOHW2_EXEC (1--10) THEN
  ENSURES_FINAL_STATE_TAC THEN
  ASM_REWRITE_TAC[] THEN
  REPEAT CONJ_TAC THENL
...
let SHA256_BLOCK_DATA_ORDER_NOHW2_SUBROUTINE_CORRECT = prove
 (`!num_blocks (blocks:(int32 list) list)
    (a:int32) b c d (e:int32) f g h
  ...
  ARM_ADD_RETURN_STACK_TAC ~pre_post_nsteps:(5,5) NOHW2_EXEC
        SHA256_BLOCK_DATA_ORDER_NOHW2_CORRECT
