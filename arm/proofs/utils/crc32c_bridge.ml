...
(* ------------------------------------------------------------------------- *)
(* Residue=9 helpers (8-byte CRC32CX path then 1-byte CRC32CB path).          *)
(* Composed from RESIDUE8_X_UPDATE and RESIDUE1_X_UPDATE via                   *)
(* crc32c_bytes_APPEND, splitting the 9-byte tail of bs into [n,n+8) and      *)
(* [n+8,n+9).                                                                 *)
(* ------------------------------------------------------------------------- *)

let RESIDUE9_X_UPDATE = prove
 (`!(bs:byte list) n.
        LENGTH bs = n + 9
        ==> crc32c_hw_step
              (crc32c_hw_step
              ...
              (word_subword
                (word_zx
                  (word(num_of_bytelist
                          (SUB_LIST(n + 8, 1) bs)):byte):int32)
                (0, 8):byte)
            = crc32c_bytes (word 0xFFFFFFFF:int32) bs`,
  REPEAT STRIP_TAC THEN
  REWRITE_TAC[WORD_SUBWORD_ZX_BYTE_TRIVIAL] THEN
  SUBGOAL_THEN
    `?b0 b1 b2 b3 b4 b5 b6 b7:byte.
        SUB_LIST (n, 8) (bs:byte list) = [b0; b1; b2; b3; b4; b5; b6; b7]`
   STRIP_ASSUME_TAC THENL
   [MATCH_MP_TAC LENGTH_EQ_8_DECOMP THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
    `SUB_LIST (n + 8, 1) (bs:byte list) = [EL (n + 8) bs]`
   SUBST1_TAC THENL
   [ASM_SIMP_TAC[SUB_LIST_1;
      ARITH_RULE `LENGTH (bs:byte list) = n + 9 ==> n + 8 < LENGTH bs`];
    ALL_TAC] THEN
  SUBGOAL_THEN
    `word (num_of_bytelist [EL (n + 8) (bs:byte list)]) = EL (n + 8) bs:byte`
  SUBST1_TAC THENL
   [REWRITE_TAC[num_of_bytelist; MULT_0; ADD_0] THEN CONV_TAC WORD_BLAST;
    ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[crc32c_bytes_APPEND; crc32c_bytes];
    ...
    REWRITE_TAC[GSYM APPEND_ASSOC] THEN
    SUBGOAL_THEN
      `APPEND [b0; b1; b2; b3; b4; b5; b6; b7] [EL (n + 8) (bs:byte list)] =
       SUB_LIST(n, 9) bs`
    SUBST1_TAC THENL
     [POP_ASSUM(SUBST1_TAC o SYM) THEN
      SUBGOAL_THEN `[EL (n + 8) (bs:byte list)] = SUB_LIST(n + 8, 1) bs`
      SUBST1_TAC THENL
       [ASM_SIMP_TAC[SUB_LIST_1;
          ARITH_RULE `LENGTH (bs:byte list) = n + 9 ==> n + 8 < LENGTH bs`];
        ALL_TAC] THEN
      MP_TAC(ISPECL [`bs:byte list`; `8:num`; `1:num`; `n:num`]
                    SUB_LIST_SPLIT) THEN
    ...
    MP_TAC(ISPECL [`bs:byte list`; `n:num`] SUB_LIST_TOPSPLIT) THEN
    ASM_SIMP_TAC[ARITH_RULE
      `LENGTH (bs:byte list) = n + 9 ==> LENGTH bs - n = 9`]]);;

(* ------------------------------------------------------------------------- *)
(* RESIDUE9_MEMORY_CLOSE: per-buffer memory zero-projection for residue=9.   *)
(* Composes the orthogonal-prefix lift `bytelist (a, k) s = ZEROES`           *)
(* with the post-STR int64 zero `bytes64 (a + k) s = word 0`                  *)
(* and the post-STRB int8 zero `bytes8 (a + (k + 8)) s = word 0`              *)
(* into the full-buffer zero-fill `bytelist (a, k + 9) s = ZEROES`.           *)
(* ------------------------------------------------------------------------- *)

let RESIDUE9_MEMORY_CLOSE = prove
 (`!(a:int64) (s:armstate) k.
        read (memory :> bytelist (a, k)) s = REPLICATE k (word 0) /\
        read (memory :> bytes64 (word_add a (word k))) s = word 0 /\
        read (memory :> bytes8 (word_add a (word (k + 8)))) s = word 0
        ==> read (memory :> bytelist (a, k + 9)) s =
            REPLICATE (k + 9) (word 0)`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN `k + 9 = (k + 8) + 1` SUBST1_TAC THENL
   [ARITH_TAC; ALL_TAC] THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=9 per-buffer cut-state adapters.                                   *)
(* The cut state on entry to the residue=9 case has a 9-byte suffix           *)
(* bytelist hypothesis                                                       *)
(*    read (memory :> bytelist (word_add a (word k), 9)) s =                  *)
(*      SUB_LIST(k, 9) bs                                                     *)
(* (with LENGTH bs = k + 9). The kernel's tail-block performs both an LDR X   *)
(* (8 bytes at offset k) and an LDRB (1 byte at offset k+8); we stage the     *)
(* corresponding bytes64 / bytes8 reads at s0 so ARM_STEPS_TAC can resolve    *)
(* both. They are pre-store reads at s0 — the post-imm STR XZR writes at      *)
(* a + k overlap only the 8-byte half, leaving the 1-byte half of the         *)
(* bytelist intact at a + (k + 8) for the subsequent LDRB.                    *)
(* ------------------------------------------------------------------------- *)

let RESIDUE9_BYTELIST_TO_BYTES64 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 9 /\
        read (memory :> bytelist (word_add a (word k), 9)) s =
          SUB_LIST (k, 9) bs
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k, 9) (bs:byte list) =
       APPEND (SUB_LIST(k, 8) bs) (SUB_LIST(k + 8, 1) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `8:num`; `1:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `8 + 1 = 9`];
      ALL_TAC] THEN
    SUBGOAL_THEN
      `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
       LENGTH (SUB_LIST(k + 8, 1) (bs:byte list)) = 1`
    STRIP_ASSUME_TAC THENL
     [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word k), 9)) s =
                 SUB_LIST (k, 9) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `9 = LENGTH (APPEND (SUB_LIST(k, 8) (bs:byte list))
                          (SUB_LIST(k + 8, 1) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE9_BYTELIST_TO_BYTES8 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 9 /\
        read (memory :> bytelist (word_add a (word k), 9)) s =
          SUB_LIST (k, 9) bs
        ==> read (memory :> bytes8 (word_add a (word (k + 8)))) s =
            word(num_of_bytelist (SUB_LIST(k + 8, 1) bs))`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
     LENGTH (SUB_LIST(k + 8, 1) (bs:byte list)) = 1`
  ...
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 8)), 1)) (s:armstate) =
     SUB_LIST (k + 8, 1) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `read (memory :> bytelist (word_add a (word k), 9)) (s:armstate) =
       APPEND (SUB_LIST(k, 8) (bs:byte list)) (SUB_LIST(k + 8, 1) bs)`
    MP_TAC THENL
     [ASM_REWRITE_TAC[] THEN
      MP_TAC(ISPECL [`bs:byte list`; `8:num`; `1:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
    ...
    ALL_TAC] THEN
  MP_TAC(ISPECL [`word_add a (word (k + 8)):int64`; `s:armstate`;
                 `SUB_LIST(k + 8, 1) (bs:byte list)`; `0:num`]
                BYTES8_FROM_BYTELIST) THEN
  ASM_REWRITE_TAC[ARITH; LE_REFL] THEN
  REWRITE_TAC[WORD_ADD_0] THEN
  SUBGOAL_THEN
    `SUB_LIST(0, 1) (SUB_LIST(k + 8, 1) (bs:byte list)) =
     SUB_LIST(k + 8, 1) bs`
  SUBST1_TAC THENL
   [FIRST_ASSUM(fun th ->
      GEN_REWRITE_TAC (LAND_CONV o LAND_CONV o RAND_CONV) [SYM th]) THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=6 helpers (4-byte CRC32CW path then 2-byte CRC32CH path).          *)
(* Composed from RESIDUE4_X_UPDATE and RESIDUE2_X_UPDATE via                   *)
(* crc32c_bytes_APPEND, splitting the 6-byte tail of bs into [n,n+4) and      *)
(* [n+4,n+6).                                                                 *)
(* ------------------------------------------------------------------------- *)

let RESIDUE6_X_UPDATE = prove
 (`!(bs:byte list) n.
        LENGTH bs = n + 6
        ==> crc32c_hw_step
              (crc32c_hw_step
  ...
  SUBGOAL_THEN
    `?c0 c1:byte.
        SUB_LIST (n + 4, 2) (bs:byte list) = [c0; c1]`
   STRIP_ASSUME_TAC THENL
   [MATCH_MP_TAC LENGTH_EQ_2_DECOMP THEN
    REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
    ALL_TAC] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[crc32c_bytes_APPEND; crc32c_bytes];
    ...
    REWRITE_TAC[GSYM APPEND_ASSOC] THEN
    SUBGOAL_THEN
      `APPEND [b0; b1; b2; b3] [c0; c1] =
       SUB_LIST(n, 6) (bs:byte list)`
    SUBST1_TAC THENL
     [POP_ASSUM(SUBST1_TAC o SYM) THEN POP_ASSUM(SUBST1_TAC o SYM) THEN
      MP_TAC(ISPECL [`bs:byte list`; `4:num`; `2:num`; `n:num`]
                    SUB_LIST_SPLIT) THEN
    ...
    MP_TAC(ISPECL [`bs:byte list`; `n:num`] SUB_LIST_TOPSPLIT) THEN
    ASM_SIMP_TAC[ARITH_RULE
      `LENGTH (bs:byte list) = n + 6 ==> LENGTH bs - n = 6`]]);;

(* ------------------------------------------------------------------------- *)
(* RESIDUE6_MEMORY_CLOSE: per-buffer memory zero-projection for residue=6.   *)
(* Composes the orthogonal-prefix lift `bytelist (a, k) s = ZEROES`           *)
(* with the post-STR int32 zero `bytes32 (a + k) s = word 0`                  *)
(* and the post-STRH int16 zero `bytes16 (a + (k + 4)) s = word 0`            *)
(* into the full-buffer zero-fill `bytelist (a, k + 6) s = ZEROES`.           *)
(* ------------------------------------------------------------------------- *)

let RESIDUE6_MEMORY_CLOSE = prove
 (`!(a:int64) (s:armstate) k.
        read (memory :> bytelist (a, k)) s = REPLICATE k (word 0) /\
        read (memory :> bytes32 (word_add a (word k))) s = word 0 /\
        read (memory :> bytes16 (word_add a (word (k + 4)))) s = word 0
        ==> read (memory :> bytelist (a, k + 6)) s =
            REPLICATE (k + 6) (word 0)`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN `k + 6 = (k + 4) + 2` SUBST1_TAC THENL
   [ARITH_TAC; ALL_TAC] THEN
  MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
  ASM_REWRITE_TAC[] THEN
  MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=6 per-buffer cut-state adapters.                                   *)
(* The cut state on entry to the residue=6 case has a 6-byte suffix           *)
(* bytelist hypothesis                                                       *)
(*    read (memory :> bytelist (word_add a (word k), 6)) s =                  *)
(*      SUB_LIST(k, 6) bs                                                     *)
(* (with LENGTH bs = k + 6). The kernel's tail-block performs both an LDR W   *)
(* (4 bytes at offset k) and an LDRH (2 bytes at offset k+4); we stage the    *)
(* corresponding bytes32 / bytes16 reads at s0 so ARM_STEPS_TAC can resolve   *)
(* both. They are pre-store reads at s0 — the post-imm STR WZR writes at      *)
(* a + k overlap only the 4-byte half, leaving the 2-byte half of the         *)
(* bytelist intact at a + (k + 4) for the subsequent LDRH.                    *)
(* ------------------------------------------------------------------------- *)

let RESIDUE6_BYTELIST_TO_BYTES32 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 6 /\
        read (memory :> bytelist (word_add a (word k), 6)) s =
          SUB_LIST (k, 6) bs
  ...
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
    `read (memory :> bytelist (word_add a (word k), 4)) (s:armstate) =
     SUB_LIST (k, 4) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k, 6) (bs:byte list) =
       APPEND (SUB_LIST(k, 4) bs) (SUB_LIST(k + 4, 2) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `4:num`; `2:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `4 + 2 = 6`];
      ALL_TAC] THEN
    SUBGOAL_THEN
      `LENGTH (SUB_LIST(k, 4) (bs:byte list)) = 4 /\
       LENGTH (SUB_LIST(k + 4, 2) (bs:byte list)) = 2`
    STRIP_ASSUME_TAC THENL
     [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word k), 6)) s =
                 SUB_LIST (k, 6) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `6 = LENGTH (APPEND (SUB_LIST(k, 4) (bs:byte list))
                          (SUB_LIST(k + 4, 2) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE6_BYTELIST_TO_BYTES16 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 6 /\
        read (memory :> bytelist (word_add a (word k), 6)) s =
          SUB_LIST (k, 6) bs
        ==> read (memory :> bytes16 (word_add a (word (k + 4)))) s =
            word(num_of_bytelist (SUB_LIST(k + 4, 2) bs))`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 4) (bs:byte list)) = 4 /\
     LENGTH (SUB_LIST(k + 4, 2) (bs:byte list)) = 2`
  ...
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 4)), 2)) (s:armstate) =
     SUB_LIST (k + 4, 2) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `read (memory :> bytelist (word_add a (word k), 6)) (s:armstate) =
       APPEND (SUB_LIST(k, 4) (bs:byte list)) (SUB_LIST(k + 4, 2) bs)`
    MP_TAC THENL
     [ASM_REWRITE_TAC[] THEN
      MP_TAC(ISPECL [`bs:byte list`; `4:num`; `2:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
    ...
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `word_add (word_add (a:int64) (word k)) (word 4) =
       word_add a (word (k + 4))`
    SUBST1_TAC THENL
     [CONV_TAC WORD_RULE; ALL_TAC] THEN
    STRIP_TAC THEN ASM_REWRITE_TAC[];
    ALL_TAC] THEN
  MP_TAC(ISPECL [`word_add a (word (k + 4)):int64`; `s:armstate`;
                 `SUB_LIST(k + 4, 2) (bs:byte list)`; `0:num`]
                BYTES16_FROM_BYTELIST) THEN
  ASM_REWRITE_TAC[ARITH; LE_REFL] THEN
  REWRITE_TAC[WORD_ADD_0] THEN
  SUBGOAL_THEN
    `SUB_LIST(0, 2) (SUB_LIST(k + 4, 2) (bs:byte list)) =
     SUB_LIST(k + 4, 2) bs`
  SUBST1_TAC THENL
   [FIRST_ASSUM(fun th ->
      GEN_REWRITE_TAC (LAND_CONV o LAND_CONV o RAND_CONV) [SYM th]) THEN
    ...
    DISCH_THEN ACCEPT_TAC]);;
(* ------------------------------------------------------------------------- *)
(* Residue=5 helpers (4-byte CRC32CW path then 1-byte CRC32CB path).          *)
(* Composed from RESIDUE4_X_UPDATE and RESIDUE1_X_UPDATE via                   *)
(* crc32c_bytes_APPEND, splitting the 5-byte tail of bs into [n,n+4) and      *)
(* [n+4,n+5).                                                                 *)
(* ------------------------------------------------------------------------- *)

let RESIDUE5_X_UPDATE = prove
 (`!(bs:byte list) n.
        LENGTH bs = n + 5
        ==> crc32c_hw_step
              (crc32c_hw_step
  ...
  SUBGOAL_THEN
   ...
   [MATCH_MP_TAC LENGTH_EQ_4_DECOMP THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
    `SUB_LIST (n + 4, 1) (bs:byte list) = [EL (n + 4) bs]`
   SUBST1_TAC THENL
   [ASM_SIMP_TAC[SUB_LIST_1;
      ARITH_RULE `LENGTH (bs:byte list) = n + 5 ==> n + 4 < LENGTH bs`];
    ALL_TAC] THEN
  SUBGOAL_THEN
    `word (num_of_bytelist [EL (n + 4) (bs:byte list)]) = EL (n + 4) bs:byte`
  SUBST1_TAC THENL
   [REWRITE_TAC[num_of_bytelist; MULT_0; ADD_0] THEN CONV_TAC WORD_BLAST;
    ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[crc32c_bytes_APPEND; crc32c_bytes];
    ...
    REWRITE_TAC[GSYM APPEND_ASSOC] THEN
    SUBGOAL_THEN
      `APPEND [b0; b1; b2; b3] [EL (n + 4) (bs:byte list)] =
       SUB_LIST(n, 5) bs`
    SUBST1_TAC THENL
     [POP_ASSUM(SUBST1_TAC o SYM) THEN
      SUBGOAL_THEN `[EL (n + 4) (bs:byte list)] = SUB_LIST(n + 4, 1) bs`
      SUBST1_TAC THENL
       [ASM_SIMP_TAC[SUB_LIST_1;
          ARITH_RULE `LENGTH (bs:byte list) = n + 5 ==> n + 4 < LENGTH bs`];
        ALL_TAC] THEN
      MP_TAC(ISPECL [`bs:byte list`; `4:num`; `1:num`; `n:num`]
                    SUB_LIST_SPLIT) THEN
    ...
    MP_TAC(ISPECL [`bs:byte list`; `n:num`] SUB_LIST_TOPSPLIT) THEN
    ASM_SIMP_TAC[ARITH_RULE
      `LENGTH (bs:byte list) = n + 5 ==> LENGTH bs - n = 5`]]);;

(* ------------------------------------------------------------------------- *)
(* RESIDUE5_MEMORY_CLOSE: per-buffer memory zero-projection for residue=5.   *)
(* Composes the orthogonal-prefix lift `bytelist (a, k) s = ZEROES`           *)
(* with the post-STR int32 zero `bytes32 (a + k) s = word 0`                  *)
(* and the post-STRB int8 zero `bytes8 (a + (k + 4)) s = word 0`              *)
(* into the full-buffer zero-fill `bytelist (a, k + 5) s = ZEROES`.           *)
(* ------------------------------------------------------------------------- *)

let RESIDUE5_MEMORY_CLOSE = prove
 (`!(a:int64) (s:armstate) k.
        read (memory :> bytelist (a, k)) s = REPLICATE k (word 0) /\
        read (memory :> bytes32 (word_add a (word k))) s = word 0 /\
        read (memory :> bytes8 (word_add a (word (k + 4)))) s = word 0
        ==> read (memory :> bytelist (a, k + 5)) s =
            REPLICATE (k + 5) (word 0)`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN `k + 5 = (k + 4) + 1` SUBST1_TAC THENL
   [ARITH_TAC; ALL_TAC] THEN
  MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
  ASM_REWRITE_TAC[] THEN
  MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=5 per-buffer cut-state adapters.                                   *)
(* The cut state on entry to the residue=5 case has a 5-byte suffix           *)
(* bytelist hypothesis                                                       *)
(*    read (memory :> bytelist (word_add a (word k), 5)) s =                  *)
(*      SUB_LIST(k, 5) bs                                                     *)
(* (with LENGTH bs = k + 5). The kernel's tail-block performs both an LDR W   *)
(* (4 bytes at offset k) and an LDRB (1 byte at offset k+4); we stage the     *)
(* corresponding bytes32 / bytes8 reads at s0 so ARM_STEPS_TAC can resolve    *)
(* both. They are pre-store reads at s0 — the post-imm STR WZR writes at      *)
(* a + k overlap only the 4-byte half, leaving the 1-byte half of the         *)
(* bytelist intact at a + (k + 4) for the subsequent LDRB.                    *)
(* ------------------------------------------------------------------------- *)

let RESIDUE5_BYTELIST_TO_BYTES32 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 5 /\
        read (memory :> bytelist (word_add a (word k), 5)) s =
          SUB_LIST (k, 5) bs
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k, 5) (bs:byte list) =
       APPEND (SUB_LIST(k, 4) bs) (SUB_LIST(k + 4, 1) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `4:num`; `1:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `4 + 1 = 5`];
      ALL_TAC] THEN
    SUBGOAL_THEN
      `LENGTH (SUB_LIST(k, 4) (bs:byte list)) = 4 /\
       LENGTH (SUB_LIST(k + 4, 1) (bs:byte list)) = 1`
    STRIP_ASSUME_TAC THENL
     [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word k), 5)) s =
                 SUB_LIST (k, 5) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `5 = LENGTH (APPEND (SUB_LIST(k, 4) (bs:byte list))
                          (SUB_LIST(k + 4, 1) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE5_BYTELIST_TO_BYTES8 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 5 /\
        read (memory :> bytelist (word_add a (word k), 5)) s =
          SUB_LIST (k, 5) bs
        ==> read (memory :> bytes8 (word_add a (word (k + 4)))) s =
            word(num_of_bytelist (SUB_LIST(k + 4, 1) bs))`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 4) (bs:byte list)) = 4 /\
     LENGTH (SUB_LIST(k + 4, 1) (bs:byte list)) = 1`
  ...
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 4)), 1)) (s:armstate) =
     SUB_LIST (k + 4, 1) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `read (memory :> bytelist (word_add a (word k), 5)) (s:armstate) =
       APPEND (SUB_LIST(k, 4) (bs:byte list)) (SUB_LIST(k + 4, 1) bs)`
    MP_TAC THENL
     [ASM_REWRITE_TAC[] THEN
      MP_TAC(ISPECL [`bs:byte list`; `4:num`; `1:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
    ...
    ALL_TAC] THEN
  MP_TAC(ISPECL [`word_add a (word (k + 4)):int64`; `s:armstate`;
                 `SUB_LIST(k + 4, 1) (bs:byte list)`; `0:num`]
                BYTES8_FROM_BYTELIST) THEN
  ASM_REWRITE_TAC[ARITH; LE_REFL] THEN
  REWRITE_TAC[WORD_ADD_0] THEN
  SUBGOAL_THEN
    `SUB_LIST(0, 1) (SUB_LIST(k + 4, 1) (bs:byte list)) =
     SUB_LIST(k + 4, 1) bs`
  SUBST1_TAC THENL
   [FIRST_ASSUM(fun th ->
      GEN_REWRITE_TAC (LAND_CONV o LAND_CONV o RAND_CONV) [SYM th]) THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=3 helpers (2-byte CRC32CH path then 1-byte CRC32CB path).          *)
(* Composed from RESIDUE2_X_UPDATE and RESIDUE1_X_UPDATE via                   *)
(* crc32c_bytes_APPEND, splitting the 3-byte tail of bs into [n,n+2) and      *)
(* [n+2,n+3).                                                                 *)
(* ------------------------------------------------------------------------- *)

let RESIDUE3_X_UPDATE = prove
 (`!(bs:byte list) n.
        LENGTH bs = n + 3
        ==> crc32c_hw_step
              (crc32c_hw_step
              ...
              (word_subword
                (word_zx
                  (word(num_of_bytelist
                          (SUB_LIST(n + 2, 1) bs)):byte):int32)
                (0, 8):byte)
            = crc32c_bytes (word 0xFFFFFFFF:int32) bs`,
  REPEAT STRIP_TAC THEN
  REWRITE_TAC[WORD_SUBWORD_ZX_INT16_BYTE0; WORD_SUBWORD_ZX_INT16_BYTE1;
              WORD_SUBWORD_ZX_BYTE_TRIVIAL] THEN
  SUBGOAL_THEN
    `?b0 b1:byte. SUB_LIST (n, 2) (bs:byte list) = [b0; b1]`
   STRIP_ASSUME_TAC THENL
   [MATCH_MP_TAC LENGTH_EQ_2_DECOMP THEN
    REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN
    `SUB_LIST (n + 2, 1) (bs:byte list) = [EL (n + 2) bs]`
   SUBST1_TAC THENL
   [ASM_SIMP_TAC[SUB_LIST_1;
      ARITH_RULE `LENGTH (bs:byte list) = n + 3 ==> n + 2 < LENGTH bs`];
    ALL_TAC] THEN
  SUBGOAL_THEN
    `word (num_of_bytelist [EL (n + 2) (bs:byte list)]) = EL (n + 2) bs:byte`
  SUBST1_TAC THENL
   [REWRITE_TAC[num_of_bytelist; MULT_0; ADD_0] THEN CONV_TAC WORD_BLAST;
    ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[crc32c_bytes_APPEND; crc32c_bytes];
    ...
    REWRITE_TAC[GSYM APPEND_ASSOC] THEN
    SUBGOAL_THEN
      `APPEND [b0; b1] [EL (n + 2) (bs:byte list)] =
       SUB_LIST(n, 3) bs`
    SUBST1_TAC THENL
     [POP_ASSUM(SUBST1_TAC o SYM) THEN
      SUBGOAL_THEN `[EL (n + 2) (bs:byte list)] = SUB_LIST(n + 2, 1) bs`
      SUBST1_TAC THENL
       [ASM_SIMP_TAC[SUB_LIST_1;
          ARITH_RULE `LENGTH (bs:byte list) = n + 3 ==> n + 2 < LENGTH bs`];
        ALL_TAC] THEN
      MP_TAC(ISPECL [`bs:byte list`; `2:num`; `1:num`; `n:num`]
                    SUB_LIST_SPLIT) THEN
    ...
    MP_TAC(ISPECL [`bs:byte list`; `n:num`] SUB_LIST_TOPSPLIT) THEN
    ASM_SIMP_TAC[ARITH_RULE
      `LENGTH (bs:byte list) = n + 3 ==> LENGTH bs - n = 3`]]);;

(* ------------------------------------------------------------------------- *)
(* RESIDUE3_MEMORY_CLOSE: per-buffer memory zero-projection for residue=3.   *)
(* Composes the orthogonal-prefix lift `bytelist (a, k) s = ZEROES`           *)
(* with the post-STRH int16 zero `bytes16 (a + k) s = word 0`                 *)
(* and the post-STRB int8 zero `bytes8 (a + (k + 2)) s = word 0`              *)
(* into the full-buffer zero-fill `bytelist (a, k + 3) s = ZEROES`.           *)
(* ------------------------------------------------------------------------- *)

let RESIDUE3_MEMORY_CLOSE = prove
 (`!(a:int64) (s:armstate) k.
        read (memory :> bytelist (a, k)) s = REPLICATE k (word 0) /\
        read (memory :> bytes16 (word_add a (word k))) s = word 0 /\
        read (memory :> bytes8 (word_add a (word (k + 2)))) s = word 0
        ==> read (memory :> bytelist (a, k + 3)) s =
            REPLICATE (k + 3) (word 0)`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN `k + 3 = (k + 2) + 1` SUBST1_TAC THENL
   [ARITH_TAC; ALL_TAC] THEN
  MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
  ASM_REWRITE_TAC[] THEN
  MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=3 per-buffer cut-state adapters.                                   *)
(* The cut state on entry to the residue=3 case has a 3-byte suffix           *)
(* bytelist hypothesis                                                        *)
(*    read (memory :> bytelist (word_add a (word k), 3)) s =                  *)
(*      SUB_LIST(k, 3) bs                                                     *)
(* (with LENGTH bs = k + 3). The kernel's tail-block performs both an LDRH    *)
(* (2 bytes at offset k) and an LDRB (1 byte at offset k+2); we stage the     *)
(* corresponding bytes16 / bytes8 reads at s0 so ARM_STEPS_TAC can resolve    *)
(* both. They are pre-store reads at s0 — the post-imm STRH WZR writes at     *)
(* a + k overlap only the 2-byte half, leaving the 1-byte half of the         *)
(* bytelist intact at a + (k + 2) for the subsequent LDRB.                    *)
(* ------------------------------------------------------------------------- *)

let RESIDUE3_BYTELIST_TO_BYTES16 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 3 /\
        read (memory :> bytelist (word_add a (word k), 3)) s =
          SUB_LIST (k, 3) bs
  ...
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
    `read (memory :> bytelist (word_add a (word k), 2)) (s:armstate) =
     SUB_LIST (k, 2) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k, 3) (bs:byte list) =
       APPEND (SUB_LIST(k, 2) bs) (SUB_LIST(k + 2, 1) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `2:num`; `1:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `2 + 1 = 3`];
      ALL_TAC] THEN
    SUBGOAL_THEN
      `LENGTH (SUB_LIST(k, 2) (bs:byte list)) = 2 /\
       LENGTH (SUB_LIST(k + 2, 1) (bs:byte list)) = 1`
    STRIP_ASSUME_TAC THENL
     [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word k), 3)) s =
                 SUB_LIST (k, 3) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `3 = LENGTH (APPEND (SUB_LIST(k, 2) (bs:byte list))
                          (SUB_LIST(k + 2, 1) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE3_BYTELIST_TO_BYTES8 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 3 /\
        read (memory :> bytelist (word_add a (word k), 3)) s =
          SUB_LIST (k, 3) bs
        ==> read (memory :> bytes8 (word_add a (word (k + 2)))) s =
            word(num_of_bytelist (SUB_LIST(k + 2, 1) bs))`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 2) (bs:byte list)) = 2 /\
     LENGTH (SUB_LIST(k + 2, 1) (bs:byte list)) = 1`
  ...
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 2)), 1)) (s:armstate) =
     SUB_LIST (k + 2, 1) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `read (memory :> bytelist (word_add a (word k), 3)) (s:armstate) =
       APPEND (SUB_LIST(k, 2) (bs:byte list)) (SUB_LIST(k + 2, 1) bs)`
    MP_TAC THENL
     [ASM_REWRITE_TAC[] THEN
      MP_TAC(ISPECL [`bs:byte list`; `2:num`; `1:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
    ...
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `word_add (word_add (a:int64) (word k)) (word 2) =
       word_add a (word (k + 2))`
    SUBST1_TAC THENL
     [CONV_TAC WORD_RULE; ALL_TAC] THEN
    STRIP_TAC THEN ASM_REWRITE_TAC[];
    ALL_TAC] THEN
  MP_TAC(ISPECL [`word_add a (word (k + 2)):int64`; `s:armstate`;
                 `SUB_LIST(k + 2, 1) (bs:byte list)`; `0:num`]
                BYTES8_FROM_BYTELIST) THEN
  ASM_REWRITE_TAC[ARITH; LE_REFL] THEN
  REWRITE_TAC[WORD_ADD_0] THEN
  SUBGOAL_THEN
    `SUB_LIST(0, 1) (SUB_LIST(k + 2, 1) (bs:byte list)) =
     SUB_LIST(k + 2, 1) bs`
  SUBST1_TAC THENL
   [FIRST_ASSUM(fun th ->
      GEN_REWRITE_TAC (LAND_CONV o LAND_CONV o RAND_CONV) [SYM th]) THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=14 helpers (8-byte CRC32CX path, 4-byte CRC32CW path, 2-byte      *)
(* CRC32CH path). Composed from RESIDUE8_X_UPDATE, RESIDUE4_X_UPDATE and     *)
(* RESIDUE2_X_UPDATE via crc32c_bytes_APPEND, splitting the 14-byte tail of  *)
(* bs into [n,n+8), [n+8,n+12) and [n+12,n+14).                              *)
(* ------------------------------------------------------------------------- *)

let RESIDUE14_X_UPDATE = prove
 (`!(bs:byte list) n.
        LENGTH bs = n + 14
        ==> crc32c_hw_step
              (crc32c_hw_step
                ...
                (word_subword
                  (word_zx
                    (word(num_of_bytelist
                            (SUB_LIST(n + 12, 2) bs)):int16):int32)
                  (0, 8):byte))
              (word_subword
                (word_zx
                  (word(num_of_bytelist
  ...
  SUBGOAL_THEN
    `?d0 d1:byte.
        SUB_LIST (n + 12, 2) (bs:byte list) = [d0; d1]`
   STRIP_ASSUME_TAC THENL
   [MATCH_MP_TAC LENGTH_EQ_2_DECOMP THEN
    REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
    ALL_TAC] THEN
   ...
   [REWRITE_TAC[crc32c_bytes_APPEND; crc32c_bytes];
    ...
    SUBGOAL_THEN
      `APPEND [b0; b1; b2; b3; b4; b5; b6; b7]
              (APPEND [c0; c1; c2; c3] [d0; d1]) =
       SUB_LIST(n, 14) (bs:byte list)`
    SUBST1_TAC THENL
     [POP_ASSUM(SUBST1_TAC o SYM) THEN
      POP_ASSUM(SUBST1_TAC o SYM) THEN
      POP_ASSUM(SUBST1_TAC o SYM) THEN
      MP_TAC(ISPECL [`bs:byte list`; `4:num`; `2:num`; `n + 8:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `4 + 2 = 6`;
                  ARITH_RULE `(n + 8) + 4 = n + 12`] THEN
      DISCH_THEN(SUBST1_TAC o SYM) THEN
      MP_TAC(ISPECL [`bs:byte list`; `8:num`; `6:num`; `n:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `8 + 6 = 14`] THEN
      DISCH_THEN(SUBST1_TAC o SYM) THEN REFL_TAC;
      ALL_TAC] THEN
    MP_TAC(ISPECL [`bs:byte list`; `n:num`] SUB_LIST_TOPSPLIT) THEN
    ASM_SIMP_TAC[ARITH_RULE
      `LENGTH (bs:byte list) = n + 14 ==> LENGTH bs - n = 14`]]);;

(* ------------------------------------------------------------------------- *)
(* RESIDUE14_MEMORY_CLOSE: per-buffer memory zero-projection for residue=14. *)
(* Composes the orthogonal-prefix lift `bytelist (a, k) s = ZEROES`           *)
(* with the post-STR int64 zero `bytes64 (a + k) s = word 0`                  *)
(* and the post-STR int32 zero `bytes32 (a + (k + 8)) s = word 0`             *)
(* and the post-STRH int16 zero `bytes16 (a + (k + 12)) s = word 0`           *)
(* into the full-buffer zero-fill `bytelist (a, k + 14) s = ZEROES`.          *)
(* ------------------------------------------------------------------------- *)

let RESIDUE14_MEMORY_CLOSE = prove
 (`!(a:int64) (s:armstate) k.
        read (memory :> bytelist (a, k)) s = REPLICATE k (word 0) /\
        read (memory :> bytes64 (word_add a (word k))) s = word 0 /\
        read (memory :> bytes32 (word_add a (word (k + 8)))) s = word 0 /\
        read (memory :> bytes16 (word_add a (word (k + 12)))) s = word 0
        ==> read (memory :> bytelist (a, k + 14)) s =
            REPLICATE (k + 14) (word 0)`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN `k + 14 = (k + 12) + 2` SUBST1_TAC THENL
   [ARITH_TAC; ALL_TAC] THEN
  MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
  ASM_REWRITE_TAC[] THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=14 per-buffer cut-state adapters.                                  *)
(* The cut state on entry to the residue=14 case has a 14-byte suffix         *)
(* bytelist hypothesis                                                       *)
(*    read (memory :> bytelist (word_add a (word k), 14)) s =                 *)
(*      SUB_LIST(k, 14) bs                                                    *)
(* (with LENGTH bs = k + 14). The kernel's tail-block performs an LDR X       *)
(* (8 bytes at offset k), an LDR W (4 bytes at offset k+8) and an LDRH        *)
(* (2 bytes at offset k+12); we stage the corresponding bytes64 / bytes32 /   *)
(* bytes16 reads at s0 so ARM_STEPS_TAC can resolve all three. They are       *)
(* pre-store reads at s0 — the post-imm STR XZR / STR WZR / STRH WZR writes   *)
(* are sequenced after the corresponding LDRs.                                *)
(* ------------------------------------------------------------------------- *)

let RESIDUE14_BYTELIST_TO_BYTES64 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 14 /\
        read (memory :> bytelist (word_add a (word k), 14)) s =
          SUB_LIST (k, 14) bs
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k, 14) (bs:byte list) =
       APPEND (SUB_LIST(k, 8) bs) (SUB_LIST(k + 8, 6) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `8:num`; `6:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `8 + 6 = 14`];
      ALL_TAC] THEN
    SUBGOAL_THEN
      `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
       LENGTH (SUB_LIST(k + 8, 6) (bs:byte list)) = 6`
    STRIP_ASSUME_TAC THENL
     [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word k), 14)) s =
                 SUB_LIST (k, 14) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `14 = LENGTH (APPEND (SUB_LIST(k, 8) (bs:byte list))
                           (SUB_LIST(k + 8, 6) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE14_BYTELIST_TO_BYTES32 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 14 /\
        read (memory :> bytelist (word_add a (word k), 14)) s =
          SUB_LIST (k, 14) bs
  ...
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
     LENGTH (SUB_LIST(k + 8, 4) (bs:byte list)) = 4 /\
     LENGTH (SUB_LIST(k + 12, 2) (bs:byte list)) = 2 /\
     LENGTH (SUB_LIST(k + 8, 6) (bs:byte list)) = 6`
  STRIP_ASSUME_TAC THENL
   [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 8)), 6)) (s:armstate) =
     SUB_LIST (k + 8, 6) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `read (memory :> bytelist (word_add a (word k), 14)) (s:armstate) =
       APPEND (SUB_LIST(k, 8) (bs:byte list)) (SUB_LIST(k + 8, 6) bs)`
    MP_TAC THENL
     [ASM_REWRITE_TAC[] THEN
      MP_TAC(ISPECL [`bs:byte list`; `8:num`; `6:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 8, 6) (bs:byte list) =
       APPEND (SUB_LIST(k + 8, 4) bs) (SUB_LIST(k + 12, 2) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `4:num`; `2:num`; `k + 8:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `4 + 2 = 6`;
                  ARITH_RULE `(k + 8) + 4 = k + 12`];
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word (k + 8)), 6)) s =
                 SUB_LIST (k + 8, 6) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `6 = LENGTH (APPEND (SUB_LIST(k + 8, 4) (bs:byte list))
                          (SUB_LIST(k + 12, 2) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE14_BYTELIST_TO_BYTES16 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 14 /\
        read (memory :> bytelist (word_add a (word k), 14)) s =
          SUB_LIST (k, 14) bs
        ==> read (memory :> bytes16 (word_add a (word (k + 12)))) s =
            word(num_of_bytelist (SUB_LIST(k + 12, 2) bs))`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
     LENGTH (SUB_LIST(k + 8, 4) (bs:byte list)) = 4 /\
  ...
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 12)), 2)) (s:armstate) =
     SUB_LIST (k + 12, 2) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 8, 6) (bs:byte list) =
       APPEND (SUB_LIST(k + 8, 4) bs) (SUB_LIST(k + 12, 2) bs)`
    ...
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `word_add (word_add (a:int64) (word (k + 8))) (word 4) =
       word_add a (word (k + 12))`
    SUBST1_TAC THENL
     [CONV_TAC WORD_RULE; ALL_TAC] THEN
    STRIP_TAC THEN ASM_REWRITE_TAC[];
    ALL_TAC] THEN
  MP_TAC(ISPECL [`word_add a (word (k + 12)):int64`; `s:armstate`;
                 `SUB_LIST(k + 12, 2) (bs:byte list)`; `0:num`]
                BYTES16_FROM_BYTELIST) THEN
  ASM_REWRITE_TAC[ARITH; LE_REFL] THEN
  REWRITE_TAC[WORD_ADD_0] THEN
  SUBGOAL_THEN
    `SUB_LIST(0, 2) (SUB_LIST(k + 12, 2) (bs:byte list)) =
     SUB_LIST(k + 12, 2) bs`
  SUBST1_TAC THENL
   [FIRST_ASSUM(fun th ->
      GEN_REWRITE_TAC (LAND_CONV o LAND_CONV o RAND_CONV) [SYM th]) THEN
    ...
    DISCH_THEN ACCEPT_TAC]);;
(* ------------------------------------------------------------------------- *)
(* Residue=15 helpers (8-byte CRC32CX path, 4-byte CRC32CW path,             *)
(* 2-byte CRC32CH path, 1-byte CRC32CB path). Composed from                  *)
(* RESIDUE8_X_UPDATE, RESIDUE4_X_UPDATE, RESIDUE2_X_UPDATE and               *)
(* RESIDUE1_X_UPDATE via crc32c_bytes_APPEND, splitting the 15-byte tail of  *)
(* bs into [n,n+8), [n+8,n+12), [n+12,n+14) and [n+14,n+15).                 *)
(* ------------------------------------------------------------------------- *)

let RESIDUE15_X_UPDATE = prove
 (`!(bs:byte list) n.
        LENGTH bs = n + 15
        ==> crc32c_hw_step
              (crc32c_hw_step
  ...
  SUBGOAL_THEN
   ...
   [MATCH_MP_TAC LENGTH_EQ_2_DECOMP THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
    `SUB_LIST (n + 14, 1) (bs:byte list) = [EL (n + 14) bs]`
   SUBST1_TAC THENL
   [ASM_SIMP_TAC[SUB_LIST_1;
      ARITH_RULE `LENGTH (bs:byte list) = n + 15 ==> n + 14 < LENGTH bs`];
    ALL_TAC] THEN
  SUBGOAL_THEN
    `word (num_of_bytelist [EL (n + 14) (bs:byte list)]) = EL (n + 14) bs:byte`
  SUBST1_TAC THENL
   [REWRITE_TAC[num_of_bytelist; MULT_0; ADD_0] THEN CONV_TAC WORD_BLAST;
    ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[crc32c_bytes_APPEND; crc32c_bytes];
    ...
    SUBGOAL_THEN
      `APPEND [b0; b1; b2; b3; b4; b5; b6; b7]
              (APPEND [c0; c1; c2; c3]
                      (APPEND [d0; d1] [EL (n + 14) (bs:byte list)])) =
       SUB_LIST(n, 15) (bs:byte list)`
    SUBST1_TAC THENL
     [POP_ASSUM(SUBST1_TAC o SYM) THEN
      POP_ASSUM(SUBST1_TAC o SYM) THEN
      POP_ASSUM(SUBST1_TAC o SYM) THEN
      SUBGOAL_THEN `[EL (n + 14) (bs:byte list)] = SUB_LIST(n + 14, 1) bs`
      SUBST1_TAC THENL
       [ASM_SIMP_TAC[SUB_LIST_1;
          ARITH_RULE `LENGTH (bs:byte list) = n + 15 ==> n + 14 < LENGTH bs`];
        ALL_TAC] THEN
      MP_TAC(ISPECL [`bs:byte list`; `2:num`; `1:num`; `n + 12:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `2 + 1 = 3`;
                  ARITH_RULE `(n + 12) + 2 = n + 14`] THEN
      DISCH_THEN(SUBST1_TAC o SYM) THEN
      MP_TAC(ISPECL [`bs:byte list`; `4:num`; `3:num`; `n + 8:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `4 + 3 = 7`;
                  ARITH_RULE `(n + 8) + 4 = n + 12`] THEN
      DISCH_THEN(SUBST1_TAC o SYM) THEN
      MP_TAC(ISPECL [`bs:byte list`; `8:num`; `7:num`; `n:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `8 + 7 = 15`] THEN
      DISCH_THEN(SUBST1_TAC o SYM) THEN REFL_TAC;
      ALL_TAC] THEN
    MP_TAC(ISPECL [`bs:byte list`; `n:num`] SUB_LIST_TOPSPLIT) THEN
    ASM_SIMP_TAC[ARITH_RULE
      `LENGTH (bs:byte list) = n + 15 ==> LENGTH bs - n = 15`]]);;

(* ------------------------------------------------------------------------- *)
(* RESIDUE15_MEMORY_CLOSE: per-buffer memory zero-projection for residue=15. *)
(* Composes the orthogonal-prefix lift `bytelist (a, k) s = ZEROES`           *)
(* with the post-STR int64 zero `bytes64 (a + k) s = word 0`                  *)
(* and the post-STR int32 zero `bytes32 (a + (k + 8)) s = word 0`             *)
(* and the post-STRH int16 zero `bytes16 (a + (k + 12)) s = word 0`           *)
(* and the post-STRB int8 zero `bytes8 (a + (k + 14)) s = word 0`             *)
(* into the full-buffer zero-fill `bytelist (a, k + 15) s = ZEROES`.          *)
(* ------------------------------------------------------------------------- *)

let RESIDUE15_MEMORY_CLOSE = prove
 (`!(a:int64) (s:armstate) k.
        read (memory :> bytelist (a, k)) s = REPLICATE k (word 0) /\
        read (memory :> bytes64 (word_add a (word k))) s = word 0 /\
        read (memory :> bytes32 (word_add a (word (k + 8)))) s = word 0 /\
        read (memory :> bytes16 (word_add a (word (k + 12)))) s = word 0 /\
        read (memory :> bytes8 (word_add a (word (k + 14)))) s = word 0
        ==> read (memory :> bytelist (a, k + 15)) s =
            REPLICATE (k + 15) (word 0)`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN `k + 15 = (k + 14) + 1` SUBST1_TAC THENL
   [ARITH_TAC; ALL_TAC] THEN
  MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
  ASM_REWRITE_TAC[] THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=15 per-buffer cut-state adapters.                                  *)
(* The cut state on entry to the residue=15 case has a 15-byte suffix         *)
(* bytelist hypothesis                                                        *)
(*    read (memory :> bytelist (word_add a (word k), 15)) s =                 *)
(*      SUB_LIST(k, 15) bs                                                    *)
(* (with LENGTH bs = k + 15). The kernel's tail-block performs an LDR X       *)
(* (8 bytes at offset k), an LDR W (4 bytes at offset k+8), an LDRH (2 bytes  *)
(* at offset k+12), and an LDRB (1 byte at offset k+14); we stage the         *)
(* corresponding bytes64 / bytes32 / bytes16 / bytes8 reads at s0 so          *)
(* ARM_STEPS_TAC can resolve all four. They are pre-store reads at s0 — the   *)
(* post-imm STR XZR / STR WZR / STRH WZR / STRB WZR writes are sequenced      *)
(* after the corresponding LDRs.                                              *)
(* ------------------------------------------------------------------------- *)

let RESIDUE15_BYTELIST_TO_BYTES64 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 15 /\
        read (memory :> bytelist (word_add a (word k), 15)) s =
          SUB_LIST (k, 15) bs
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k, 15) (bs:byte list) =
       APPEND (SUB_LIST(k, 8) bs) (SUB_LIST(k + 8, 7) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `8:num`; `7:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `8 + 7 = 15`];
      ALL_TAC] THEN
    SUBGOAL_THEN
      `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
       LENGTH (SUB_LIST(k + 8, 7) (bs:byte list)) = 7`
    STRIP_ASSUME_TAC THENL
     [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word k), 15)) s =
                 SUB_LIST (k, 15) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `15 = LENGTH (APPEND (SUB_LIST(k, 8) (bs:byte list))
                           (SUB_LIST(k + 8, 7) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE15_BYTELIST_TO_BYTES32 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 15 /\
        read (memory :> bytelist (word_add a (word k), 15)) s =
          SUB_LIST (k, 15) bs
  ...
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
     LENGTH (SUB_LIST(k + 8, 4) (bs:byte list)) = 4 /\
     LENGTH (SUB_LIST(k + 8, 7) (bs:byte list)) = 7 /\
     LENGTH (SUB_LIST(k + 12, 3) (bs:byte list)) = 3`
  STRIP_ASSUME_TAC THENL
   [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 8)), 7)) (s:armstate) =
     SUB_LIST (k + 8, 7) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `read (memory :> bytelist (word_add a (word k), 15)) (s:armstate) =
       APPEND (SUB_LIST(k, 8) (bs:byte list)) (SUB_LIST(k + 8, 7) bs)`
    MP_TAC THENL
     [ASM_REWRITE_TAC[] THEN
      MP_TAC(ISPECL [`bs:byte list`; `8:num`; `7:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 8, 7) (bs:byte list) =
       APPEND (SUB_LIST(k + 8, 4) bs) (SUB_LIST(k + 12, 3) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `4:num`; `3:num`; `k + 8:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `4 + 3 = 7`;
                  ARITH_RULE `(k + 8) + 4 = k + 12`];
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word (k + 8)), 7)) s =
                 SUB_LIST (k + 8, 7) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `7 = LENGTH (APPEND (SUB_LIST(k + 8, 4) (bs:byte list))
                          (SUB_LIST(k + 12, 3) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE15_BYTELIST_TO_BYTES16 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 15 /\
        read (memory :> bytelist (word_add a (word k), 15)) s =
          SUB_LIST (k, 15) bs
  ...
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
     LENGTH (SUB_LIST(k + 8, 4) (bs:byte list)) = 4 /\
     LENGTH (SUB_LIST(k + 12, 2) (bs:byte list)) = 2 /\
     LENGTH (SUB_LIST(k + 14, 1) (bs:byte list)) = 1 /\
     LENGTH (SUB_LIST(k + 8, 7) (bs:byte list)) = 7 /\
     LENGTH (SUB_LIST(k + 12, 3) (bs:byte list)) = 3`
  STRIP_ASSUME_TAC THENL
   [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
  ...
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 12)), 3)) (s:armstate) =
     SUB_LIST (k + 12, 3) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 8, 7) (bs:byte list) =
       APPEND (SUB_LIST(k + 8, 4) bs) (SUB_LIST(k + 12, 3) bs)`
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 12, 3) (bs:byte list) =
       APPEND (SUB_LIST(k + 12, 2) bs) (SUB_LIST(k + 14, 1) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `2:num`; `1:num`; `k + 12:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `2 + 1 = 3`;
                  ARITH_RULE `(k + 12) + 2 = k + 14`];
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word (k + 12)), 3)) s =
                 SUB_LIST (k + 12, 3) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `3 = LENGTH (APPEND (SUB_LIST(k + 12, 2) (bs:byte list))
                          (SUB_LIST(k + 14, 1) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE15_BYTELIST_TO_BYTES8 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 15 /\
        read (memory :> bytelist (word_add a (word k), 15)) s =
          SUB_LIST (k, 15) bs
        ==> read (memory :> bytes8 (word_add a (word (k + 14)))) s =
            word(num_of_bytelist (SUB_LIST(k + 14, 1) bs))`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
     LENGTH (SUB_LIST(k + 8, 4) (bs:byte list)) = 4 /\
  ...
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 14)), 1)) (s:armstate) =
     SUB_LIST (k + 14, 1) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 12, 3) (bs:byte list) =
       APPEND (SUB_LIST(k + 12, 2) bs) (SUB_LIST(k + 14, 1) bs)`
    ...
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `word_add (word_add (a:int64) (word (k + 12))) (word 2) =
       word_add a (word (k + 14))`
    SUBST1_TAC THENL
     [CONV_TAC WORD_RULE; ALL_TAC] THEN
    STRIP_TAC THEN ASM_REWRITE_TAC[];
    ALL_TAC] THEN
  MP_TAC(ISPECL [`word_add a (word (k + 14)):int64`; `s:armstate`;
                 `SUB_LIST(k + 14, 1) (bs:byte list)`; `0:num`]
                BYTES8_FROM_BYTELIST) THEN
  ASM_REWRITE_TAC[ARITH; LE_REFL] THEN
  REWRITE_TAC[WORD_ADD_0] THEN
  SUBGOAL_THEN
    `SUB_LIST(0, 1) (SUB_LIST(k + 14, 1) (bs:byte list)) =
     SUB_LIST(k + 14, 1) bs`
  SUBST1_TAC THENL
   [FIRST_ASSUM(fun th ->
      GEN_REWRITE_TAC (LAND_CONV o LAND_CONV o RAND_CONV) [SYM th]) THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=7 helpers (4-byte CRC32CW path, 2-byte CRC32CH path,              *)
(* 1-byte CRC32CB path). Composed from RESIDUE4_X_UPDATE,                    *)
(* RESIDUE2_X_UPDATE and RESIDUE1_X_UPDATE via crc32c_bytes_APPEND,          *)
(* splitting the 7-byte tail of bs into [n,n+4), [n+4,n+6) and [n+6,n+7).   *)
(* ------------------------------------------------------------------------- *)

let RESIDUE7_X_UPDATE = prove
 (`!(bs:byte list) n.
        LENGTH bs = n + 7
        ==> crc32c_hw_step
              (crc32c_hw_step
  ...
  SUBGOAL_THEN
   ...
   [MATCH_MP_TAC LENGTH_EQ_2_DECOMP THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
    `SUB_LIST (n + 6, 1) (bs:byte list) = [EL (n + 6) bs]`
   SUBST1_TAC THENL
   [ASM_SIMP_TAC[SUB_LIST_1;
      ARITH_RULE `LENGTH (bs:byte list) = n + 7 ==> n + 6 < LENGTH bs`];
    ALL_TAC] THEN
  SUBGOAL_THEN
    `word (num_of_bytelist [EL (n + 6) (bs:byte list)]) = EL (n + 6) bs:byte`
  SUBST1_TAC THENL
   [REWRITE_TAC[num_of_bytelist; MULT_0; ADD_0] THEN CONV_TAC WORD_BLAST;
    ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[crc32c_bytes_APPEND; crc32c_bytes];
    ...
    SUBGOAL_THEN
      `APPEND [b0; b1; b2; b3]
              (APPEND [c0; c1] [EL (n + 6) (bs:byte list)]) =
       SUB_LIST(n, 7) (bs:byte list)`
    SUBST1_TAC THENL
     [POP_ASSUM(SUBST1_TAC o SYM) THEN
      POP_ASSUM(SUBST1_TAC o SYM) THEN
      SUBGOAL_THEN `[EL (n + 6) (bs:byte list)] = SUB_LIST(n + 6, 1) bs`
      SUBST1_TAC THENL
       [ASM_SIMP_TAC[SUB_LIST_1;
          ARITH_RULE `LENGTH (bs:byte list) = n + 7 ==> n + 6 < LENGTH bs`];
        ALL_TAC] THEN
      MP_TAC(ISPECL [`bs:byte list`; `2:num`; `1:num`; `n + 4:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `2 + 1 = 3`;
                  ARITH_RULE `(n + 4) + 2 = n + 6`] THEN
      DISCH_THEN(SUBST1_TAC o SYM) THEN
      MP_TAC(ISPECL [`bs:byte list`; `4:num`; `3:num`; `n:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `4 + 3 = 7`] THEN
      DISCH_THEN(SUBST1_TAC o SYM) THEN REFL_TAC;
      ALL_TAC] THEN
    MP_TAC(ISPECL [`bs:byte list`; `n:num`] SUB_LIST_TOPSPLIT) THEN
    ASM_SIMP_TAC[ARITH_RULE
      `LENGTH (bs:byte list) = n + 7 ==> LENGTH bs - n = 7`]]);;

(* ------------------------------------------------------------------------- *)
(* RESIDUE7_MEMORY_CLOSE: per-buffer memory zero-projection for residue=7.   *)
(* Composes the orthogonal-prefix lift `bytelist (a, k) s = ZEROES`           *)
(* with the post-STR int32 zero `bytes32 (a + k) s = word 0`                  *)
(* and the post-STRH int16 zero `bytes16 (a + (k + 4)) s = word 0`            *)
(* and the post-STRB int8 zero `bytes8 (a + (k + 6)) s = word 0`              *)
(* into the full-buffer zero-fill `bytelist (a, k + 7) s = ZEROES`.           *)
(* ------------------------------------------------------------------------- *)

let RESIDUE7_MEMORY_CLOSE = prove
 (`!(a:int64) (s:armstate) k.
        read (memory :> bytelist (a, k)) s = REPLICATE k (word 0) /\
        read (memory :> bytes32 (word_add a (word k))) s = word 0 /\
        read (memory :> bytes16 (word_add a (word (k + 4)))) s = word 0 /\
        read (memory :> bytes8 (word_add a (word (k + 6)))) s = word 0
        ==> read (memory :> bytelist (a, k + 7)) s =
            REPLICATE (k + 7) (word 0)`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN `k + 7 = (k + 6) + 1` SUBST1_TAC THENL
   [ARITH_TAC; ALL_TAC] THEN
  MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
  ASM_REWRITE_TAC[] THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=7 per-buffer cut-state adapters.                                   *)
(* The cut state on entry to the residue=7 case has a 7-byte suffix           *)
(* bytelist hypothesis                                                        *)
(*    read (memory :> bytelist (word_add a (word k), 7)) s =                  *)
(*      SUB_LIST(k, 7) bs                                                     *)
(* (with LENGTH bs = k + 7). The kernel's tail-block performs an LDR W        *)
(* (4 bytes at offset k), an LDRH (2 bytes at offset k+4) and an LDRB         *)
(* (1 byte at offset k+6); we stage the corresponding bytes32 / bytes16 /     *)
(* bytes8 reads at s0 so ARM_STEPS_TAC can resolve all three. They are        *)
(* pre-store reads at s0 — the post-imm STR WZR / STRH WZR / STRB WZR writes  *)
(* are sequenced after the corresponding LDRs.                                *)
(* ------------------------------------------------------------------------- *)

let RESIDUE7_BYTELIST_TO_BYTES32 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 7 /\
        read (memory :> bytelist (word_add a (word k), 7)) s =
          SUB_LIST (k, 7) bs
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k, 7) (bs:byte list) =
       APPEND (SUB_LIST(k, 4) bs) (SUB_LIST(k + 4, 3) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `4:num`; `3:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `4 + 3 = 7`];
      ALL_TAC] THEN
    SUBGOAL_THEN
      `LENGTH (SUB_LIST(k, 4) (bs:byte list)) = 4 /\
       LENGTH (SUB_LIST(k + 4, 3) (bs:byte list)) = 3`
    STRIP_ASSUME_TAC THENL
     [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word k), 7)) s =
                 SUB_LIST (k, 7) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `7 = LENGTH (APPEND (SUB_LIST(k, 4) (bs:byte list))
                          (SUB_LIST(k + 4, 3) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE7_BYTELIST_TO_BYTES16 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 7 /\
        read (memory :> bytelist (word_add a (word k), 7)) s =
          SUB_LIST (k, 7) bs
  ...
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 4) (bs:byte list)) = 4 /\
     LENGTH (SUB_LIST(k + 4, 2) (bs:byte list)) = 2 /\
     LENGTH (SUB_LIST(k + 6, 1) (bs:byte list)) = 1 /\
     LENGTH (SUB_LIST(k + 4, 3) (bs:byte list)) = 3`
  STRIP_ASSUME_TAC THENL
   [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 4)), 3)) (s:armstate) =
     SUB_LIST (k + 4, 3) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `read (memory :> bytelist (word_add a (word k), 7)) (s:armstate) =
       APPEND (SUB_LIST(k, 4) (bs:byte list)) (SUB_LIST(k + 4, 3) bs)`
    MP_TAC THENL
     [ASM_REWRITE_TAC[] THEN
      MP_TAC(ISPECL [`bs:byte list`; `4:num`; `3:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 4, 3) (bs:byte list) =
       APPEND (SUB_LIST(k + 4, 2) bs) (SUB_LIST(k + 6, 1) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `2:num`; `1:num`; `k + 4:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `2 + 1 = 3`;
                  ARITH_RULE `(k + 4) + 2 = k + 6`];
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word (k + 4)), 3)) s =
                 SUB_LIST (k + 4, 3) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `3 = LENGTH (APPEND (SUB_LIST(k + 4, 2) (bs:byte list))
                          (SUB_LIST(k + 6, 1) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE7_BYTELIST_TO_BYTES8 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 7 /\
        read (memory :> bytelist (word_add a (word k), 7)) s =
          SUB_LIST (k, 7) bs
        ==> read (memory :> bytes8 (word_add a (word (k + 6)))) s =
            word(num_of_bytelist (SUB_LIST(k + 6, 1) bs))`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 4) (bs:byte list)) = 4 /\
     LENGTH (SUB_LIST(k + 4, 2) (bs:byte list)) = 2 /\
  ...
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 6)), 1)) (s:armstate) =
     SUB_LIST (k + 6, 1) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 4, 3) (bs:byte list) =
       APPEND (SUB_LIST(k + 4, 2) bs) (SUB_LIST(k + 6, 1) bs)`
    ...
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `word_add (word_add (a:int64) (word (k + 4))) (word 2) =
       word_add a (word (k + 6))`
    SUBST1_TAC THENL
     [CONV_TAC WORD_RULE; ALL_TAC] THEN
    STRIP_TAC THEN ASM_REWRITE_TAC[];
    ALL_TAC] THEN
  MP_TAC(ISPECL [`word_add a (word (k + 6)):int64`; `s:armstate`;
                 `SUB_LIST(k + 6, 1) (bs:byte list)`; `0:num`]
                BYTES8_FROM_BYTELIST) THEN
  ASM_REWRITE_TAC[ARITH; LE_REFL] THEN
  REWRITE_TAC[WORD_ADD_0] THEN
  SUBGOAL_THEN
    `SUB_LIST(0, 1) (SUB_LIST(k + 6, 1) (bs:byte list)) =
     SUB_LIST(k + 6, 1) bs`
  SUBST1_TAC THENL
   [FIRST_ASSUM(fun th ->
      GEN_REWRITE_TAC (LAND_CONV o LAND_CONV o RAND_CONV) [SYM th]) THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=11 helpers (8-byte CRC32CX path, 2-byte CRC32CH path,             *)
(* 1-byte CRC32CB path). Composed from RESIDUE8_X_UPDATE,                    *)
(* RESIDUE2_X_UPDATE and RESIDUE1_X_UPDATE via crc32c_bytes_APPEND,          *)
(* splitting the 11-byte tail of bs into [n,n+8), [n+8,n+10) and             *)
(* [n+10,n+11).                                                              *)
(* ------------------------------------------------------------------------- *)

let RESIDUE11_X_UPDATE = prove
 (`!(bs:byte list) n.
        LENGTH bs = n + 11
        ==> crc32c_hw_step
              (crc32c_hw_step
  ...
  SUBGOAL_THEN
   ...
   [MATCH_MP_TAC LENGTH_EQ_2_DECOMP THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
    `SUB_LIST (n + 10, 1) (bs:byte list) = [EL (n + 10) bs]`
   SUBST1_TAC THENL
   [ASM_SIMP_TAC[SUB_LIST_1;
      ARITH_RULE `LENGTH (bs:byte list) = n + 11 ==> n + 10 < LENGTH bs`];
    ALL_TAC] THEN
  SUBGOAL_THEN
    `word (num_of_bytelist [EL (n + 10) (bs:byte list)]) = EL (n + 10) bs:byte`
  SUBST1_TAC THENL
   [REWRITE_TAC[num_of_bytelist; MULT_0; ADD_0] THEN CONV_TAC WORD_BLAST;
    ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[crc32c_bytes_APPEND; crc32c_bytes];
    ...
    SUBGOAL_THEN
      `APPEND [b0; b1; b2; b3; b4; b5; b6; b7]
              (APPEND [c0; c1] [EL (n + 10) (bs:byte list)]) =
       SUB_LIST(n, 11) (bs:byte list)`
    SUBST1_TAC THENL
     [POP_ASSUM(SUBST1_TAC o SYM) THEN
      POP_ASSUM(SUBST1_TAC o SYM) THEN
      SUBGOAL_THEN `[EL (n + 10) (bs:byte list)] = SUB_LIST(n + 10, 1) bs`
      SUBST1_TAC THENL
       [ASM_SIMP_TAC[SUB_LIST_1;
          ARITH_RULE `LENGTH (bs:byte list) = n + 11 ==> n + 10 < LENGTH bs`];
        ALL_TAC] THEN
      MP_TAC(ISPECL [`bs:byte list`; `2:num`; `1:num`; `n + 8:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `2 + 1 = 3`;
                  ARITH_RULE `(n + 8) + 2 = n + 10`] THEN
      DISCH_THEN(SUBST1_TAC o SYM) THEN
      MP_TAC(ISPECL [`bs:byte list`; `8:num`; `3:num`; `n:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `8 + 3 = 11`] THEN
      DISCH_THEN(SUBST1_TAC o SYM) THEN REFL_TAC;
      ALL_TAC] THEN
    MP_TAC(ISPECL [`bs:byte list`; `n:num`] SUB_LIST_TOPSPLIT) THEN
    ASM_SIMP_TAC[ARITH_RULE
      `LENGTH (bs:byte list) = n + 11 ==> LENGTH bs - n = 11`]]);;

(* ------------------------------------------------------------------------- *)
(* RESIDUE11_MEMORY_CLOSE: per-buffer memory zero-projection for residue=11. *)
(* Composes the orthogonal-prefix lift `bytelist (a, k) s = ZEROES`           *)
(* with the post-STR int64 zero `bytes64 (a + k) s = word 0`                  *)
(* and the post-STRH int16 zero `bytes16 (a + (k + 8)) s = word 0`            *)
(* and the post-STRB int8 zero `bytes8 (a + (k + 10)) s = word 0`             *)
(* into the full-buffer zero-fill `bytelist (a, k + 11) s = ZEROES`.          *)
(* ------------------------------------------------------------------------- *)

let RESIDUE11_MEMORY_CLOSE = prove
 (`!(a:int64) (s:armstate) k.
        read (memory :> bytelist (a, k)) s = REPLICATE k (word 0) /\
        read (memory :> bytes64 (word_add a (word k))) s = word 0 /\
        read (memory :> bytes16 (word_add a (word (k + 8)))) s = word 0 /\
        read (memory :> bytes8 (word_add a (word (k + 10)))) s = word 0
        ==> read (memory :> bytelist (a, k + 11)) s =
            REPLICATE (k + 11) (word 0)`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN `k + 11 = (k + 10) + 1` SUBST1_TAC THENL
   [ARITH_TAC; ALL_TAC] THEN
  MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
  ASM_REWRITE_TAC[] THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=11 per-buffer cut-state adapters.                                  *)
(* The cut state on entry to the residue=11 case has an 11-byte suffix        *)
(* bytelist hypothesis                                                        *)
(*    read (memory :> bytelist (word_add a (word k), 11)) s =                 *)
(*      SUB_LIST(k, 11) bs                                                    *)
(* (with LENGTH bs = k + 11). The kernel's tail-block performs an LDR X       *)
(* (8 bytes at offset k), an LDRH (2 bytes at offset k+8) and an LDRB         *)
(* (1 byte at offset k+10); we stage the corresponding bytes64 / bytes16 /    *)
(* bytes8 reads at s0 so ARM_STEPS_TAC can resolve all three. They are        *)
(* pre-store reads at s0 — the post-imm STR XZR / STRH WZR / STRB WZR writes  *)
(* are sequenced after the corresponding LDRs.                                *)
(* ------------------------------------------------------------------------- *)

let RESIDUE11_BYTELIST_TO_BYTES64 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 11 /\
        read (memory :> bytelist (word_add a (word k), 11)) s =
          SUB_LIST (k, 11) bs
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k, 11) (bs:byte list) =
       APPEND (SUB_LIST(k, 8) bs) (SUB_LIST(k + 8, 3) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `8:num`; `3:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `8 + 3 = 11`];
      ALL_TAC] THEN
    SUBGOAL_THEN
      `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
       LENGTH (SUB_LIST(k + 8, 3) (bs:byte list)) = 3`
    STRIP_ASSUME_TAC THENL
     [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word k), 11)) s =
                 SUB_LIST (k, 11) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `11 = LENGTH (APPEND (SUB_LIST(k, 8) (bs:byte list))
                           (SUB_LIST(k + 8, 3) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE11_BYTELIST_TO_BYTES16 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 11 /\
        read (memory :> bytelist (word_add a (word k), 11)) s =
          SUB_LIST (k, 11) bs
  ...
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
     LENGTH (SUB_LIST(k + 8, 2) (bs:byte list)) = 2 /\
     LENGTH (SUB_LIST(k + 10, 1) (bs:byte list)) = 1 /\
     LENGTH (SUB_LIST(k + 8, 3) (bs:byte list)) = 3`
  STRIP_ASSUME_TAC THENL
   [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 8)), 3)) (s:armstate) =
     SUB_LIST (k + 8, 3) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `read (memory :> bytelist (word_add a (word k), 11)) (s:armstate) =
       APPEND (SUB_LIST(k, 8) (bs:byte list)) (SUB_LIST(k + 8, 3) bs)`
    MP_TAC THENL
     [ASM_REWRITE_TAC[] THEN
      MP_TAC(ISPECL [`bs:byte list`; `8:num`; `3:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 8, 3) (bs:byte list) =
       APPEND (SUB_LIST(k + 8, 2) bs) (SUB_LIST(k + 10, 1) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `2:num`; `1:num`; `k + 8:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `2 + 1 = 3`;
                  ARITH_RULE `(k + 8) + 2 = k + 10`];
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word (k + 8)), 3)) s =
                 SUB_LIST (k + 8, 3) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `3 = LENGTH (APPEND (SUB_LIST(k + 8, 2) (bs:byte list))
                          (SUB_LIST(k + 10, 1) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE11_BYTELIST_TO_BYTES8 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 11 /\
        read (memory :> bytelist (word_add a (word k), 11)) s =
          SUB_LIST (k, 11) bs
        ==> read (memory :> bytes8 (word_add a (word (k + 10)))) s =
            word(num_of_bytelist (SUB_LIST(k + 10, 1) bs))`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
     LENGTH (SUB_LIST(k + 8, 2) (bs:byte list)) = 2 /\
  ...
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 10)), 1)) (s:armstate) =
     SUB_LIST (k + 10, 1) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 8, 3) (bs:byte list) =
       APPEND (SUB_LIST(k + 8, 2) bs) (SUB_LIST(k + 10, 1) bs)`
    ...
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `word_add (word_add (a:int64) (word (k + 8))) (word 2) =
       word_add a (word (k + 10))`
    SUBST1_TAC THENL
     [CONV_TAC WORD_RULE; ALL_TAC] THEN
    STRIP_TAC THEN ASM_REWRITE_TAC[];
    ALL_TAC] THEN
  MP_TAC(ISPECL [`word_add a (word (k + 10)):int64`; `s:armstate`;
                 `SUB_LIST(k + 10, 1) (bs:byte list)`; `0:num`]
                BYTES8_FROM_BYTELIST) THEN
  ASM_REWRITE_TAC[ARITH; LE_REFL] THEN
  REWRITE_TAC[WORD_ADD_0] THEN
  SUBGOAL_THEN
    `SUB_LIST(0, 1) (SUB_LIST(k + 10, 1) (bs:byte list)) =
     SUB_LIST(k + 10, 1) bs`
  SUBST1_TAC THENL
   [FIRST_ASSUM(fun th ->
      GEN_REWRITE_TAC (LAND_CONV o LAND_CONV o RAND_CONV) [SYM th]) THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=13 helpers (8-byte CRC32CX path, 4-byte CRC32CW path,             *)
(* 1-byte CRC32CB path). Composed from RESIDUE8_X_UPDATE, RESIDUE4_X_UPDATE  *)
(* and RESIDUE1_X_UPDATE via crc32c_bytes_APPEND, splitting the 13-byte tail *)
(* of bs into [n,n+8), [n+8,n+12) and [n+12,n+13).                            *)
(* ------------------------------------------------------------------------- *)

let RESIDUE13_X_UPDATE = prove
 (`!(bs:byte list) n.
        LENGTH bs = n + 13
        ==> crc32c_hw_step
              (crc32c_hw_step
  ...
  SUBGOAL_THEN
   ...
   [MATCH_MP_TAC LENGTH_EQ_4_DECOMP THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
    `SUB_LIST (n + 12, 1) (bs:byte list) = [EL (n + 12) bs]`
   SUBST1_TAC THENL
   [ASM_SIMP_TAC[SUB_LIST_1;
      ARITH_RULE `LENGTH (bs:byte list) = n + 13 ==> n + 12 < LENGTH bs`];
    ALL_TAC] THEN
  SUBGOAL_THEN
    `word (num_of_bytelist [EL (n + 12) (bs:byte list)]) = EL (n + 12) bs:byte`
  SUBST1_TAC THENL
   [REWRITE_TAC[num_of_bytelist; MULT_0; ADD_0] THEN CONV_TAC WORD_BLAST;
    ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[crc32c_bytes_APPEND; crc32c_bytes];
    ...
    SUBGOAL_THEN
      `APPEND [b0; b1; b2; b3; b4; b5; b6; b7]
              (APPEND [c0; c1; c2; c3] [EL (n + 12) (bs:byte list)]) =
       SUB_LIST(n, 13) (bs:byte list)`
    SUBST1_TAC THENL
     [POP_ASSUM(SUBST1_TAC o SYM) THEN
      POP_ASSUM(SUBST1_TAC o SYM) THEN
      SUBGOAL_THEN `[EL (n + 12) (bs:byte list)] = SUB_LIST(n + 12, 1) bs`
      SUBST1_TAC THENL
       [ASM_SIMP_TAC[SUB_LIST_1;
          ARITH_RULE `LENGTH (bs:byte list) = n + 13 ==> n + 12 < LENGTH bs`];
        ALL_TAC] THEN
      MP_TAC(ISPECL [`bs:byte list`; `4:num`; `1:num`; `n + 8:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `4 + 1 = 5`;
                  ARITH_RULE `(n + 8) + 4 = n + 12`] THEN
      DISCH_THEN(SUBST1_TAC o SYM) THEN
      MP_TAC(ISPECL [`bs:byte list`; `8:num`; `5:num`; `n:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `8 + 5 = 13`] THEN
      DISCH_THEN(SUBST1_TAC o SYM) THEN REFL_TAC;
      ALL_TAC] THEN
    MP_TAC(ISPECL [`bs:byte list`; `n:num`] SUB_LIST_TOPSPLIT) THEN
    ASM_SIMP_TAC[ARITH_RULE
      `LENGTH (bs:byte list) = n + 13 ==> LENGTH bs - n = 13`]]);;

(* ------------------------------------------------------------------------- *)
(* RESIDUE13_MEMORY_CLOSE: per-buffer memory zero-projection for residue=13. *)
(* Composes the orthogonal-prefix lift `bytelist (a, k) s = ZEROES`           *)
(* with the post-STR int64 zero `bytes64 (a + k) s = word 0`                  *)
(* and the post-STR int32 zero `bytes32 (a + (k + 8)) s = word 0`             *)
(* and the post-STRB int8 zero `bytes8 (a + (k + 12)) s = word 0`             *)
(* into the full-buffer zero-fill `bytelist (a, k + 13) s = ZEROES`.          *)
(* ------------------------------------------------------------------------- *)

let RESIDUE13_MEMORY_CLOSE = prove
 (`!(a:int64) (s:armstate) k.
        read (memory :> bytelist (a, k)) s = REPLICATE k (word 0) /\
        read (memory :> bytes64 (word_add a (word k))) s = word 0 /\
        read (memory :> bytes32 (word_add a (word (k + 8)))) s = word 0 /\
        read (memory :> bytes8 (word_add a (word (k + 12)))) s = word 0
        ==> read (memory :> bytelist (a, k + 13)) s =
            REPLICATE (k + 13) (word 0)`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN `k + 13 = (k + 12) + 1` SUBST1_TAC THENL
   [ARITH_TAC; ALL_TAC] THEN
  MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
  ASM_REWRITE_TAC[] THEN
  MATCH_MP_TAC RESIDUE12_MEMORY_CLOSE THEN
...
(* ------------------------------------------------------------------------- *)
(* Residue=13 per-buffer cut-state adapters.                                  *)
(* The cut state on entry to the residue=13 case has a 13-byte suffix         *)
(* bytelist hypothesis                                                        *)
(*    read (memory :> bytelist (word_add a (word k), 13)) s =                 *)
(*      SUB_LIST(k, 13) bs                                                    *)
(* (with LENGTH bs = k + 13). The kernel's tail-block performs an LDR X       *)
(* (8 bytes at offset k), an LDR W (4 bytes at offset k+8), and an LDRB       *)
(* (1 byte at offset k+12); we stage the corresponding bytes64 / bytes32 /    *)
(* bytes8 reads at s0 so ARM_STEPS_TAC can resolve all three. They are        *)
(* pre-store reads at s0 — the post-imm STR XZR / STR WZR / STRB WZR writes   *)
(* are sequenced after the corresponding LDRs.                                *)
(* ------------------------------------------------------------------------- *)

let RESIDUE13_BYTELIST_TO_BYTES64 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 13 /\
        read (memory :> bytelist (word_add a (word k), 13)) s =
          SUB_LIST (k, 13) bs
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k, 13) (bs:byte list) =
       APPEND (SUB_LIST(k, 8) bs) (SUB_LIST(k + 8, 5) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `8:num`; `5:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `8 + 5 = 13`];
      ALL_TAC] THEN
    SUBGOAL_THEN
      `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
       LENGTH (SUB_LIST(k + 8, 5) (bs:byte list)) = 5`
    STRIP_ASSUME_TAC THENL
     [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word k), 13)) s =
                 SUB_LIST (k, 13) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `13 = LENGTH (APPEND (SUB_LIST(k, 8) (bs:byte list))
                           (SUB_LIST(k + 8, 5) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE13_BYTELIST_TO_BYTES32 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 13 /\
        read (memory :> bytelist (word_add a (word k), 13)) s =
          SUB_LIST (k, 13) bs
  ...
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
     LENGTH (SUB_LIST(k + 8, 4) (bs:byte list)) = 4 /\
     LENGTH (SUB_LIST(k + 12, 1) (bs:byte list)) = 1 /\
     LENGTH (SUB_LIST(k + 8, 5) (bs:byte list)) = 5`
  STRIP_ASSUME_TAC THENL
   [REWRITE_TAC[LENGTH_SUB_LIST] THEN ASM_ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 8)), 5)) (s:armstate) =
     SUB_LIST (k + 8, 5) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `read (memory :> bytelist (word_add a (word k), 13)) (s:armstate) =
       APPEND (SUB_LIST(k, 8) (bs:byte list)) (SUB_LIST(k + 8, 5) bs)`
    MP_TAC THENL
     [ASM_REWRITE_TAC[] THEN
      MP_TAC(ISPECL [`bs:byte list`; `8:num`; `5:num`; `k:num`]
                    SUB_LIST_SPLIT) THEN
  ...
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 8, 5) (bs:byte list) =
       APPEND (SUB_LIST(k + 8, 4) bs) (SUB_LIST(k + 12, 1) bs)`
    ASSUME_TAC THENL
     [MP_TAC(ISPECL [`bs:byte list`; `4:num`; `1:num`; `k + 8:num`]
                    SUB_LIST_SPLIT) THEN
      REWRITE_TAC[ARITH_RULE `4 + 1 = 5`;
                  ARITH_RULE `(k + 8) + 4 = k + 12`];
      ALL_TAC] THEN
    UNDISCH_TAC `read (memory :> bytelist (word_add a (word (k + 8)), 5)) s =
                 SUB_LIST (k + 8, 5) (bs:byte list)` THEN
    ASM_REWRITE_TAC[] THEN
    SUBGOAL_THEN
      `5 = LENGTH (APPEND (SUB_LIST(k + 8, 4) (bs:byte list))
                          (SUB_LIST(k + 12, 1) bs))`
    SUBST1_TAC THENL
     [REWRITE_TAC[LENGTH_APPEND] THEN ASM_REWRITE_TAC[] THEN ARITH_TAC;
      ALL_TAC] THEN
...
let RESIDUE13_BYTELIST_TO_BYTES8 = prove
 (`!(a:int64) (s:armstate) (bs:byte list) k.
        LENGTH bs = k + 13 /\
        read (memory :> bytelist (word_add a (word k), 13)) s =
          SUB_LIST (k, 13) bs
        ==> read (memory :> bytes8 (word_add a (word (k + 12)))) s =
            word(num_of_bytelist (SUB_LIST(k + 12, 1) bs))`,
  REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
    `LENGTH (SUB_LIST(k, 8) (bs:byte list)) = 8 /\
     LENGTH (SUB_LIST(k + 8, 4) (bs:byte list)) = 4 /\
  ...
  SUBGOAL_THEN
    `read (memory :> bytelist
       (word_add a (word (k + 12)), 1)) (s:armstate) =
     SUB_LIST (k + 12, 1) (bs:byte list)`
  ASSUME_TAC THENL
   [SUBGOAL_THEN
      `SUB_LIST(k + 8, 5) (bs:byte list) =
       APPEND (SUB_LIST(k + 8, 4) bs) (SUB_LIST(k + 12, 1) bs)`
    ...
    ALL_TAC] THEN
  MP_TAC(ISPECL [`word_add a (word (k + 12)):int64`; `s:armstate`;
                 `SUB_LIST(k + 12, 1) (bs:byte list)`; `0:num`]
                BYTES8_FROM_BYTELIST) THEN
  ASM_REWRITE_TAC[ARITH; LE_REFL] THEN
  REWRITE_TAC[WORD_ADD_0] THEN
  SUBGOAL_THEN
    `SUB_LIST(0, 1) (SUB_LIST(k + 12, 1) (bs:byte list)) =
     SUB_LIST(k + 12, 1) bs`
  SUBST1_TAC THENL
   [FIRST_ASSUM(fun th ->
      GEN_REWRITE_TAC (LAND_CONV o LAND_CONV o RAND_CONV) [SYM th]) THEN
