...
(*                                                                           *)
(* ------------------------------------------------------------------------- *)
(* Phase 8 BLOCKER (DISCOVERED Session 040):                                 *)
(*                                                                           *)
(* `NIST_FULL_KERNEL_CORRECT`'s post-condition does NOT assert Q4..Q7 at    *)
(* loop-body exit.  Specifically, post is only:                              *)
(*                                                                           *)
(*     read PC s = word (pc + 0x5c4) /\                                      *)
(*     read X2 s = word_add cptr (word 64) /\                                *)
(*     read Q11 s = nist_ghash h prev_tag [ct0; ct1; ct2; ct3]               *)
(*                                                                           *)
(* Q4..Q7 are in MAYCHANGE (free).  But the body PRECONDITION requires       *)
(* Q4..Q7 to be related to ct0..ct3 via                                      *)
(* `aes_gcm_rev64_int128 q5_pre = byteswap128 ct1` etc.  So an iteration's  *)
(* exit Q4..Q7 must be the THIS iteration's ciphertexts (ct0..ct3 from      *)
(* the body cut's perspective) for the next iteration to consume them.      *)
(*                                                                           *)
(* By inspection of the kernel disasm at offsets 0x554..0x5c0:               *)
(*   - 0x554: eor v4.16b, v4.16b, v0.16b   (Q4 = ptxt_lo XOR aes_round V0)  *)
(*   - 0x568: eor v5.16b, v5.16b, v1.16b   (Q5 = ptxt_lo XOR aes_round V1)  *)
(*   - 0x5a0: eor v6.16b, v6.16b, v2.16b                                     *)
(*   - 0x5bc: eor v7.16b, v7.16b, v3.16b                                     *)
(*   - 0x580/0x594/0x5a8/0x5c0: st1 v4..v7, [x2], #16                       *)
(*                                                                           *)
(* So at exit, Q4..Q7 ARE the new ciphertexts (ct'_0..ct'_3 in the spec).   *)
(* But the body cut's POST does not say so — and extending it requires      *)
(* either:                                                                   *)
(*   (a) re-running symbolic execution with stronger output post (revising  *)
(*       NIST_FULL_KERNEL_CORRECT and its DISCHARGE/NIST_SPEC_FORM ancestors *)
(*       — substantial proof rework),                                        *)
(*   (b) wrapping NIST_FULL_KERNEL_CORRECT in a tighter ensures cut that    *)
(*       composes a separate "Q4..Q7 = ciphertexts at exit" cut over the   *)
(*       last 8-12 instructions of the body.                                 *)
(*                                                                           *)
(* Recommended approach: option (b).  Author a small Phase 8 prep cut       *)
(* `BODY_Q4Q7_EXIT_CORRECT` over offsets 0x554..0x5c4 (last ~28              *)
(* instructions) that establishes Q4..Q7 = ciphertext_blocks given Q0..Q3   *)
(* AES-final outputs and X19..X24/X6/X7 plaintext-XOR'd values.  Then        *)
(* compose with NIST_FULL_KERNEL_CORRECT via ARM_BIGSTEP-style cut chaining *)
(* OR extend the post directly.                                              *)
(*                                                                           *)
(* SIMPLER ALTERNATIVE: add Q4..Q7 as ABBREV'd post conjuncts in a wrapped  *)
(* ensures cut, not as part of the spec-level postcondition.  At loop-body  *)
(* exit, Q4..Q7 take SOME concrete simulator-emit form (function of Q0..Q3 *)
(* and plaintext-loaded X regs).  The loop invariant carries them in this  *)
(* form, abbreviated, and the next iter's NIST_FULL_KERNEL_CORRECT pre is   *)
(* satisfied because we choose the abbreviated forms to match.              *)
(*                                                                           *)
(* For Phase 9 (tail), Q4..Q7 also matter, so this work MUST land before   *)
(* Phase 8 closes.                                                           *)
(* ------------------------------------------------------------------------- *)
