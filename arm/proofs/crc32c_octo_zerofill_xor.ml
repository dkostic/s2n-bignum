 ...
 (`!a0 a1 a2 a3 a4 a5 a6 a7
  ...
  ASM_CASES_TAC `len:num < 16` THENL
   [(* Branch A: len < 16, b.lt taken -> tail entry pc+0xbc.                    *)
    (* Cut at pc+0xbc with the same parameterized spec form Branch B uses (so   *)
    (* the residue tail closure block can be shared verbatim). For Branch A:    *)
    (* len DIV 16 = 0 (so prefix-zero bytelists are vacuous), len MOD 16 = len  *)
    (* (suffix bytelists span the entire buffer), and X8..X15 init values       *)
    (* 0xFFFFFFFF reduce to word_zx (crc32c_bytes [] = 0xFFFFFFFF) via          *)
    (* crc32c_bytes_NIL.                                                        *)
    SUBGOAL_THEN `len DIV 16 = 0` ASSUME_TAC THENL
     [ASM_SIMP_TAC[DIV_LT]; ALL_TAC] THEN
    SUBGOAL_THEN `len MOD 16 = len` ASSUME_TAC THENL
    ...
    CONJ_TAC THENL
     [(* Sub1 (Branch A): pc+0x28 -> pc+0xbc, b.lt taken. Spec form is the      *)
      ...
      (* Sub2 (Branch A): pc+0xbc -> pc+0x268, copied verbatim from Branch B's  *)
      (* sub-subgoal 2 (file lines 1018-3347 prior to this edit). The block is  *)
      (* invariant under Branch A's iters=0, residue=len reduction because the  *)
      (* inner SUBGOAL_THEN at the top only asserts                               *)
      (*   residue < 16, 16*iters+residue=len, 16*iters<=len                      *)
      (* which all hold trivially for iters=0, residue=len.                       *)
