   ...
   [(* Sub-subgoal 1: pc+0xbc -> pc+0x24c, tail blocks (TBZ-gated).             *)
    ...
    ASM_CASES_TAC `len = 0` THENL
     [(* Case len = 0: all 4 TBZs taken, no bytes consumed in tail.              *)
      ...
      (* Cases len ∈ {1..15}. *)
      ASM_CASES_TAC `len = 1` THENL
       [(* Case len = 1: TBZs at bits 3,2,1 taken; bit 0 falls through to    *)
        (* the 1-byte block (24 instrs). 28 ARM steps total (3 TBZs + 24 +   *)
        (* final B). Closure mirrors Branch B residue=1 (lines 1258-1369)    *)
        (* with offset k = 0.                                                 *)
        UNDISCH_TAC `len = 1` THEN DISCH_THEN SUBST_ALL_TAC THEN
        SUBGOAL_THEN
          `LENGTH (bs0:byte list) = 0 + 1 /\
           LENGTH (bs1:byte list) = 0 + 1 /\
           LENGTH (bs2:byte list) = 0 + 1 /\
           LENGTH (bs3:byte list) = 0 + 1 /\
           LENGTH (bs4:byte list) = 0 + 1 /\
           LENGTH (bs5:byte list) = 0 + 1 /\
           LENGTH (bs6:byte list) = 0 + 1 /\
           LENGTH (bs7:byte list) = 0 + 1`
          STRIP_ASSUME_TAC THENL
         [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
        (* Pre-stage suffix-bytelist hypotheses with k = 0 form: rewrite      *)
        (* `bytelist (a, 1)` → `bytelist (word_add a (word 0), 1)` and        *)
        (* `bs_i` → `SUB_LIST(0, 1) bs_i` so SUFFIX_BYTELIST_TO_BYTES8        *)
        (* antecedents match.                                                  *)
        SUBGOAL_THEN
          `SUB_LIST(0,1) (bs0:byte list) = bs0 /\
           SUB_LIST(0,1) (bs1:byte list) = bs1 /\
           SUB_LIST(0,1) (bs2:byte list) = bs2 /\
           SUB_LIST(0,1) (bs3:byte list) = bs3 /\
           SUB_LIST(0,1) (bs4:byte list) = bs4 /\
           SUB_LIST(0,1) (bs5:byte list) = bs5 /\
           SUB_LIST(0,1) (bs6:byte list) = bs6 /\
           SUB_LIST(0,1) (bs7:byte list) = bs7`
          STRIP_ASSUME_TAC THENL
         [REPEAT CONJ_TAC THEN ASM_MESON_TAC[SUB_LIST_LENGTH]; ALL_TAC] THEN
        ENSURES_INIT_TAC "s0" THEN
        (* Pre-stage 8 bytes8 reads at s0 by applying                         *)
        (* SUFFIX_BYTELIST_TO_BYTES8 to each bytelist hypothesis with k = 0.  *)
        (* Use ANTS_TAC + ASM_ARITH_TAC to fully discharge the antecedent     *)
        (* `LENGTH bs = 0 + 1` rather than leaving it as a leftover           *)
        (* `1 = 0 + 1` conditional that ASM_REWRITE can't consume.            *)
        ...
        REPEAT CONJ_TAC THENL
         [(* X8: bridge `word 0xFFFFFFFF` literal to                         *)
          (* `crc32c_bytes (word 0xFFFFFFFF) (SUB_LIST(0, 0) bs0)` via       *)
          (* SUB_LIST/crc32c_bytes first clauses, then accept the strip.    *)
          MP_TAC(SPECL [`bs0:byte list`; `0:num`] RESIDUE1_X_UPDATE) THEN
          REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
          ASM_REWRITE_TAC[] THEN
          DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
          (* X9 *)
          MP_TAC(SPECL [`bs1:byte list`; `0:num`] RESIDUE1_X_UPDATE) THEN
          REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
          ASM_REWRITE_TAC[] THEN
          DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
          (* X10 *)
          MP_TAC(SPECL [`bs2:byte list`; `0:num`] RESIDUE1_X_UPDATE) THEN
          REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
          ASM_REWRITE_TAC[] THEN
          DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
          (* X11 *)
          MP_TAC(SPECL [`bs3:byte list`; `0:num`] RESIDUE1_X_UPDATE) THEN
          REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
          ASM_REWRITE_TAC[] THEN
          DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
          (* X12 *)
          MP_TAC(SPECL [`bs4:byte list`; `0:num`] RESIDUE1_X_UPDATE) THEN
          REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
          ASM_REWRITE_TAC[] THEN
          DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
          (* X13 *)
          MP_TAC(SPECL [`bs5:byte list`; `0:num`] RESIDUE1_X_UPDATE) THEN
          REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
          ASM_REWRITE_TAC[] THEN
          DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
          (* X14 *)
          MP_TAC(SPECL [`bs6:byte list`; `0:num`] RESIDUE1_X_UPDATE) THEN
          REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
          ASM_REWRITE_TAC[] THEN
          DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
          (* X15 *)
          MP_TAC(SPECL [`bs7:byte list`; `0:num`] RESIDUE1_X_UPDATE) THEN
          REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
          ASM_REWRITE_TAC[] THEN
          DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
          (* memory a0..a7: rewrite `bytelist (a, 1)` → `[bytes8 a]` via    *)
          (* MEMORY_BYTELIST_1_EQ_BYTES8, then unfold `REPLICATE 1` via      *)
          (* `ONE; REPLICATE` so the goal becomes `[bytes8 a s] = [word 0]` *)
          (* and ASM_REWRITE consumes it via the SUFFIX_BYTELIST hyp.        *)
           ...
           [(* X8 *)
            ...
            CONJ_TAC THENL
             [REWRITE_TAC[READ_COMPONENT_COMPOSE; bytelist_clauses; REPLICATE];
              REWRITE_TAC[WORD_ADD_0] THEN ASM_REWRITE_TAC[]]];
          (* Cases len ∈ {3, 4, 5..15}. *)
          ASM_CASES_TAC `len = 4` THENL
           [(* Case len = 4: TBZ at bit 3 taken; bit 2 falls through to    *)
            (* the 4-byte block (24 instrs); bits 1,0 taken. 28 ARM steps   *)
            (* total. Closure mirrors residue=2 with width-4 helpers        *)
            (* (BYTES32_FROM_BYTELIST/SUFFIX_BYTELIST_TO_BYTES32/           *)
            (* RESIDUE4_X_UPDATE/RESIDUE4_MEMORY_CLOSE) at offset k = 0.    *)
            UNDISCH_TAC `len = 4` THEN DISCH_THEN SUBST_ALL_TAC THEN
            SUBGOAL_THEN
              `LENGTH (bs0:byte list) = 0 + 4 /\
               LENGTH (bs1:byte list) = 0 + 4 /\
               LENGTH (bs2:byte list) = 0 + 4 /\
               LENGTH (bs3:byte list) = 0 + 4 /\
               LENGTH (bs4:byte list) = 0 + 4 /\
               LENGTH (bs5:byte list) = 0 + 4 /\
               LENGTH (bs6:byte list) = 0 + 4 /\
               LENGTH (bs7:byte list) = 0 + 4`
              STRIP_ASSUME_TAC THENL
             [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
            SUBGOAL_THEN
              `SUB_LIST(0,4) (bs0:byte list) = bs0 /\
               SUB_LIST(0,4) (bs1:byte list) = bs1 /\
               SUB_LIST(0,4) (bs2:byte list) = bs2 /\
               SUB_LIST(0,4) (bs3:byte list) = bs3 /\
               SUB_LIST(0,4) (bs4:byte list) = bs4 /\
               SUB_LIST(0,4) (bs5:byte list) = bs5 /\
               SUB_LIST(0,4) (bs6:byte list) = bs6 /\
               SUB_LIST(0,4) (bs7:byte list) = bs7`
              STRIP_ASSUME_TAC THENL
             [REPEAT CONJ_TAC THEN ASM_MESON_TAC[SUB_LIST_LENGTH]; ALL_TAC] THEN
            ENSURES_INIT_TAC "s0" THEN
            MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
            ...
            REPEAT CONJ_TAC THENL
             [(* X8 *)
              MP_TAC(SPECL [`bs0:byte list`; `0:num`] RESIDUE4_X_UPDATE) THEN
              REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
              ASM_REWRITE_TAC[] THEN
              DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
              (* X9 *)
              MP_TAC(SPECL [`bs1:byte list`; `0:num`] RESIDUE4_X_UPDATE) THEN
              REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
              ASM_REWRITE_TAC[] THEN
              DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
              (* X10 *)
              MP_TAC(SPECL [`bs2:byte list`; `0:num`] RESIDUE4_X_UPDATE) THEN
              REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
              ASM_REWRITE_TAC[] THEN
              DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
              (* X11 *)
              MP_TAC(SPECL [`bs3:byte list`; `0:num`] RESIDUE4_X_UPDATE) THEN
              REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
              ASM_REWRITE_TAC[] THEN
              DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
              (* X12 *)
              MP_TAC(SPECL [`bs4:byte list`; `0:num`] RESIDUE4_X_UPDATE) THEN
              REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
              ASM_REWRITE_TAC[] THEN
              DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
              (* X13 *)
              MP_TAC(SPECL [`bs5:byte list`; `0:num`] RESIDUE4_X_UPDATE) THEN
              REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
              ASM_REWRITE_TAC[] THEN
              DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
              (* X14 *)
              MP_TAC(SPECL [`bs6:byte list`; `0:num`] RESIDUE4_X_UPDATE) THEN
              REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
              ASM_REWRITE_TAC[] THEN
              DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
              (* X15 *)
              MP_TAC(SPECL [`bs7:byte list`; `0:num`] RESIDUE4_X_UPDATE) THEN
              REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
              ASM_REWRITE_TAC[] THEN
              DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
              (* memory a0..a7: rewrite `bytelist (a, 4)` as                *)
              (* `bytelist (a, 0+4)` then close via RESIDUE4_MEMORY_CLOSE.  *)
              SUBGOAL_THEN `(4:num) = 0 + 4` SUBST1_TAC THENL
               [ARITH_TAC; ALL_TAC] THEN
              ...
              CONJ_TAC THENL
               [REWRITE_TAC[READ_COMPONENT_COMPOSE; bytelist_clauses; REPLICATE];
                REWRITE_TAC[WORD_ADD_0] THEN ASM_REWRITE_TAC[]]];
            (* Cases len ∈ {3, 5, 6, 7, 8, 9..15}. *)
            ASM_CASES_TAC `len = 8` THENL
             [(* Case len = 8: TBZ at bit 3 NOT taken; falls through to the   *)
              (* 8-byte block (24 instrs); bits 2,1,0 taken. 28 ARM steps     *)
              (* total. Closure mirrors residue=4 with width-8 helpers        *)
              (* (BYTES64_FROM_BYTELIST/SUFFIX_BYTELIST_TO_BYTES64/           *)
              (* RESIDUE8_X_UPDATE/RESIDUE8_MEMORY_CLOSE) at offset k = 0.    *)
              UNDISCH_TAC `len = 8` THEN DISCH_THEN SUBST_ALL_TAC THEN
              SUBGOAL_THEN
                `LENGTH (bs0:byte list) = 0 + 8 /\
                 LENGTH (bs1:byte list) = 0 + 8 /\
                 LENGTH (bs2:byte list) = 0 + 8 /\
                 LENGTH (bs3:byte list) = 0 + 8 /\
                 LENGTH (bs4:byte list) = 0 + 8 /\
                 LENGTH (bs5:byte list) = 0 + 8 /\
                 LENGTH (bs6:byte list) = 0 + 8 /\
                 LENGTH (bs7:byte list) = 0 + 8`
                STRIP_ASSUME_TAC THENL
               [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
              SUBGOAL_THEN
                `SUB_LIST(0,8) (bs0:byte list) = bs0 /\
                 SUB_LIST(0,8) (bs1:byte list) = bs1 /\
                 SUB_LIST(0,8) (bs2:byte list) = bs2 /\
                 SUB_LIST(0,8) (bs3:byte list) = bs3 /\
                 SUB_LIST(0,8) (bs4:byte list) = bs4 /\
                 SUB_LIST(0,8) (bs5:byte list) = bs5 /\
                 SUB_LIST(0,8) (bs6:byte list) = bs6 /\
                 SUB_LIST(0,8) (bs7:byte list) = bs7`
                STRIP_ASSUME_TAC THENL
               [REPEAT CONJ_TAC THEN ASM_MESON_TAC[SUB_LIST_LENGTH]; ALL_TAC] THEN
              ENSURES_INIT_TAC "s0" THEN
              MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
              ...
              REPEAT CONJ_TAC THENL
               [(* X8 *)
                MP_TAC(SPECL [`bs0:byte list`; `0:num`] RESIDUE8_X_UPDATE) THEN
                REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
                ASM_REWRITE_TAC[] THEN
                DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
                (* X9 *)
                MP_TAC(SPECL [`bs1:byte list`; `0:num`] RESIDUE8_X_UPDATE) THEN
                REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
                ASM_REWRITE_TAC[] THEN
                DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
                (* X10 *)
                MP_TAC(SPECL [`bs2:byte list`; `0:num`] RESIDUE8_X_UPDATE) THEN
                REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
                ASM_REWRITE_TAC[] THEN
                DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
                (* X11 *)
                MP_TAC(SPECL [`bs3:byte list`; `0:num`] RESIDUE8_X_UPDATE) THEN
                REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
                ASM_REWRITE_TAC[] THEN
                DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
                (* X12 *)
                MP_TAC(SPECL [`bs4:byte list`; `0:num`] RESIDUE8_X_UPDATE) THEN
                REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
                ASM_REWRITE_TAC[] THEN
                DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
                (* X13 *)
                MP_TAC(SPECL [`bs5:byte list`; `0:num`] RESIDUE8_X_UPDATE) THEN
                REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
                ASM_REWRITE_TAC[] THEN
                DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
                (* X14 *)
                MP_TAC(SPECL [`bs6:byte list`; `0:num`] RESIDUE8_X_UPDATE) THEN
                REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
                ASM_REWRITE_TAC[] THEN
                DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
                (* X15 *)
                MP_TAC(SPECL [`bs7:byte list`; `0:num`] RESIDUE8_X_UPDATE) THEN
                REWRITE_TAC[SUB_LIST; crc32c_bytes; ADD_CLAUSES] THEN
                ASM_REWRITE_TAC[] THEN
                DISCH_THEN(fun th -> AP_TERM_TAC THEN ACCEPT_TAC th);
                (* memory a0..a7: rewrite `bytelist (a, 8)` as                 *)
                (* `bytelist (a, 0+8)` then close via RESIDUE8_MEMORY_CLOSE.   *)
                SUBGOAL_THEN `(8:num) = 0 + 8` SUBST1_TAC THENL
                 [ARITH_TAC; ALL_TAC] THEN
                ...
                CONJ_TAC THENL
                 [REWRITE_TAC[READ_COMPONENT_COMPOSE; bytelist_clauses; REPLICATE];
                  REWRITE_TAC[WORD_ADD_0] THEN ASM_REWRITE_TAC[]]];
              (* Cases len ∈ {3, 5, 6, 7, 9..15}. *)
              ASM_CASES_TAC `len = 3` THENL
               [(* Case len = 3: TBZs at bits 3,2 taken; bit 1 falls through to *)
                (* the 2-byte block (24 instrs); bit 0 falls through to the    *)
                (* 1-byte block (24 instrs). 52 ARM steps total. Closure       *)
                (* CHAINS the singletons RESIDUE2 + RESIDUE1: per-buffer X-    *)
                (* update applies RESIDUE1_X_UPDATE with n=2 to consume the    *)
                (* outer CRC32CB step, then RESIDUE2_X_UPDATE with bs :=        *)
                (* SUB_LIST(0,2) bs and n=0 (using SUB_LIST_IDEMPOTENT) to     *)
                (* collapse the inner CRC32CH chain. Memory-close composes     *)
                (* RESIDUE1_MEMORY_CLOSE (closes [0,3) from [0,2) + bytes8      *)
                (* (a + 2)) atop RESIDUE2_MEMORY_CLOSE (closes [0,2) from [0,0)*)
                (* + bytes16 a). Pre-staging uses BYTES16_FROM_BYTELIST (n=0)   *)
                (* and BYTES8_FROM_BYTELIST (n=2) since the full bs has        *)
                (* length 3 — SUFFIX_BYTELIST_TO_BYTES{16,8} antecedents       *)
                (* don't apply at the full-bs level (LENGTH bs = k + N).        *)
                UNDISCH_TAC `len = 3` THEN DISCH_THEN SUBST_ALL_TAC THEN
                SUBGOAL_THEN
                  `LENGTH (bs0:byte list) = 3 /\
                   LENGTH (bs1:byte list) = 3 /\
                   LENGTH (bs2:byte list) = 3 /\
                   LENGTH (bs3:byte list) = 3 /\
                   LENGTH (bs4:byte list) = 3 /\
                   LENGTH (bs5:byte list) = 3 /\
                   LENGTH (bs6:byte list) = 3 /\
                   LENGTH (bs7:byte list) = 3`
                  STRIP_ASSUME_TAC THENL
                 [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                ENSURES_INIT_TAC "s0" THEN
                (* Pre-stage 8 bytes16 and 8 bytes8 reads at s0 from the       *)
                (* full-bytelist precondition via BYTES{16,8}_FROM_BYTELIST.    *)
                MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                               `bs0:byte list`; `0:num`]
                ...
                SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                         ...
                         ARITH_RULE `8 <= 64`] THEN
                REPEAT CONJ_TAC THENL
                 [(* X8: chain RESIDUE1_X_UPDATE (n=2) for outer CRC32CB,     *)
                  (*    then RESIDUE2_X_UPDATE (bs := SUB_LIST(0,2) bs0, n=0)*)
                  (*    via SUB_LIST_IDEMPOTENT to collapse inner CRC32CH.   *)
                  AP_TERM_TAC THEN
                  MP_TAC(SPECL [`bs0:byte list`; `2:num`] RESIDUE1_X_UPDATE) THEN
                  REWRITE_TAC[ARITH_RULE `2 + 1 = 3`] THEN
                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                  AP_THM_TAC THEN AP_TERM_TAC THEN
                  MP_TAC(SPECL [`SUB_LIST(0,2) (bs0:byte list)`; `0:num`]
                               RESIDUE2_X_UPDATE) THEN
                  REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                              SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                  ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                  DISCH_THEN ACCEPT_TAC;
                  (* X9 *)
                  AP_TERM_TAC THEN
                  MP_TAC(SPECL [`bs1:byte list`; `2:num`] RESIDUE1_X_UPDATE) THEN
                  REWRITE_TAC[ARITH_RULE `2 + 1 = 3`] THEN
                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                  AP_THM_TAC THEN AP_TERM_TAC THEN
                  MP_TAC(SPECL [`SUB_LIST(0,2) (bs1:byte list)`; `0:num`]
                               RESIDUE2_X_UPDATE) THEN
                  REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                              SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                  ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                  ...
                  (* X10 *)
                  AP_TERM_TAC THEN
                  MP_TAC(SPECL [`bs2:byte list`; `2:num`] RESIDUE1_X_UPDATE) THEN
                  REWRITE_TAC[ARITH_RULE `2 + 1 = 3`] THEN
                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                  AP_THM_TAC THEN AP_TERM_TAC THEN
                  MP_TAC(SPECL [`SUB_LIST(0,2) (bs2:byte list)`; `0:num`]
                               RESIDUE2_X_UPDATE) THEN
                  REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                              SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                  ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                  ...
                  (* X11 *)
                  AP_TERM_TAC THEN
                  MP_TAC(SPECL [`bs3:byte list`; `2:num`] RESIDUE1_X_UPDATE) THEN
                  REWRITE_TAC[ARITH_RULE `2 + 1 = 3`] THEN
                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                  AP_THM_TAC THEN AP_TERM_TAC THEN
                  MP_TAC(SPECL [`SUB_LIST(0,2) (bs3:byte list)`; `0:num`]
                               RESIDUE2_X_UPDATE) THEN
                  REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                              SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                  ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                  ...
                  (* X12 *)
                  AP_TERM_TAC THEN
                  MP_TAC(SPECL [`bs4:byte list`; `2:num`] RESIDUE1_X_UPDATE) THEN
                  REWRITE_TAC[ARITH_RULE `2 + 1 = 3`] THEN
                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                  AP_THM_TAC THEN AP_TERM_TAC THEN
                  MP_TAC(SPECL [`SUB_LIST(0,2) (bs4:byte list)`; `0:num`]
                               RESIDUE2_X_UPDATE) THEN
                  REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                              SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                  ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                  ...
                  (* X13 *)
                  AP_TERM_TAC THEN
                  MP_TAC(SPECL [`bs5:byte list`; `2:num`] RESIDUE1_X_UPDATE) THEN
                  REWRITE_TAC[ARITH_RULE `2 + 1 = 3`] THEN
                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                  AP_THM_TAC THEN AP_TERM_TAC THEN
                  MP_TAC(SPECL [`SUB_LIST(0,2) (bs5:byte list)`; `0:num`]
                               RESIDUE2_X_UPDATE) THEN
                  REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                              SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                  ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                  ...
                  (* X14 *)
                  AP_TERM_TAC THEN
                  MP_TAC(SPECL [`bs6:byte list`; `2:num`] RESIDUE1_X_UPDATE) THEN
                  REWRITE_TAC[ARITH_RULE `2 + 1 = 3`] THEN
                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                  AP_THM_TAC THEN AP_TERM_TAC THEN
                  MP_TAC(SPECL [`SUB_LIST(0,2) (bs6:byte list)`; `0:num`]
                               RESIDUE2_X_UPDATE) THEN
                  REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                              SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                  ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                  ...
                  (* X15 *)
                  AP_TERM_TAC THEN
                  MP_TAC(SPECL [`bs7:byte list`; `2:num`] RESIDUE1_X_UPDATE) THEN
                  REWRITE_TAC[ARITH_RULE `2 + 1 = 3`] THEN
                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                  AP_THM_TAC THEN AP_TERM_TAC THEN
                  MP_TAC(SPECL [`SUB_LIST(0,2) (bs7:byte list)`; `0:num`]
                               RESIDUE2_X_UPDATE) THEN
                  REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                              SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                  ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                  DISCH_THEN ACCEPT_TAC;
                  (* memory a0..a7: rewrite `bytelist (a, 3)` as              *)
                  (* `bytelist (a, (0+2)+1)` then chain RESIDUE1_MEMORY_CLOSE  *)
                  (* atop RESIDUE2_MEMORY_CLOSE.                               *)
                  SUBGOAL_THEN `(3:num) = (0 + 2) + 1` SUBST1_TAC THENL
                   [ARITH_TAC; ALL_TAC] THEN
                  MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
                  CONJ_TAC THENL
                   [MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
                    CONJ_TAC THENL
                     [REWRITE_TAC[READ_COMPONENT_COMPOSE; bytelist_clauses;
                                  REPLICATE];
                      REWRITE_TAC[WORD_ADD_0] THEN ASM_REWRITE_TAC[]];
                   ...
                   [MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
                    CONJ_TAC THENL
                     [REWRITE_TAC[READ_COMPONENT_COMPOSE; bytelist_clauses;
                      ...
                      REWRITE_TAC[WORD_ADD_0] THEN ASM_REWRITE_TAC[]];
                    REWRITE_TAC[ADD_CLAUSES] THEN ASM_REWRITE_TAC[]]];
                (* Cases len ∈ {5, 6, 7, 9..15}. *)
                ASM_CASES_TAC `len = 5` THENL
                 [(* Case len = 5: TBZ at bit 3 taken; bit 2 falls through to    *)
                  (* the 4-byte block (24 instrs); bit 1 taken; bit 0 falls       *)
                  (* through to the 1-byte block (24 instrs). 52 ARM steps total. *)
                  (* Closure CHAINS the singletons RESIDUE4 + RESIDUE1: per-     *)
                  (* buffer X-update applies RESIDUE1_X_UPDATE with n=4 to       *)
                  (* consume the outer CRC32CB step, then RESIDUE4_X_UPDATE with *)
                  (* bs := SUB_LIST(0,4) bs and n=0 (using SUB_LIST_IDEMPOTENT)  *)
                  (* to collapse the inner CRC32CW chain. Memory-close composes  *)
                  (* RESIDUE1_MEMORY_CLOSE (closes [0,5) from [0,4) + bytes8     *)
                  (* (a + 4)) atop RESIDUE4_MEMORY_CLOSE (closes [0,4) from      *)
                  (* [0,0) + bytes32 a). Pre-staging uses BYTES32_FROM_BYTELIST  *)
                  (* (n=0) and BYTES8_FROM_BYTELIST (n=4).                       *)
                  UNDISCH_TAC `len = 5` THEN DISCH_THEN SUBST_ALL_TAC THEN
                  SUBGOAL_THEN
                    `LENGTH (bs0:byte list) = 5 /\
                     LENGTH (bs1:byte list) = 5 /\
                     LENGTH (bs2:byte list) = 5 /\
                     LENGTH (bs3:byte list) = 5 /\
                     LENGTH (bs4:byte list) = 5 /\
                     LENGTH (bs5:byte list) = 5 /\
                     LENGTH (bs6:byte list) = 5 /\
                     LENGTH (bs7:byte list) = 5`
                    STRIP_ASSUME_TAC THENL
                   [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                  ENSURES_INIT_TAC "s0" THEN
                  (* Pre-stage 8 bytes32 and 8 bytes8 reads at s0 from the       *)
                  (* full-bytelist precondition via BYTES{32,8}_FROM_BYTELIST.   *)
                  MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                 `bs0:byte list`; `0:num`]
                  ...
                  ARM_STEPS_TAC CRC32C_OCTO_ZERO_FILL_XOR_EXEC (1--52) THEN
                  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
                  SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8;
                           DIMINDEX_32; DIMINDEX_64;
                           ARITH_RULE `8 <= 32`;
                           ARITH_RULE `32 <= 64`;
                           ARITH_RULE `8 <= 64`] THEN
                  REPEAT CONJ_TAC THENL
                   [(* X8: chain RESIDUE1_X_UPDATE (n=4) for outer CRC32CB,    *)
                    (*    then RESIDUE4_X_UPDATE (bs := SUB_LIST(0,4) bs0,    *)
                    (*    n=0) via SUB_LIST_IDEMPOTENT to collapse inner       *)
                    (*    CRC32CW chain.                                       *)
                    AP_TERM_TAC THEN
                    MP_TAC(SPECL [`bs0:byte list`; `4:num`] RESIDUE1_X_UPDATE) THEN
                    REWRITE_TAC[ARITH_RULE `4 + 1 = 5`] THEN
                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                    AP_THM_TAC THEN AP_TERM_TAC THEN
                    MP_TAC(SPECL [`SUB_LIST(0,4) (bs0:byte list)`; `0:num`]
                                 RESIDUE4_X_UPDATE) THEN
                    REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                    ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                    DISCH_THEN ACCEPT_TAC;
                    (* X9 *)
                    AP_TERM_TAC THEN
                    MP_TAC(SPECL [`bs1:byte list`; `4:num`] RESIDUE1_X_UPDATE) THEN
                    REWRITE_TAC[ARITH_RULE `4 + 1 = 5`] THEN
                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                    AP_THM_TAC THEN AP_TERM_TAC THEN
                    MP_TAC(SPECL [`SUB_LIST(0,4) (bs1:byte list)`; `0:num`]
                                 RESIDUE4_X_UPDATE) THEN
                    REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                    ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                    ...
                    (* X10 *)
                    AP_TERM_TAC THEN
                    MP_TAC(SPECL [`bs2:byte list`; `4:num`] RESIDUE1_X_UPDATE) THEN
                    REWRITE_TAC[ARITH_RULE `4 + 1 = 5`] THEN
                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                    AP_THM_TAC THEN AP_TERM_TAC THEN
                    MP_TAC(SPECL [`SUB_LIST(0,4) (bs2:byte list)`; `0:num`]
                                 RESIDUE4_X_UPDATE) THEN
                    REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                    ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                    ...
                    (* X11 *)
                    AP_TERM_TAC THEN
                    MP_TAC(SPECL [`bs3:byte list`; `4:num`] RESIDUE1_X_UPDATE) THEN
                    REWRITE_TAC[ARITH_RULE `4 + 1 = 5`] THEN
                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                    AP_THM_TAC THEN AP_TERM_TAC THEN
                    MP_TAC(SPECL [`SUB_LIST(0,4) (bs3:byte list)`; `0:num`]
                                 RESIDUE4_X_UPDATE) THEN
                    REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                    ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                    ...
                    (* X12 *)
                    AP_TERM_TAC THEN
                    MP_TAC(SPECL [`bs4:byte list`; `4:num`] RESIDUE1_X_UPDATE) THEN
                    REWRITE_TAC[ARITH_RULE `4 + 1 = 5`] THEN
                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                    AP_THM_TAC THEN AP_TERM_TAC THEN
                    MP_TAC(SPECL [`SUB_LIST(0,4) (bs4:byte list)`; `0:num`]
                                 RESIDUE4_X_UPDATE) THEN
                    REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                    ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                    ...
                    (* X13 *)
                    AP_TERM_TAC THEN
                    MP_TAC(SPECL [`bs5:byte list`; `4:num`] RESIDUE1_X_UPDATE) THEN
                    REWRITE_TAC[ARITH_RULE `4 + 1 = 5`] THEN
                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                    AP_THM_TAC THEN AP_TERM_TAC THEN
                    MP_TAC(SPECL [`SUB_LIST(0,4) (bs5:byte list)`; `0:num`]
                                 RESIDUE4_X_UPDATE) THEN
                    REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                    ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                    ...
                    (* X14 *)
                    AP_TERM_TAC THEN
                    MP_TAC(SPECL [`bs6:byte list`; `4:num`] RESIDUE1_X_UPDATE) THEN
                    REWRITE_TAC[ARITH_RULE `4 + 1 = 5`] THEN
                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                    AP_THM_TAC THEN AP_TERM_TAC THEN
                    MP_TAC(SPECL [`SUB_LIST(0,4) (bs6:byte list)`; `0:num`]
                                 RESIDUE4_X_UPDATE) THEN
                    REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                    ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                    ...
                    (* X15 *)
                    AP_TERM_TAC THEN
                    MP_TAC(SPECL [`bs7:byte list`; `4:num`] RESIDUE1_X_UPDATE) THEN
                    REWRITE_TAC[ARITH_RULE `4 + 1 = 5`] THEN
                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                    AP_THM_TAC THEN AP_TERM_TAC THEN
                    MP_TAC(SPECL [`SUB_LIST(0,4) (bs7:byte list)`; `0:num`]
                                 RESIDUE4_X_UPDATE) THEN
                    REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                    ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                    DISCH_THEN ACCEPT_TAC;
                    (* memory a0..a7: rewrite `bytelist (a, 5)` as              *)
                    (* `bytelist (a, (0+4)+1)` then chain                       *)
                    (* RESIDUE1_MEMORY_CLOSE atop RESIDUE4_MEMORY_CLOSE.         *)
                    SUBGOAL_THEN `(5:num) = (0 + 4) + 1` SUBST1_TAC THENL
                     [ARITH_TAC; ALL_TAC] THEN
                    MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
                    CONJ_TAC THENL
                     [MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                     ...
                     [MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                      CONJ_TAC THENL
                       [REWRITE_TAC[READ_COMPONENT_COMPOSE; bytelist_clauses;
                        ...
                        REWRITE_TAC[WORD_ADD_0] THEN ASM_REWRITE_TAC[]];
                      REWRITE_TAC[ADD_CLAUSES] THEN ASM_REWRITE_TAC[]]];
                  ASM_CASES_TAC `len = 6` THENL
                   [(* Case len = 6: TBZ at bit 3 taken; bit 2 falls through  *)
                    (* to the 4-byte block (24 instrs); bit 1 falls through    *)
                    (* to the 2-byte block (24 instrs); bit 0 taken to EOR.    *)
                    (* 52 ARM steps total. Closure CHAINS the singletons       *)
                    (* RESIDUE4 + RESIDUE2: per-buffer X-update applies        *)
                    (* RESIDUE2_X_UPDATE with n=4 to consume the outer         *)
                    (* CRC32CH steps, then RESIDUE4_X_UPDATE with bs :=        *)
                    (* SUB_LIST(0,4) bs and n=0 (using SUB_LIST_IDEMPOTENT)    *)
                    (* to collapse the inner CRC32CW chain. Memory-close       *)
                    (* composes RESIDUE2_MEMORY_CLOSE (closes [0,6) from       *)
                    (* [0,4) + bytes16 (a + 4)) atop RESIDUE4_MEMORY_CLOSE     *)
                    (* (closes [0,4) from [0,0) + bytes32 a). Pre-staging     *)
                    (* uses BYTES32_FROM_BYTELIST (n=0) and                    *)
                    (* BYTES16_FROM_BYTELIST (n=4).                            *)
                    UNDISCH_TAC `len = 6` THEN DISCH_THEN SUBST_ALL_TAC THEN
                    SUBGOAL_THEN
                      `LENGTH (bs0:byte list) = 6 /\
                       LENGTH (bs1:byte list) = 6 /\
                       LENGTH (bs2:byte list) = 6 /\
                       LENGTH (bs3:byte list) = 6 /\
                       LENGTH (bs4:byte list) = 6 /\
                       LENGTH (bs5:byte list) = 6 /\
                       LENGTH (bs6:byte list) = 6 /\
                       LENGTH (bs7:byte list) = 6`
                      STRIP_ASSUME_TAC THENL
                     [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                    ENSURES_INIT_TAC "s0" THEN
                    (* Pre-stage 8 bytes32 and 8 bytes16 reads at s0 from the *)
                    (* full-bytelist precondition via BYTES{32,16}_FROM_BYTELIST.*)
                    MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                   `bs0:byte list`; `0:num`]
                                  BYTES32_FROM_BYTELIST) THEN
                    ANTS_TAC THENL
                    ...
                    SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                             ...
                             ARITH_RULE `8 <= 64`] THEN
                    REPEAT CONJ_TAC THENL
                     [(* X8: chain RESIDUE2_X_UPDATE (n=4) for outer CRC32CH, *)
                      (*    then RESIDUE4_X_UPDATE (bs := SUB_LIST(0,4) bs0,  *)
                      (*    n=0) via SUB_LIST_IDEMPOTENT to collapse inner    *)
                      (*    CRC32CW chain.                                    *)
                      AP_TERM_TAC THEN
                      MP_TAC(SPECL [`bs0:byte list`; `4:num`] RESIDUE2_X_UPDATE) THEN
                      REWRITE_TAC[ARITH_RULE `4 + 2 = 6`] THEN
                      ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                      DISCH_THEN(SUBST1_TAC o SYM) THEN
                      AP_THM_TAC THEN AP_TERM_TAC THEN
                      AP_THM_TAC THEN AP_TERM_TAC THEN
                      ...
                      (* X9 *)
                      AP_TERM_TAC THEN
                      MP_TAC(SPECL [`bs1:byte list`; `4:num`] RESIDUE2_X_UPDATE) THEN
                      REWRITE_TAC[ARITH_RULE `4 + 2 = 6`] THEN
                      ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                      DISCH_THEN(SUBST1_TAC o SYM) THEN
                      AP_THM_TAC THEN AP_TERM_TAC THEN
                      ...
                      (* X10 *)
                      AP_TERM_TAC THEN
                      MP_TAC(SPECL [`bs2:byte list`; `4:num`] RESIDUE2_X_UPDATE) THEN
                      REWRITE_TAC[ARITH_RULE `4 + 2 = 6`] THEN
                      ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                      DISCH_THEN(SUBST1_TAC o SYM) THEN
                      AP_THM_TAC THEN AP_TERM_TAC THEN
                      ...
                      (* X11 *)
                      AP_TERM_TAC THEN
                      MP_TAC(SPECL [`bs3:byte list`; `4:num`] RESIDUE2_X_UPDATE) THEN
                      REWRITE_TAC[ARITH_RULE `4 + 2 = 6`] THEN
                      ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                      DISCH_THEN(SUBST1_TAC o SYM) THEN
                      AP_THM_TAC THEN AP_TERM_TAC THEN
                      ...
                      (* X12 *)
                      AP_TERM_TAC THEN
                      MP_TAC(SPECL [`bs4:byte list`; `4:num`] RESIDUE2_X_UPDATE) THEN
                      REWRITE_TAC[ARITH_RULE `4 + 2 = 6`] THEN
                      ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                      DISCH_THEN(SUBST1_TAC o SYM) THEN
                      AP_THM_TAC THEN AP_TERM_TAC THEN
                      ...
                      (* X13 *)
                      AP_TERM_TAC THEN
                      MP_TAC(SPECL [`bs5:byte list`; `4:num`] RESIDUE2_X_UPDATE) THEN
                      REWRITE_TAC[ARITH_RULE `4 + 2 = 6`] THEN
                      ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                      DISCH_THEN(SUBST1_TAC o SYM) THEN
                      AP_THM_TAC THEN AP_TERM_TAC THEN
                      ...
                      (* X14 *)
                      AP_TERM_TAC THEN
                      MP_TAC(SPECL [`bs6:byte list`; `4:num`] RESIDUE2_X_UPDATE) THEN
                      REWRITE_TAC[ARITH_RULE `4 + 2 = 6`] THEN
                      ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                      DISCH_THEN(SUBST1_TAC o SYM) THEN
                      AP_THM_TAC THEN AP_TERM_TAC THEN
                      ...
                      (* X15 *)
                      AP_TERM_TAC THEN
                      MP_TAC(SPECL [`bs7:byte list`; `4:num`] RESIDUE2_X_UPDATE) THEN
                      REWRITE_TAC[ARITH_RULE `4 + 2 = 6`] THEN
                      ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                      DISCH_THEN(SUBST1_TAC o SYM) THEN
                      AP_THM_TAC THEN AP_TERM_TAC THEN
                      ...
                      ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                      DISCH_THEN ACCEPT_TAC;
                      (* memory a0..a7: rewrite `bytelist (a, 6)` as           *)
                      (* `bytelist (a, (0+4)+2)` then chain                    *)
                      (* RESIDUE2_MEMORY_CLOSE atop RESIDUE4_MEMORY_CLOSE.     *)
                      SUBGOAL_THEN `(6:num) = (0 + 4) + 2` SUBST1_TAC THENL
                       [ARITH_TAC; ALL_TAC] THEN
                      MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
                      CONJ_TAC THENL
                       [MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                       ...
                       [MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                        CONJ_TAC THENL
                         [REWRITE_TAC[READ_COMPONENT_COMPOSE; bytelist_clauses;
                          ...
                          REWRITE_TAC[WORD_ADD_0] THEN ASM_REWRITE_TAC[]];
                        REWRITE_TAC[ADD_CLAUSES] THEN ASM_REWRITE_TAC[]]];
                    ASM_CASES_TAC `len = 7` THENL
                     [(* Case len = 7: TBZ at bit 3 taken; bit 2 falls through *)
                      (* to the 4-byte block (24 instrs); bit 1 falls through *)
                      (* to the 2-byte block (24 instrs); bit 0 falls through *)
                      (* to the 1-byte block (24 instrs). 76 ARM steps total. *)
                      (* Closure CHAINS three singletons RESIDUE4 + RESIDUE2 + *)
                      (* RESIDUE1: per-buffer X-update applies                *)
                      (* RESIDUE1_X_UPDATE with n=6 to consume the outer       *)
                      (* CRC32CB step, then RESIDUE2_X_UPDATE with bs :=       *)
                      (* SUB_LIST(0,6) bs and n=4 to collapse the middle      *)
                      (* CRC32CH chain (using SUB_LIST_SUB_LIST to match the   *)
                      (* arm-side X-state's SUB_LIST(4,2) bs term), then       *)
                      (* RESIDUE4_X_UPDATE with bs := SUB_LIST(0,4) bs and    *)
                      (* n=0 to collapse the inner CRC32CW chain. Memory-     *)
                      (* close composes RESIDUE1_MEMORY_CLOSE atop             *)
                      (* RESIDUE2_MEMORY_CLOSE atop RESIDUE4_MEMORY_CLOSE.    *)
                      (* Pre-staging uses BYTES32_FROM_BYTELIST (n=0),         *)
                      (* BYTES16_FROM_BYTELIST (n=4) and                       *)
                      (* BYTES8_FROM_BYTELIST (n=6).                           *)
                      UNDISCH_TAC `len = 7` THEN DISCH_THEN SUBST_ALL_TAC THEN
                      SUBGOAL_THEN
                        `LENGTH (bs0:byte list) = 7 /\
                         LENGTH (bs1:byte list) = 7 /\
                         LENGTH (bs2:byte list) = 7 /\
                         LENGTH (bs3:byte list) = 7 /\
                         LENGTH (bs4:byte list) = 7 /\
                         LENGTH (bs5:byte list) = 7 /\
                         LENGTH (bs6:byte list) = 7 /\
                         LENGTH (bs7:byte list) = 7`
                        STRIP_ASSUME_TAC THENL
                       [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                      ENSURES_INIT_TAC "s0" THEN
                      (* Pre-stage 8 bytes32 (n=0), 8 bytes16 (n=4) and 8     *)
                      (* bytes8 (n=6) reads at s0 from the full-bytelist      *)
                      (* precondition via BYTES{32,16,8}_FROM_BYTELIST.        *)
                      MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                     `bs0:byte list`; `0:num`]
                                    BYTES32_FROM_BYTELIST) THEN
                      ANTS_TAC THENL
                      ...
                      SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                               ...
                               ARITH_RULE `8 <= 64`] THEN
                      REPEAT CONJ_TAC THENL
                       [(* X8: chain RESIDUE1_X_UPDATE (n=6) for outer       *)
                        (*   CRC32CB, then RESIDUE2_X_UPDATE                  *)
                        (*   (bs := SUB_LIST(0,6) bs0, n=4) for middle       *)
                        (*   CRC32CH chain via SUB_LIST_SUB_LIST, then       *)
                        (*   RESIDUE4_X_UPDATE                                *)
                        (*   (bs := SUB_LIST(0,4) bs0, n=0) for inner        *)
                        (*   CRC32CW chain via SUB_LIST_IDEMPOTENT.          *)
                        AP_TERM_TAC THEN
                        MP_TAC(SPECL [`bs0:byte list`; `6:num`]
                                     RESIDUE1_X_UPDATE) THEN
                        REWRITE_TAC[ARITH_RULE `6 + 1 = 7`] THEN
                        ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                        DISCH_THEN(SUBST1_TAC o SYM) THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        SUBGOAL_THEN
                          `SUB_LIST(0, 4) (SUB_LIST(0, 6) (bs0:byte list))
                             = SUB_LIST(0, 4) bs0 /\
                           SUB_LIST(4, 2) (SUB_LIST(0, 6) (bs0:byte list))
                             = SUB_LIST(4, 2) bs0`
                          STRIP_ASSUME_TAC THENL
                         [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                          ARITH_TAC; ALL_TAC] THEN
                        MP_TAC(SPECL [`SUB_LIST(0,6) (bs0:byte list)`;
                                      `4:num`]
                                     RESIDUE2_X_UPDATE) THEN
                        ASM_REWRITE_TAC[] THEN
                        ANTS_TAC THENL
                        ...
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        MP_TAC(SPECL [`SUB_LIST(0,4) (bs0:byte list)`;
                                      `0:num`]
                                     RESIDUE4_X_UPDATE) THEN
                        REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                    SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                        ...
                        (* X9 *)
                        AP_TERM_TAC THEN
                        MP_TAC(SPECL [`bs1:byte list`; `6:num`]
                                     RESIDUE1_X_UPDATE) THEN
                        REWRITE_TAC[ARITH_RULE `6 + 1 = 7`] THEN
                        ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                        DISCH_THEN(SUBST1_TAC o SYM) THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        SUBGOAL_THEN
                          `SUB_LIST(0, 4) (SUB_LIST(0, 6) (bs1:byte list))
                             = SUB_LIST(0, 4) bs1 /\
                           SUB_LIST(4, 2) (SUB_LIST(0, 6) (bs1:byte list))
                             = SUB_LIST(4, 2) bs1`
                          STRIP_ASSUME_TAC THENL
                         [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                          ARITH_TAC; ALL_TAC] THEN
                        MP_TAC(SPECL [`SUB_LIST(0,6) (bs1:byte list)`;
                                      `4:num`]
                                     RESIDUE2_X_UPDATE) THEN
                        ASM_REWRITE_TAC[] THEN
                        ANTS_TAC THENL
                        ...
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        MP_TAC(SPECL [`SUB_LIST(0,4) (bs1:byte list)`;
                                      `0:num`]
                                     RESIDUE4_X_UPDATE) THEN
                        REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                    SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                        ...
                        (* X10 *)
                        AP_TERM_TAC THEN
                        MP_TAC(SPECL [`bs2:byte list`; `6:num`]
                                     RESIDUE1_X_UPDATE) THEN
                        REWRITE_TAC[ARITH_RULE `6 + 1 = 7`] THEN
                        ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                        DISCH_THEN(SUBST1_TAC o SYM) THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        SUBGOAL_THEN
                          `SUB_LIST(0, 4) (SUB_LIST(0, 6) (bs2:byte list))
                             = SUB_LIST(0, 4) bs2 /\
                           SUB_LIST(4, 2) (SUB_LIST(0, 6) (bs2:byte list))
                             = SUB_LIST(4, 2) bs2`
                          STRIP_ASSUME_TAC THENL
                         [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                          ARITH_TAC; ALL_TAC] THEN
                        MP_TAC(SPECL [`SUB_LIST(0,6) (bs2:byte list)`;
                                      `4:num`]
                                     RESIDUE2_X_UPDATE) THEN
                        ASM_REWRITE_TAC[] THEN
                        ANTS_TAC THENL
                        ...
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        MP_TAC(SPECL [`SUB_LIST(0,4) (bs2:byte list)`;
                                      `0:num`]
                                     RESIDUE4_X_UPDATE) THEN
                        REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                    SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                        ...
                        (* X11 *)
                        AP_TERM_TAC THEN
                        MP_TAC(SPECL [`bs3:byte list`; `6:num`]
                                     RESIDUE1_X_UPDATE) THEN
                        REWRITE_TAC[ARITH_RULE `6 + 1 = 7`] THEN
                        ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                        DISCH_THEN(SUBST1_TAC o SYM) THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        SUBGOAL_THEN
                          `SUB_LIST(0, 4) (SUB_LIST(0, 6) (bs3:byte list))
                             = SUB_LIST(0, 4) bs3 /\
                           SUB_LIST(4, 2) (SUB_LIST(0, 6) (bs3:byte list))
                             = SUB_LIST(4, 2) bs3`
                          STRIP_ASSUME_TAC THENL
                         [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                          ARITH_TAC; ALL_TAC] THEN
                        MP_TAC(SPECL [`SUB_LIST(0,6) (bs3:byte list)`;
                                      `4:num`]
                                     RESIDUE2_X_UPDATE) THEN
                        ASM_REWRITE_TAC[] THEN
                        ANTS_TAC THENL
                        ...
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        MP_TAC(SPECL [`SUB_LIST(0,4) (bs3:byte list)`;
                                      `0:num`]
                                     RESIDUE4_X_UPDATE) THEN
                        REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                    SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                        ...
                        (* X12 *)
                        AP_TERM_TAC THEN
                        MP_TAC(SPECL [`bs4:byte list`; `6:num`]
                                     RESIDUE1_X_UPDATE) THEN
                        REWRITE_TAC[ARITH_RULE `6 + 1 = 7`] THEN
                        ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                        DISCH_THEN(SUBST1_TAC o SYM) THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        SUBGOAL_THEN
                          `SUB_LIST(0, 4) (SUB_LIST(0, 6) (bs4:byte list))
                             = SUB_LIST(0, 4) bs4 /\
                           SUB_LIST(4, 2) (SUB_LIST(0, 6) (bs4:byte list))
                             = SUB_LIST(4, 2) bs4`
                          STRIP_ASSUME_TAC THENL
                         [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                          ARITH_TAC; ALL_TAC] THEN
                        MP_TAC(SPECL [`SUB_LIST(0,6) (bs4:byte list)`;
                                      `4:num`]
                                     RESIDUE2_X_UPDATE) THEN
                        ASM_REWRITE_TAC[] THEN
                        ANTS_TAC THENL
                        ...
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        MP_TAC(SPECL [`SUB_LIST(0,4) (bs4:byte list)`;
                                      `0:num`]
                                     RESIDUE4_X_UPDATE) THEN
                        REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                    SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                        ...
                        (* X13 *)
                        AP_TERM_TAC THEN
                        MP_TAC(SPECL [`bs5:byte list`; `6:num`]
                                     RESIDUE1_X_UPDATE) THEN
                        REWRITE_TAC[ARITH_RULE `6 + 1 = 7`] THEN
                        ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                        DISCH_THEN(SUBST1_TAC o SYM) THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        SUBGOAL_THEN
                          `SUB_LIST(0, 4) (SUB_LIST(0, 6) (bs5:byte list))
                             = SUB_LIST(0, 4) bs5 /\
                           SUB_LIST(4, 2) (SUB_LIST(0, 6) (bs5:byte list))
                             = SUB_LIST(4, 2) bs5`
                          STRIP_ASSUME_TAC THENL
                         [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                          ARITH_TAC; ALL_TAC] THEN
                        MP_TAC(SPECL [`SUB_LIST(0,6) (bs5:byte list)`;
                                      `4:num`]
                                     RESIDUE2_X_UPDATE) THEN
                        ASM_REWRITE_TAC[] THEN
                        ANTS_TAC THENL
                        ...
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        MP_TAC(SPECL [`SUB_LIST(0,4) (bs5:byte list)`;
                                      `0:num`]
                                     RESIDUE4_X_UPDATE) THEN
                        REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                    SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                        ...
                        (* X14 *)
                        AP_TERM_TAC THEN
                        MP_TAC(SPECL [`bs6:byte list`; `6:num`]
                                     RESIDUE1_X_UPDATE) THEN
                        REWRITE_TAC[ARITH_RULE `6 + 1 = 7`] THEN
                        ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                        DISCH_THEN(SUBST1_TAC o SYM) THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        SUBGOAL_THEN
                          `SUB_LIST(0, 4) (SUB_LIST(0, 6) (bs6:byte list))
                             = SUB_LIST(0, 4) bs6 /\
                           SUB_LIST(4, 2) (SUB_LIST(0, 6) (bs6:byte list))
                             = SUB_LIST(4, 2) bs6`
                          STRIP_ASSUME_TAC THENL
                         [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                          ARITH_TAC; ALL_TAC] THEN
                        MP_TAC(SPECL [`SUB_LIST(0,6) (bs6:byte list)`;
                                      `4:num`]
                                     RESIDUE2_X_UPDATE) THEN
                        ASM_REWRITE_TAC[] THEN
                        ANTS_TAC THENL
                        ...
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        MP_TAC(SPECL [`SUB_LIST(0,4) (bs6:byte list)`;
                                      `0:num`]
                                     RESIDUE4_X_UPDATE) THEN
                        REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                    SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                        ...
                        (* X15 *)
                        AP_TERM_TAC THEN
                        MP_TAC(SPECL [`bs7:byte list`; `6:num`]
                                     RESIDUE1_X_UPDATE) THEN
                        REWRITE_TAC[ARITH_RULE `6 + 1 = 7`] THEN
                        ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                        DISCH_THEN(SUBST1_TAC o SYM) THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        SUBGOAL_THEN
                          `SUB_LIST(0, 4) (SUB_LIST(0, 6) (bs7:byte list))
                             = SUB_LIST(0, 4) bs7 /\
                           SUB_LIST(4, 2) (SUB_LIST(0, 6) (bs7:byte list))
                             = SUB_LIST(4, 2) bs7`
                          STRIP_ASSUME_TAC THENL
                         [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                          ARITH_TAC; ALL_TAC] THEN
                        MP_TAC(SPECL [`SUB_LIST(0,6) (bs7:byte list)`;
                                      `4:num`]
                                     RESIDUE2_X_UPDATE) THEN
                        ASM_REWRITE_TAC[] THEN
                        ANTS_TAC THENL
                        ...
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        AP_THM_TAC THEN AP_TERM_TAC THEN
                        MP_TAC(SPECL [`SUB_LIST(0,4) (bs7:byte list)`;
                                      `0:num`]
                                     RESIDUE4_X_UPDATE) THEN
                        REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                    SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                        ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                        DISCH_THEN ACCEPT_TAC;
                        (* memory a0..a7: rewrite `bytelist (a, 7)` as       *)
                        (* `bytelist (a, ((0+4)+2)+1)` then chain             *)
                        (* RESIDUE1_MEMORY_CLOSE atop RESIDUE2_MEMORY_CLOSE  *)
                        (* atop RESIDUE4_MEMORY_CLOSE.                        *)
                        SUBGOAL_THEN `(7:num) = ((0 + 4) + 2) + 1`
                           ...
                           [MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                            CONJ_TAC THENL
                             [REWRITE_TAC[READ_COMPONENT_COMPOSE;
                                          bytelist_clauses; REPLICATE];
                              REWRITE_TAC[WORD_ADD_0] THEN
                              ASM_REWRITE_TAC[]];
                            REWRITE_TAC[ADD_CLAUSES] THEN
                            ASM_REWRITE_TAC[]];
                          REWRITE_TAC[ADD_CLAUSES; ARITH_RULE `4 + 2 = 6`] THEN ASM_REWRITE_TAC[]];
                        SUBGOAL_THEN `(7:num) = ((0 + 4) + 2) + 1`
                          SUBST1_TAC THENL [ARITH_TAC; ALL_TAC] THEN
                        MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
                        CONJ_TAC THENL
                         ...
                         [MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
                          CONJ_TAC THENL
                           [MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                            ...
                            ASM_REWRITE_TAC[]];
                          REWRITE_TAC[ADD_CLAUSES; ARITH_RULE `4 + 2 = 6`] THEN ASM_REWRITE_TAC[]]];
                      ASM_CASES_TAC `len = 9` THENL
                       [(* Case len = 9: TBZ at bit 3 NOT taken; falls through *)
                        (* to the 8-byte block (24 instrs); bits 2,1 taken;    *)
                        (* bit 0 falls through to the 1-byte block (24 instrs).*)
                        (* 52 ARM steps total. Closure CHAINS the singletons   *)
                        (* RESIDUE8 + RESIDUE1: per-buffer X-update applies    *)
                        (* RESIDUE1_X_UPDATE with n=8 to consume the outer     *)
                        (* CRC32CB step, then RESIDUE8_X_UPDATE with bs :=     *)
                        (* SUB_LIST(0,8) bs and n=0 (using SUB_LIST_IDEMPOTENT)*)
                        (* to collapse the inner CRC32CX chain. Memory-close   *)
                        (* composes RESIDUE1_MEMORY_CLOSE (closes [0,9) from   *)
                        (* [0,8) + bytes8 (a + 8)) atop RESIDUE8_MEMORY_CLOSE  *)
                        (* (closes [0,8) from [0,0) + bytes64 a). Pre-staging  *)
                        (* uses BYTES64_FROM_BYTELIST (n=0) and                *)
                        (* BYTES8_FROM_BYTELIST (n=8).                         *)
                        UNDISCH_TAC `len = 9` THEN DISCH_THEN SUBST_ALL_TAC THEN
                        SUBGOAL_THEN
                          `LENGTH (bs0:byte list) = 9 /\
                           LENGTH (bs1:byte list) = 9 /\
                           LENGTH (bs2:byte list) = 9 /\
                           LENGTH (bs3:byte list) = 9 /\
                           LENGTH (bs4:byte list) = 9 /\
                           LENGTH (bs5:byte list) = 9 /\
                           LENGTH (bs6:byte list) = 9 /\
                           LENGTH (bs7:byte list) = 9`
                          STRIP_ASSUME_TAC THENL
                         [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                        ENSURES_INIT_TAC "s0" THEN
                        (* Pre-stage 8 bytes64 (n=0) and 8 bytes8 (n=8) reads  *)
                        (* at s0 from the full-bytelist precondition via       *)
                        (* BYTES{64,8}_FROM_BYTELIST.                          *)
                        MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                       `bs0:byte list`; `0:num`]
                        ...
                        SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8;
                                 ...
                                 ARITH_RULE `8 <= 64`] THEN
                        REPEAT CONJ_TAC THENL
                         [(* X8: chain RESIDUE1_X_UPDATE (n=8) for outer       *)
                          (*    CRC32CB, then RESIDUE8_X_UPDATE (bs :=         *)
                          (*    SUB_LIST(0,8) bs0, n=0) via SUB_LIST_IDEMPOTENT*)
                          (*    to collapse inner CRC32CX chain.               *)
                          AP_TERM_TAC THEN
                          MP_TAC(SPECL [`bs0:byte list`; `8:num`] RESIDUE1_X_UPDATE) THEN
                          REWRITE_TAC[ARITH_RULE `8 + 1 = 9`] THEN
                          ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                          DISCH_THEN(SUBST1_TAC o SYM) THEN
                          AP_THM_TAC THEN AP_TERM_TAC THEN
                          MP_TAC(SPECL [`SUB_LIST(0,8) (bs0:byte list)`; `0:num`]
                                       RESIDUE8_X_UPDATE) THEN
                          REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                      SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                          ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                          DISCH_THEN ACCEPT_TAC;
                          (* X9 *)
                          AP_TERM_TAC THEN
                          MP_TAC(SPECL [`bs1:byte list`; `8:num`] RESIDUE1_X_UPDATE) THEN
                          REWRITE_TAC[ARITH_RULE `8 + 1 = 9`] THEN
                          ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                          DISCH_THEN(SUBST1_TAC o SYM) THEN
                          AP_THM_TAC THEN AP_TERM_TAC THEN
                          MP_TAC(SPECL [`SUB_LIST(0,8) (bs1:byte list)`; `0:num`]
                                       RESIDUE8_X_UPDATE) THEN
                          REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                      SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                          ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                          ...
                          (* X10 *)
                          AP_TERM_TAC THEN
                          MP_TAC(SPECL [`bs2:byte list`; `8:num`] RESIDUE1_X_UPDATE) THEN
                          REWRITE_TAC[ARITH_RULE `8 + 1 = 9`] THEN
                          ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                          DISCH_THEN(SUBST1_TAC o SYM) THEN
                          AP_THM_TAC THEN AP_TERM_TAC THEN
                          MP_TAC(SPECL [`SUB_LIST(0,8) (bs2:byte list)`; `0:num`]
                                       RESIDUE8_X_UPDATE) THEN
                          REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                      SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                          ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                          ...
                          (* X11 *)
                          AP_TERM_TAC THEN
                          MP_TAC(SPECL [`bs3:byte list`; `8:num`] RESIDUE1_X_UPDATE) THEN
                          REWRITE_TAC[ARITH_RULE `8 + 1 = 9`] THEN
                          ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                          DISCH_THEN(SUBST1_TAC o SYM) THEN
                          AP_THM_TAC THEN AP_TERM_TAC THEN
                          MP_TAC(SPECL [`SUB_LIST(0,8) (bs3:byte list)`; `0:num`]
                                       RESIDUE8_X_UPDATE) THEN
                          REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                      SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                          ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                          ...
                          (* X12 *)
                          AP_TERM_TAC THEN
                          MP_TAC(SPECL [`bs4:byte list`; `8:num`] RESIDUE1_X_UPDATE) THEN
                          REWRITE_TAC[ARITH_RULE `8 + 1 = 9`] THEN
                          ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                          DISCH_THEN(SUBST1_TAC o SYM) THEN
                          AP_THM_TAC THEN AP_TERM_TAC THEN
                          MP_TAC(SPECL [`SUB_LIST(0,8) (bs4:byte list)`; `0:num`]
                                       RESIDUE8_X_UPDATE) THEN
                          REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                      SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                          ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                          ...
                          (* X13 *)
                          AP_TERM_TAC THEN
                          MP_TAC(SPECL [`bs5:byte list`; `8:num`] RESIDUE1_X_UPDATE) THEN
                          REWRITE_TAC[ARITH_RULE `8 + 1 = 9`] THEN
                          ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                          DISCH_THEN(SUBST1_TAC o SYM) THEN
                          AP_THM_TAC THEN AP_TERM_TAC THEN
                          MP_TAC(SPECL [`SUB_LIST(0,8) (bs5:byte list)`; `0:num`]
                                       RESIDUE8_X_UPDATE) THEN
                          REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                      SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                          ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                          ...
                          (* X14 *)
                          AP_TERM_TAC THEN
                          MP_TAC(SPECL [`bs6:byte list`; `8:num`] RESIDUE1_X_UPDATE) THEN
                          REWRITE_TAC[ARITH_RULE `8 + 1 = 9`] THEN
                          ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                          DISCH_THEN(SUBST1_TAC o SYM) THEN
                          AP_THM_TAC THEN AP_TERM_TAC THEN
                          MP_TAC(SPECL [`SUB_LIST(0,8) (bs6:byte list)`; `0:num`]
                                       RESIDUE8_X_UPDATE) THEN
                          REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                      SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                          ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                          ...
                          (* X15 *)
                          AP_TERM_TAC THEN
                          MP_TAC(SPECL [`bs7:byte list`; `8:num`] RESIDUE1_X_UPDATE) THEN
                          REWRITE_TAC[ARITH_RULE `8 + 1 = 9`] THEN
                          ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                          DISCH_THEN(SUBST1_TAC o SYM) THEN
                          AP_THM_TAC THEN AP_TERM_TAC THEN
                          MP_TAC(SPECL [`SUB_LIST(0,8) (bs7:byte list)`; `0:num`]
                                       RESIDUE8_X_UPDATE) THEN
                          REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                      SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                          ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                          DISCH_THEN ACCEPT_TAC;
                          (* memory a0..a7: rewrite `bytelist (a, 9)` as       *)
                          (* `bytelist (a, (0+8)+1)` then chain                *)
                          (* RESIDUE1_MEMORY_CLOSE atop RESIDUE8_MEMORY_CLOSE. *)
                          SUBGOAL_THEN `(9:num) = (0 + 8) + 1` SUBST1_TAC THENL
                           [ARITH_TAC; ALL_TAC] THEN
                          MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
                          CONJ_TAC THENL
                           [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                           ...
                           [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                            CONJ_TAC THENL
                             [REWRITE_TAC[READ_COMPONENT_COMPOSE; bytelist_clauses;
                              ...
                              REWRITE_TAC[WORD_ADD_0] THEN ASM_REWRITE_TAC[]];
                            REWRITE_TAC[ADD_CLAUSES] THEN ASM_REWRITE_TAC[]]];
                        ASM_CASES_TAC `len = 10` THENL
                         [(* Case len = 10: TBZ at bit 3 NOT taken; falls through*)
                          (* to the 8-byte block (24 instrs); bit 2 taken;       *)
                          (* bit 1 NOT taken falls through to the 2-byte block   *)
                          (* (24 instrs); bit 0 taken to EOR. 52 ARM steps.      *)
                          (* Closure CHAINS singletons RESIDUE8 + RESIDUE2:      *)
                          (* per-buffer X-update applies RESIDUE2_X_UPDATE with  *)
                          (* n=8 to consume the outer CRC32CH halfword steps,    *)
                          (* then RESIDUE8_X_UPDATE with bs := SUB_LIST(0,8) bs  *)
                          (* and n=0 (using SUB_LIST_IDEMPOTENT) to collapse the *)
                          (* inner CRC32CX chain. Memory-close composes          *)
                          (* RESIDUE2_MEMORY_CLOSE (closes [0,10) from [0,8) +   *)
                          (* bytes16 (a + 8)) atop RESIDUE8_MEMORY_CLOSE (closes *)
                          (* [0,8) from [0,0) + bytes64 a). Pre-staging uses     *)
                          (* BYTES64_FROM_BYTELIST (n=0) and                     *)
                          (* BYTES16_FROM_BYTELIST (n=8).                        *)
                          UNDISCH_TAC `len = 10` THEN DISCH_THEN SUBST_ALL_TAC THEN
                          SUBGOAL_THEN
                            `LENGTH (bs0:byte list) = 10 /\
                             LENGTH (bs1:byte list) = 10 /\
                             LENGTH (bs2:byte list) = 10 /\
                             LENGTH (bs3:byte list) = 10 /\
                             LENGTH (bs4:byte list) = 10 /\
                             LENGTH (bs5:byte list) = 10 /\
                             LENGTH (bs6:byte list) = 10 /\
                             LENGTH (bs7:byte list) = 10`
                            STRIP_ASSUME_TAC THENL
                           [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                          ENSURES_INIT_TAC "s0" THEN
                          (* Pre-stage 8 bytes64 (n=0) and 8 bytes16 (n=8) reads *)
                          (* at s0 from the full-bytelist precondition via       *)
                          (* BYTES{64,16}_FROM_BYTELIST.                         *)
                          MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                         `bs0:byte list`; `0:num`]
                                        BYTES64_FROM_BYTELIST) THEN
                          ANTS_TAC THENL
                          ...
                          SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                   ...
                                   ARITH_RULE `8 <= 64`] THEN
                          REPEAT CONJ_TAC THENL
                           [(* X8: chain RESIDUE2_X_UPDATE (n=8) for outer       *)
                            (*    CRC32CH halfword, then RESIDUE8_X_UPDATE (bs   *)
                            (*    := SUB_LIST(0,8) bs0, n=0) via                 *)
                            (*    SUB_LIST_IDEMPOTENT to collapse inner CRC32CX. *)
                            AP_TERM_TAC THEN
                            MP_TAC(SPECL [`bs0:byte list`; `8:num`] RESIDUE2_X_UPDATE) THEN
                            ...
                            (* X9 *)
                            AP_TERM_TAC THEN
                            MP_TAC(SPECL [`bs1:byte list`; `8:num`] RESIDUE2_X_UPDATE) THEN
                            REWRITE_TAC[ARITH_RULE `8 + 2 = 10`] THEN
                            ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                            DISCH_THEN(SUBST1_TAC o SYM) THEN
                            AP_THM_TAC THEN AP_TERM_TAC THEN
                            ...
                            (* X10 *)
                            AP_TERM_TAC THEN
                            MP_TAC(SPECL [`bs2:byte list`; `8:num`] RESIDUE2_X_UPDATE) THEN
                            REWRITE_TAC[ARITH_RULE `8 + 2 = 10`] THEN
                            ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                            DISCH_THEN(SUBST1_TAC o SYM) THEN
                            AP_THM_TAC THEN AP_TERM_TAC THEN
                            ...
                            (* X11 *)
                            AP_TERM_TAC THEN
                            MP_TAC(SPECL [`bs3:byte list`; `8:num`] RESIDUE2_X_UPDATE) THEN
                            REWRITE_TAC[ARITH_RULE `8 + 2 = 10`] THEN
                            ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                            DISCH_THEN(SUBST1_TAC o SYM) THEN
                            AP_THM_TAC THEN AP_TERM_TAC THEN
                            ...
                            (* X12 *)
                            AP_TERM_TAC THEN
                            MP_TAC(SPECL [`bs4:byte list`; `8:num`] RESIDUE2_X_UPDATE) THEN
                            REWRITE_TAC[ARITH_RULE `8 + 2 = 10`] THEN
                            ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                            DISCH_THEN(SUBST1_TAC o SYM) THEN
                            AP_THM_TAC THEN AP_TERM_TAC THEN
                            ...
                            (* X13 *)
                            AP_TERM_TAC THEN
                            MP_TAC(SPECL [`bs5:byte list`; `8:num`] RESIDUE2_X_UPDATE) THEN
                            REWRITE_TAC[ARITH_RULE `8 + 2 = 10`] THEN
                            ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                            DISCH_THEN(SUBST1_TAC o SYM) THEN
                            AP_THM_TAC THEN AP_TERM_TAC THEN
                            ...
                            (* X14 *)
                            AP_TERM_TAC THEN
                            MP_TAC(SPECL [`bs6:byte list`; `8:num`] RESIDUE2_X_UPDATE) THEN
                            REWRITE_TAC[ARITH_RULE `8 + 2 = 10`] THEN
                            ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                            DISCH_THEN(SUBST1_TAC o SYM) THEN
                            AP_THM_TAC THEN AP_TERM_TAC THEN
                            ...
                            (* X15 *)
                            AP_TERM_TAC THEN
                            MP_TAC(SPECL [`bs7:byte list`; `8:num`] RESIDUE2_X_UPDATE) THEN
                            REWRITE_TAC[ARITH_RULE `8 + 2 = 10`] THEN
                            ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                            DISCH_THEN(SUBST1_TAC o SYM) THEN
                            AP_THM_TAC THEN AP_TERM_TAC THEN
                            ...
                            ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                            DISCH_THEN ACCEPT_TAC;
                            (* memory a0..a7: rewrite `bytelist (a, 10)` as       *)
                            (* `bytelist (a, (0+8)+2)` then chain                *)
                            (* RESIDUE2_MEMORY_CLOSE atop RESIDUE8_MEMORY_CLOSE. *)
                            SUBGOAL_THEN `(10:num) = (0 + 8) + 2` SUBST1_TAC THENL
                             [ARITH_TAC; ALL_TAC] THEN
                            MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
                            CONJ_TAC THENL
                             [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                             ...
                             [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                              CONJ_TAC THENL
                               [REWRITE_TAC[READ_COMPONENT_COMPOSE; bytelist_clauses;
                                ...
                                REWRITE_TAC[WORD_ADD_0] THEN ASM_REWRITE_TAC[]];
                              REWRITE_TAC[ADD_CLAUSES] THEN ASM_REWRITE_TAC[]]];
                          ASM_CASES_TAC `len = 11` THENL
                           [(* Case len = 11: TBZ at bit 3 NOT taken; falls   *)
                            (* through to the 8-byte block (24 instrs); bit 2 *)
                            (* taken; bit 1 NOT taken falls through to the    *)
                            (* 2-byte block (24 instrs); bit 0 NOT taken      *)
                            (* falls through to the 1-byte block (24 instrs). *)
                            (* 76 ARM steps total. Closure CHAINS three       *)
                            (* singletons RESIDUE8 + RESIDUE2 + RESIDUE1:     *)
                            (* per-buffer X-update applies RESIDUE1_X_UPDATE  *)
                            (* with n=10 to consume the outer CRC32CB step,  *)
                            (* then RESIDUE2_X_UPDATE with bs := SUB_LIST    *)
                            (* (0,10) bs and n=8 to collapse the middle      *)
                            (* CRC32CH chain (using SUB_LIST_SUB_LIST), then *)
                            (* RESIDUE8_X_UPDATE with bs := SUB_LIST(0,8) bs *)
                            (* and n=0 to collapse the inner CRC32CX chain   *)
                            (* (using SUB_LIST_IDEMPOTENT). Memory-close      *)
                            (* composes RESIDUE1_MEMORY_CLOSE atop            *)
                            (* RESIDUE2_MEMORY_CLOSE atop RESIDUE8_MEMORY_   *)
                            (* CLOSE. Pre-staging uses BYTES64_FROM_BYTELIST *)
                            (* (n=0), BYTES16_FROM_BYTELIST (n=8), and       *)
                            (* BYTES8_FROM_BYTELIST (n=10).                  *)
                            UNDISCH_TAC `len = 11` THEN DISCH_THEN SUBST_ALL_TAC THEN
                            SUBGOAL_THEN
                              `LENGTH (bs0:byte list) = 11 /\
                               LENGTH (bs1:byte list) = 11 /\
                               LENGTH (bs2:byte list) = 11 /\
                               LENGTH (bs3:byte list) = 11 /\
                               LENGTH (bs4:byte list) = 11 /\
                               LENGTH (bs5:byte list) = 11 /\
                               LENGTH (bs6:byte list) = 11 /\
                               LENGTH (bs7:byte list) = 11`
                              STRIP_ASSUME_TAC THENL
                             [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                            ENSURES_INIT_TAC "s0" THEN
                            (* Pre-stage 8 bytes64 (n=0), 8 bytes16 (n=8) and *)
                            (* 8 bytes8 (n=10) reads at s0 from the full-     *)
                            (* bytelist precondition.                          *)
                            MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                           `bs0:byte list`; `0:num`]
                                          BYTES64_FROM_BYTELIST) THEN
                            ANTS_TAC THENL
                            ...
                            DISCH_TAC THEN
                            MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                           `bs0:byte list`; `10:num`]
                                          BYTES8_FROM_BYTELIST) THEN
                            ANTS_TAC THENL
                             [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                            DISCH_TAC THEN
                            MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                           `bs1:byte list`; `10:num`]
                                          BYTES8_FROM_BYTELIST) THEN
                            ANTS_TAC THENL
                             [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                            DISCH_TAC THEN
                            MP_TAC(ISPECL [`a2:int64`; `s0:armstate`;
                                           `bs2:byte list`; `10:num`]
                                          BYTES8_FROM_BYTELIST) THEN
                            ANTS_TAC THENL
                             [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                            DISCH_TAC THEN
                            MP_TAC(ISPECL [`a3:int64`; `s0:armstate`;
                                           `bs3:byte list`; `10:num`]
                                          BYTES8_FROM_BYTELIST) THEN
                            ANTS_TAC THENL
                             [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                            DISCH_TAC THEN
                            MP_TAC(ISPECL [`a4:int64`; `s0:armstate`;
                                           `bs4:byte list`; `10:num`]
                                          BYTES8_FROM_BYTELIST) THEN
                            ANTS_TAC THENL
                             [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                            DISCH_TAC THEN
                            MP_TAC(ISPECL [`a5:int64`; `s0:armstate`;
                                           `bs5:byte list`; `10:num`]
                                          BYTES8_FROM_BYTELIST) THEN
                            ANTS_TAC THENL
                             [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                            DISCH_TAC THEN
                            MP_TAC(ISPECL [`a6:int64`; `s0:armstate`;
                                           `bs6:byte list`; `10:num`]
                                          BYTES8_FROM_BYTELIST) THEN
                            ANTS_TAC THENL
                             [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                            DISCH_TAC THEN
                            MP_TAC(ISPECL [`a7:int64`; `s0:armstate`;
                                           `bs7:byte list`; `10:num`]
                                          BYTES8_FROM_BYTELIST) THEN
                            ANTS_TAC THENL
                             [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                            DISCH_TAC THEN
                            ...
                            SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                     ...
                                     ARITH_RULE `8 <= 64`] THEN
                            REPEAT CONJ_TAC THENL
                             [(* X8: chain RESIDUE1_X_UPDATE (n=10) for outer  *)
                              (*   CRC32CB, then RESIDUE2_X_UPDATE              *)
                              (*   (bs := SUB_LIST(0,10) bs0, n=8) for middle  *)
                              (*   CRC32CH chain via SUB_LIST_SUB_LIST, then   *)
                              (*   RESIDUE8_X_UPDATE                           *)
                              (*   (bs := SUB_LIST(0,8) bs0, n=0) for inner    *)
                              (*   CRC32CX chain via SUB_LIST_IDEMPOTENT.      *)
                              AP_TERM_TAC THEN
                              MP_TAC(SPECL [`bs0:byte list`; `10:num`]
                                           RESIDUE1_X_UPDATE) THEN
                              REWRITE_TAC[ARITH_RULE `10 + 1 = 11`] THEN
                              ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                              DISCH_THEN(SUBST1_TAC o SYM) THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              SUBGOAL_THEN
                                `SUB_LIST(0, 8) (SUB_LIST(0, 10) (bs0:byte list))
                                   = SUB_LIST(0, 8) bs0 /\
                                 SUB_LIST(8, 2) (SUB_LIST(0, 10) (bs0:byte list))
                                   = SUB_LIST(8, 2) bs0`
                                STRIP_ASSUME_TAC THENL
                               [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                ARITH_TAC; ALL_TAC] THEN
                              MP_TAC(SPECL [`SUB_LIST(0,10) (bs0:byte list)`;
                                            `8:num`]
                                           RESIDUE2_X_UPDATE) THEN
                              ASM_REWRITE_TAC[] THEN
                              ANTS_TAC THENL
                              ...
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              MP_TAC(SPECL [`SUB_LIST(0,8) (bs0:byte list)`;
                                            `0:num`]
                                           RESIDUE8_X_UPDATE) THEN
                              REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                          SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                              ...
                              (* X9 *)
                              AP_TERM_TAC THEN
                              MP_TAC(SPECL [`bs1:byte list`; `10:num`]
                                           RESIDUE1_X_UPDATE) THEN
                              REWRITE_TAC[ARITH_RULE `10 + 1 = 11`] THEN
                              ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                              DISCH_THEN(SUBST1_TAC o SYM) THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              SUBGOAL_THEN
                                `SUB_LIST(0, 8) (SUB_LIST(0, 10) (bs1:byte list))
                                   = SUB_LIST(0, 8) bs1 /\
                                 SUB_LIST(8, 2) (SUB_LIST(0, 10) (bs1:byte list))
                                   = SUB_LIST(8, 2) bs1`
                                STRIP_ASSUME_TAC THENL
                               [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                ARITH_TAC; ALL_TAC] THEN
                              MP_TAC(SPECL [`SUB_LIST(0,10) (bs1:byte list)`;
                                            `8:num`]
                                           RESIDUE2_X_UPDATE) THEN
                              ASM_REWRITE_TAC[] THEN
                              ANTS_TAC THENL
                              ...
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              MP_TAC(SPECL [`SUB_LIST(0,8) (bs1:byte list)`;
                                            `0:num`]
                                           RESIDUE8_X_UPDATE) THEN
                              REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                          SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                              ...
                              (* X10 *)
                              AP_TERM_TAC THEN
                              MP_TAC(SPECL [`bs2:byte list`; `10:num`]
                                           RESIDUE1_X_UPDATE) THEN
                              REWRITE_TAC[ARITH_RULE `10 + 1 = 11`] THEN
                              ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                              DISCH_THEN(SUBST1_TAC o SYM) THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              SUBGOAL_THEN
                                `SUB_LIST(0, 8) (SUB_LIST(0, 10) (bs2:byte list))
                                   = SUB_LIST(0, 8) bs2 /\
                                 SUB_LIST(8, 2) (SUB_LIST(0, 10) (bs2:byte list))
                                   = SUB_LIST(8, 2) bs2`
                                STRIP_ASSUME_TAC THENL
                               [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                ARITH_TAC; ALL_TAC] THEN
                              MP_TAC(SPECL [`SUB_LIST(0,10) (bs2:byte list)`;
                                            `8:num`]
                                           RESIDUE2_X_UPDATE) THEN
                              ASM_REWRITE_TAC[] THEN
                              ANTS_TAC THENL
                              ...
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              MP_TAC(SPECL [`SUB_LIST(0,8) (bs2:byte list)`;
                                            `0:num`]
                                           RESIDUE8_X_UPDATE) THEN
                              REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                          SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                              ...
                              (* X11 *)
                              AP_TERM_TAC THEN
                              MP_TAC(SPECL [`bs3:byte list`; `10:num`]
                                           RESIDUE1_X_UPDATE) THEN
                              REWRITE_TAC[ARITH_RULE `10 + 1 = 11`] THEN
                              ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                              DISCH_THEN(SUBST1_TAC o SYM) THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              SUBGOAL_THEN
                                `SUB_LIST(0, 8) (SUB_LIST(0, 10) (bs3:byte list))
                                   = SUB_LIST(0, 8) bs3 /\
                                 SUB_LIST(8, 2) (SUB_LIST(0, 10) (bs3:byte list))
                                   = SUB_LIST(8, 2) bs3`
                                STRIP_ASSUME_TAC THENL
                               [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                ARITH_TAC; ALL_TAC] THEN
                              MP_TAC(SPECL [`SUB_LIST(0,10) (bs3:byte list)`;
                                            `8:num`]
                                           RESIDUE2_X_UPDATE) THEN
                              ASM_REWRITE_TAC[] THEN
                              ANTS_TAC THENL
                              ...
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              MP_TAC(SPECL [`SUB_LIST(0,8) (bs3:byte list)`;
                                            `0:num`]
                                           RESIDUE8_X_UPDATE) THEN
                              REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                          SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                              ...
                              (* X12 *)
                              AP_TERM_TAC THEN
                              MP_TAC(SPECL [`bs4:byte list`; `10:num`]
                                           RESIDUE1_X_UPDATE) THEN
                              REWRITE_TAC[ARITH_RULE `10 + 1 = 11`] THEN
                              ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                              DISCH_THEN(SUBST1_TAC o SYM) THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              SUBGOAL_THEN
                                `SUB_LIST(0, 8) (SUB_LIST(0, 10) (bs4:byte list))
                                   = SUB_LIST(0, 8) bs4 /\
                                 SUB_LIST(8, 2) (SUB_LIST(0, 10) (bs4:byte list))
                                   = SUB_LIST(8, 2) bs4`
                                STRIP_ASSUME_TAC THENL
                               [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                ARITH_TAC; ALL_TAC] THEN
                              MP_TAC(SPECL [`SUB_LIST(0,10) (bs4:byte list)`;
                                            `8:num`]
                                           RESIDUE2_X_UPDATE) THEN
                              ASM_REWRITE_TAC[] THEN
                              ANTS_TAC THENL
                              ...
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              MP_TAC(SPECL [`SUB_LIST(0,8) (bs4:byte list)`;
                                            `0:num`]
                                           RESIDUE8_X_UPDATE) THEN
                              REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                          SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                              ...
                              (* X13 *)
                              AP_TERM_TAC THEN
                              MP_TAC(SPECL [`bs5:byte list`; `10:num`]
                                           RESIDUE1_X_UPDATE) THEN
                              REWRITE_TAC[ARITH_RULE `10 + 1 = 11`] THEN
                              ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                              DISCH_THEN(SUBST1_TAC o SYM) THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              SUBGOAL_THEN
                                `SUB_LIST(0, 8) (SUB_LIST(0, 10) (bs5:byte list))
                                   = SUB_LIST(0, 8) bs5 /\
                                 SUB_LIST(8, 2) (SUB_LIST(0, 10) (bs5:byte list))
                                   = SUB_LIST(8, 2) bs5`
                                STRIP_ASSUME_TAC THENL
                               [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                ARITH_TAC; ALL_TAC] THEN
                              MP_TAC(SPECL [`SUB_LIST(0,10) (bs5:byte list)`;
                                            `8:num`]
                                           RESIDUE2_X_UPDATE) THEN
                              ASM_REWRITE_TAC[] THEN
                              ANTS_TAC THENL
                              ...
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              MP_TAC(SPECL [`SUB_LIST(0,8) (bs5:byte list)`;
                                            `0:num`]
                                           RESIDUE8_X_UPDATE) THEN
                              REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                          SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                              ...
                              (* X14 *)
                              AP_TERM_TAC THEN
                              MP_TAC(SPECL [`bs6:byte list`; `10:num`]
                                           RESIDUE1_X_UPDATE) THEN
                              REWRITE_TAC[ARITH_RULE `10 + 1 = 11`] THEN
                              ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                              DISCH_THEN(SUBST1_TAC o SYM) THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              SUBGOAL_THEN
                                `SUB_LIST(0, 8) (SUB_LIST(0, 10) (bs6:byte list))
                                   = SUB_LIST(0, 8) bs6 /\
                                 SUB_LIST(8, 2) (SUB_LIST(0, 10) (bs6:byte list))
                                   = SUB_LIST(8, 2) bs6`
                                STRIP_ASSUME_TAC THENL
                               [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                ARITH_TAC; ALL_TAC] THEN
                              MP_TAC(SPECL [`SUB_LIST(0,10) (bs6:byte list)`;
                                            `8:num`]
                                           RESIDUE2_X_UPDATE) THEN
                              ASM_REWRITE_TAC[] THEN
                              ANTS_TAC THENL
                              ...
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              MP_TAC(SPECL [`SUB_LIST(0,8) (bs6:byte list)`;
                                            `0:num`]
                                           RESIDUE8_X_UPDATE) THEN
                              REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                          SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                              ...
                              (* X15 *)
                              AP_TERM_TAC THEN
                              MP_TAC(SPECL [`bs7:byte list`; `10:num`]
                                           RESIDUE1_X_UPDATE) THEN
                              REWRITE_TAC[ARITH_RULE `10 + 1 = 11`] THEN
                              ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                              DISCH_THEN(SUBST1_TAC o SYM) THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              SUBGOAL_THEN
                                `SUB_LIST(0, 8) (SUB_LIST(0, 10) (bs7:byte list))
                                   = SUB_LIST(0, 8) bs7 /\
                                 SUB_LIST(8, 2) (SUB_LIST(0, 10) (bs7:byte list))
                                   = SUB_LIST(8, 2) bs7`
                                STRIP_ASSUME_TAC THENL
                               [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                ARITH_TAC; ALL_TAC] THEN
                              MP_TAC(SPECL [`SUB_LIST(0,10) (bs7:byte list)`;
                                            `8:num`]
                                           RESIDUE2_X_UPDATE) THEN
                              ASM_REWRITE_TAC[] THEN
                              ANTS_TAC THENL
                              ...
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              AP_THM_TAC THEN AP_TERM_TAC THEN
                              MP_TAC(SPECL [`SUB_LIST(0,8) (bs7:byte list)`;
                                            `0:num`]
                                           RESIDUE8_X_UPDATE) THEN
                              REWRITE_TAC[LENGTH_SUB_LIST; SUB_LIST_IDEMPOTENT;
                                          SUB_LIST; ADD_CLAUSES; crc32c_bytes] THEN
                              ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                              DISCH_THEN ACCEPT_TAC;
                              (* memory a0..a7: rewrite `bytelist (a, 11)` as     *)
                              (* `bytelist (a, ((0+8)+2)+1)` then chain            *)
                              (* RESIDUE1_MEMORY_CLOSE atop RESIDUE2_MEMORY_CLOSE *)
                              (* atop RESIDUE8_MEMORY_CLOSE.                       *)
                              SUBGOAL_THEN `(11:num) = ((0 + 8) + 2) + 1`
                                SUBST1_TAC THENL [ARITH_TAC; ALL_TAC] THEN
                              MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
                              CONJ_TAC THENL
                               [MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
                                CONJ_TAC THENL
                                 [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                                  ...
                                  REWRITE_TAC[ADD_CLAUSES] THEN
                                  ASM_REWRITE_TAC[]];
                                REWRITE_TAC[ADD_CLAUSES; ARITH_RULE `8 + 2 = 10`] THEN ASM_REWRITE_TAC[]];
                              SUBGOAL_THEN `(11:num) = ((0 + 8) + 2) + 1`
                                SUBST1_TAC THENL [ARITH_TAC; ALL_TAC] THEN
                              MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
                              CONJ_TAC THENL
                               ...
                               [MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
                                CONJ_TAC THENL
                                 [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                                  ...
                                  ASM_REWRITE_TAC[]];
                                REWRITE_TAC[ADD_CLAUSES; ARITH_RULE `8 + 2 = 10`] THEN ASM_REWRITE_TAC[]]];
                            ASM_CASES_TAC `len = 12` THENL
                             [(* Case len = 12: TBZ at bit 3 NOT taken; falls *)
                              (* through to the 8-byte block (24 instrs); bit *)
                              (* 2 NOT taken falls through to the 4-byte      *)
                              (* block (24 instrs); bit 1 taken; bit 0 taken  *)
                              (* to EOR. 52 ARM steps. Closure CHAINS         *)
                              (* singletons RESIDUE8 + RESIDUE4: per-buffer   *)
                              (* X-update applies RESIDUE4_X_UPDATE with n=8  *)
                              (* to consume the outer CRC32CW word steps,    *)
                              (* then RESIDUE8_X_UPDATE with bs := SUB_LIST  *)
                              (* (0,8) bs and n=0 (using SUB_LIST_IDEMPOTENT) *)
                              (* to collapse the inner CRC32CX chain. Memory *)
                              (* -close composes RESIDUE4_MEMORY_CLOSE        *)
                              (* (closes [0,12) from [0,8) + bytes32 (a + 8))*)
                              (* atop RESIDUE8_MEMORY_CLOSE (closes [0,8)   *)
                              (* from [0,0) + bytes64 a). Pre-staging uses  *)
                              (* BYTES64_FROM_BYTELIST (n=0) and             *)
                              (* BYTES32_FROM_BYTELIST (n=8).                *)
                              UNDISCH_TAC `len = 12` THEN DISCH_THEN SUBST_ALL_TAC THEN
                              SUBGOAL_THEN
                                `LENGTH (bs0:byte list) = 12 /\
                                 LENGTH (bs1:byte list) = 12 /\
                                 LENGTH (bs2:byte list) = 12 /\
                                 LENGTH (bs3:byte list) = 12 /\
                                 LENGTH (bs4:byte list) = 12 /\
                                 LENGTH (bs5:byte list) = 12 /\
                                 LENGTH (bs6:byte list) = 12 /\
                                 LENGTH (bs7:byte list) = 12`
                                STRIP_ASSUME_TAC THENL
                               [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                              ENSURES_INIT_TAC "s0" THEN
                              (* Pre-stage 8 bytes64 (n=0) and 8 bytes32 (n=8) *)
                              (* reads at s0 from the full-bytelist precondition*)
                              (* via BYTES{64,32}_FROM_BYTELIST.               *)
                              MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                             `bs0:byte list`; `0:num`]
                                            BYTES64_FROM_BYTELIST) THEN
                              ANTS_TAC THENL
                              ...
                              SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8;
                                       ...
                                       ARITH_RULE `8 <= 64`] THEN
                              REPEAT CONJ_TAC THENL
                               [(* X8: chain RESIDUE4_X_UPDATE (n=8) for outer  *)
                                (*    CRC32CW word, then RESIDUE8_X_UPDATE (bs *)
                                (*    := SUB_LIST(0,8) bs0, n=0) via            *)
                                (*    SUB_LIST_IDEMPOTENT to collapse inner    *)
                                (*    CRC32CX.                                 *)
                                AP_TERM_TAC THEN
                                MP_TAC(SPECL [`bs0:byte list`; `8:num`] RESIDUE4_X_UPDATE) THEN
                                ...
                                (* X9 *)
                                AP_TERM_TAC THEN
                                MP_TAC(SPECL [`bs1:byte list`; `8:num`] RESIDUE4_X_UPDATE) THEN
                                REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                DISCH_THEN(SUBST1_TAC o SYM) THEN
                                AP_THM_TAC THEN AP_TERM_TAC THEN
                                ...
                                (* X10 *)
                                AP_TERM_TAC THEN
                                MP_TAC(SPECL [`bs2:byte list`; `8:num`] RESIDUE4_X_UPDATE) THEN
                                REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                DISCH_THEN(SUBST1_TAC o SYM) THEN
                                AP_THM_TAC THEN AP_TERM_TAC THEN
                                ...
                                (* X11 *)
                                AP_TERM_TAC THEN
                                MP_TAC(SPECL [`bs3:byte list`; `8:num`] RESIDUE4_X_UPDATE) THEN
                                REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                DISCH_THEN(SUBST1_TAC o SYM) THEN
                                AP_THM_TAC THEN AP_TERM_TAC THEN
                                ...
                                (* X12 *)
                                AP_TERM_TAC THEN
                                MP_TAC(SPECL [`bs4:byte list`; `8:num`] RESIDUE4_X_UPDATE) THEN
                                REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                DISCH_THEN(SUBST1_TAC o SYM) THEN
                                AP_THM_TAC THEN AP_TERM_TAC THEN
                                ...
                                (* X13 *)
                                AP_TERM_TAC THEN
                                MP_TAC(SPECL [`bs5:byte list`; `8:num`] RESIDUE4_X_UPDATE) THEN
                                REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                DISCH_THEN(SUBST1_TAC o SYM) THEN
                                AP_THM_TAC THEN AP_TERM_TAC THEN
                                ...
                                (* X14 *)
                                AP_TERM_TAC THEN
                                MP_TAC(SPECL [`bs6:byte list`; `8:num`] RESIDUE4_X_UPDATE) THEN
                                REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                DISCH_THEN(SUBST1_TAC o SYM) THEN
                                AP_THM_TAC THEN AP_TERM_TAC THEN
                                ...
                                (* X15 *)
                                AP_TERM_TAC THEN
                                MP_TAC(SPECL [`bs7:byte list`; `8:num`] RESIDUE4_X_UPDATE) THEN
                                REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                DISCH_THEN(SUBST1_TAC o SYM) THEN
                                AP_THM_TAC THEN AP_TERM_TAC THEN
                                ...
                                ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                                DISCH_THEN ACCEPT_TAC;
                                (* memory a0..a7: rewrite `bytelist (a, 12)` as*)
                                (* `bytelist (a, (0+8)+4)` then chain          *)
                                (* RESIDUE4_MEMORY_CLOSE atop                  *)
                                (* RESIDUE8_MEMORY_CLOSE.                      *)
                                SUBGOAL_THEN `(12:num) = (0 + 8) + 4` SUBST1_TAC THENL
                                 [ARITH_TAC; ALL_TAC] THEN
                                MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                                CONJ_TAC THENL
                                 [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                                 ...
                                 [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                                  CONJ_TAC THENL
                                   [REWRITE_TAC[READ_COMPONENT_COMPOSE; bytelist_clauses;
                                    ...
                                    REWRITE_TAC[WORD_ADD_0] THEN ASM_REWRITE_TAC[]];
                                  REWRITE_TAC[ADD_CLAUSES] THEN ASM_REWRITE_TAC[]]];
                              ASM_CASES_TAC `len = 13` THENL
                               [(* Case len = 13: TBZ at bit 3 NOT taken;       *)
                                (* falls through to the 8-byte block (24        *)
                                (* instrs); bit 2 NOT taken falls through to    *)
                                (* the 4-byte block (24 instrs); bit 1 taken    *)
                                (* (skip 2-byte); bit 0 NOT taken falls through *)
                                (* to the 1-byte block (24 instrs). 76 ARM      *)
                                (* steps total. Closure CHAINS three singletons *)
                                (* RESIDUE8 + RESIDUE4 + RESIDUE1: per-buffer   *)
                                (* X-update applies RESIDUE1_X_UPDATE with n=12 *)
                                (* to consume the outer CRC32CB byte step, then *)
                                (* RESIDUE4_X_UPDATE with bs := SUB_LIST(0,12)  *)
                                (* bs and n=8 to collapse the middle CRC32CW    *)
                                (* word chain (using SUB_LIST_SUB_LIST), then   *)
                                (* RESIDUE8_X_UPDATE with bs := SUB_LIST(0,8)   *)
                                (* bs and n=0 to collapse the inner CRC32CX     *)
                                (* chain (using SUB_LIST_IDEMPOTENT). Memory-   *)
                                (* close composes RESIDUE1_MEMORY_CLOSE atop    *)
                                (* RESIDUE4_MEMORY_CLOSE atop RESIDUE8_MEMORY_  *)
                                (* CLOSE. Pre-staging uses BYTES64_FROM_BYTELIST*)
                                (* (n=0), BYTES32_FROM_BYTELIST (n=8), and      *)
                                (* BYTES8_FROM_BYTELIST (n=12).                 *)
                                UNDISCH_TAC `len = 13` THEN DISCH_THEN SUBST_ALL_TAC THEN
                                SUBGOAL_THEN
                                  `LENGTH (bs0:byte list) = 13 /\
                                   LENGTH (bs1:byte list) = 13 /\
                                   LENGTH (bs2:byte list) = 13 /\
                                   LENGTH (bs3:byte list) = 13 /\
                                   LENGTH (bs4:byte list) = 13 /\
                                   LENGTH (bs5:byte list) = 13 /\
                                   LENGTH (bs6:byte list) = 13 /\
                                   LENGTH (bs7:byte list) = 13`
                                  STRIP_ASSUME_TAC THENL
                                 [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                ENSURES_INIT_TAC "s0" THEN
                                (* Pre-stage 8 bytes64 (n=0), 8 bytes32 (n=8) and*)
                                (* 8 bytes8 (n=12) reads at s0 from the full-    *)
                                (* bytelist precondition.                        *)
                                MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                               `bs0:byte list`; `0:num`]
                                              BYTES64_FROM_BYTELIST) THEN
                                ...
                                DISCH_TAC THEN
                                MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                               `bs0:byte list`; `12:num`]
                                              BYTES8_FROM_BYTELIST) THEN
                                ANTS_TAC THENL
                                 [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                DISCH_TAC THEN
                                MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                               `bs1:byte list`; `12:num`]
                                              BYTES8_FROM_BYTELIST) THEN
                                ANTS_TAC THENL
                                 [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                DISCH_TAC THEN
                                MP_TAC(ISPECL [`a2:int64`; `s0:armstate`;
                                               `bs2:byte list`; `12:num`]
                                              BYTES8_FROM_BYTELIST) THEN
                                ANTS_TAC THENL
                                 [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                DISCH_TAC THEN
                                MP_TAC(ISPECL [`a3:int64`; `s0:armstate`;
                                               `bs3:byte list`; `12:num`]
                                              BYTES8_FROM_BYTELIST) THEN
                                ANTS_TAC THENL
                                 [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                DISCH_TAC THEN
                                MP_TAC(ISPECL [`a4:int64`; `s0:armstate`;
                                               `bs4:byte list`; `12:num`]
                                              BYTES8_FROM_BYTELIST) THEN
                                ANTS_TAC THENL
                                 [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                DISCH_TAC THEN
                                MP_TAC(ISPECL [`a5:int64`; `s0:armstate`;
                                               `bs5:byte list`; `12:num`]
                                              BYTES8_FROM_BYTELIST) THEN
                                ANTS_TAC THENL
                                 [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                DISCH_TAC THEN
                                MP_TAC(ISPECL [`a6:int64`; `s0:armstate`;
                                               `bs6:byte list`; `12:num`]
                                              BYTES8_FROM_BYTELIST) THEN
                                ANTS_TAC THENL
                                 [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                DISCH_TAC THEN
                                MP_TAC(ISPECL [`a7:int64`; `s0:armstate`;
                                               `bs7:byte list`; `12:num`]
                                              BYTES8_FROM_BYTELIST) THEN
                                ANTS_TAC THENL
                                 [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                DISCH_TAC THEN
                                ...
                                SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8;
                                         ...
                                         ARITH_RULE `8 <= 64`] THEN
                                REPEAT CONJ_TAC THENL
                                 [(* X8: chain RESIDUE1_X_UPDATE (n=12) for outer*)
                                  (*   CRC32CB byte, then RESIDUE4_X_UPDATE      *)
                                  (*   (bs := SUB_LIST(0,12) bs0, n=8) for       *)
                                  (*   middle CRC32CW chain via SUB_LIST_SUB_    *)
                                  (*   LIST, then RESIDUE8_X_UPDATE              *)
                                  (*   (bs := SUB_LIST(0,8) bs0, n=0) for inner  *)
                                  (*   CRC32CX chain via SUB_LIST_IDEMPOTENT.    *)
                                  AP_TERM_TAC THEN
                                  MP_TAC(SPECL [`bs0:byte list`; `12:num`]
                                               RESIDUE1_X_UPDATE) THEN
                                  REWRITE_TAC[ARITH_RULE `12 + 1 = 13`] THEN
                                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                                  AP_THM_TAC THEN AP_TERM_TAC THEN
                                  SUBGOAL_THEN
                                    `SUB_LIST(0, 8) (SUB_LIST(0, 12) (bs0:byte list))
                                       = SUB_LIST(0, 8) bs0 /\
                                     SUB_LIST(8, 4) (SUB_LIST(0, 12) (bs0:byte list))
                                       = SUB_LIST(8, 4) bs0`
                                    STRIP_ASSUME_TAC THENL
                                   [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                    ARITH_TAC; ALL_TAC] THEN
                                  MP_TAC(SPECL [`SUB_LIST(0,12) (bs0:byte list)`;
                                                `8:num`]
                                               RESIDUE4_X_UPDATE) THEN
                                  ASM_REWRITE_TAC[] THEN
                                  REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                  ...
                                  (* X9 *)
                                  AP_TERM_TAC THEN
                                  MP_TAC(SPECL [`bs1:byte list`; `12:num`]
                                               RESIDUE1_X_UPDATE) THEN
                                  REWRITE_TAC[ARITH_RULE `12 + 1 = 13`] THEN
                                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                                  AP_THM_TAC THEN AP_TERM_TAC THEN
                                  SUBGOAL_THEN
                                    `SUB_LIST(0, 8) (SUB_LIST(0, 12) (bs1:byte list))
                                       = SUB_LIST(0, 8) bs1 /\
                                     SUB_LIST(8, 4) (SUB_LIST(0, 12) (bs1:byte list))
                                       = SUB_LIST(8, 4) bs1`
                                    STRIP_ASSUME_TAC THENL
                                   [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                    ARITH_TAC; ALL_TAC] THEN
                                  MP_TAC(SPECL [`SUB_LIST(0,12) (bs1:byte list)`;
                                                `8:num`]
                                               RESIDUE4_X_UPDATE) THEN
                                  ASM_REWRITE_TAC[] THEN
                                  REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                  ...
                                  (* X10 *)
                                  AP_TERM_TAC THEN
                                  MP_TAC(SPECL [`bs2:byte list`; `12:num`]
                                               RESIDUE1_X_UPDATE) THEN
                                  REWRITE_TAC[ARITH_RULE `12 + 1 = 13`] THEN
                                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                                  AP_THM_TAC THEN AP_TERM_TAC THEN
                                  SUBGOAL_THEN
                                    `SUB_LIST(0, 8) (SUB_LIST(0, 12) (bs2:byte list))
                                       = SUB_LIST(0, 8) bs2 /\
                                     SUB_LIST(8, 4) (SUB_LIST(0, 12) (bs2:byte list))
                                       = SUB_LIST(8, 4) bs2`
                                    STRIP_ASSUME_TAC THENL
                                   [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                    ARITH_TAC; ALL_TAC] THEN
                                  MP_TAC(SPECL [`SUB_LIST(0,12) (bs2:byte list)`;
                                                `8:num`]
                                               RESIDUE4_X_UPDATE) THEN
                                  ASM_REWRITE_TAC[] THEN
                                  REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                  ...
                                  (* X11 *)
                                  AP_TERM_TAC THEN
                                  MP_TAC(SPECL [`bs3:byte list`; `12:num`]
                                               RESIDUE1_X_UPDATE) THEN
                                  REWRITE_TAC[ARITH_RULE `12 + 1 = 13`] THEN
                                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                                  AP_THM_TAC THEN AP_TERM_TAC THEN
                                  SUBGOAL_THEN
                                    `SUB_LIST(0, 8) (SUB_LIST(0, 12) (bs3:byte list))
                                       = SUB_LIST(0, 8) bs3 /\
                                     SUB_LIST(8, 4) (SUB_LIST(0, 12) (bs3:byte list))
                                       = SUB_LIST(8, 4) bs3`
                                    STRIP_ASSUME_TAC THENL
                                   [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                    ARITH_TAC; ALL_TAC] THEN
                                  MP_TAC(SPECL [`SUB_LIST(0,12) (bs3:byte list)`;
                                                `8:num`]
                                               RESIDUE4_X_UPDATE) THEN
                                  ASM_REWRITE_TAC[] THEN
                                  REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                  ...
                                  (* X12 *)
                                  AP_TERM_TAC THEN
                                  MP_TAC(SPECL [`bs4:byte list`; `12:num`]
                                               RESIDUE1_X_UPDATE) THEN
                                  REWRITE_TAC[ARITH_RULE `12 + 1 = 13`] THEN
                                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                                  AP_THM_TAC THEN AP_TERM_TAC THEN
                                  SUBGOAL_THEN
                                    `SUB_LIST(0, 8) (SUB_LIST(0, 12) (bs4:byte list))
                                       = SUB_LIST(0, 8) bs4 /\
                                     SUB_LIST(8, 4) (SUB_LIST(0, 12) (bs4:byte list))
                                       = SUB_LIST(8, 4) bs4`
                                    STRIP_ASSUME_TAC THENL
                                   [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                    ARITH_TAC; ALL_TAC] THEN
                                  MP_TAC(SPECL [`SUB_LIST(0,12) (bs4:byte list)`;
                                                `8:num`]
                                               RESIDUE4_X_UPDATE) THEN
                                  ASM_REWRITE_TAC[] THEN
                                  REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                  ...
                                  (* X13 *)
                                  AP_TERM_TAC THEN
                                  MP_TAC(SPECL [`bs5:byte list`; `12:num`]
                                               RESIDUE1_X_UPDATE) THEN
                                  REWRITE_TAC[ARITH_RULE `12 + 1 = 13`] THEN
                                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                                  AP_THM_TAC THEN AP_TERM_TAC THEN
                                  SUBGOAL_THEN
                                    `SUB_LIST(0, 8) (SUB_LIST(0, 12) (bs5:byte list))
                                       = SUB_LIST(0, 8) bs5 /\
                                     SUB_LIST(8, 4) (SUB_LIST(0, 12) (bs5:byte list))
                                       = SUB_LIST(8, 4) bs5`
                                    STRIP_ASSUME_TAC THENL
                                   [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                    ARITH_TAC; ALL_TAC] THEN
                                  MP_TAC(SPECL [`SUB_LIST(0,12) (bs5:byte list)`;
                                                `8:num`]
                                               RESIDUE4_X_UPDATE) THEN
                                  ASM_REWRITE_TAC[] THEN
                                  REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                  ...
                                  (* X14 *)
                                  AP_TERM_TAC THEN
                                  MP_TAC(SPECL [`bs6:byte list`; `12:num`]
                                               RESIDUE1_X_UPDATE) THEN
                                  REWRITE_TAC[ARITH_RULE `12 + 1 = 13`] THEN
                                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                                  AP_THM_TAC THEN AP_TERM_TAC THEN
                                  SUBGOAL_THEN
                                    `SUB_LIST(0, 8) (SUB_LIST(0, 12) (bs6:byte list))
                                       = SUB_LIST(0, 8) bs6 /\
                                     SUB_LIST(8, 4) (SUB_LIST(0, 12) (bs6:byte list))
                                       = SUB_LIST(8, 4) bs6`
                                    STRIP_ASSUME_TAC THENL
                                   [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                    ARITH_TAC; ALL_TAC] THEN
                                  MP_TAC(SPECL [`SUB_LIST(0,12) (bs6:byte list)`;
                                                `8:num`]
                                               RESIDUE4_X_UPDATE) THEN
                                  ASM_REWRITE_TAC[] THEN
                                  REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                  ...
                                  (* X15 *)
                                  AP_TERM_TAC THEN
                                  MP_TAC(SPECL [`bs7:byte list`; `12:num`]
                                               RESIDUE1_X_UPDATE) THEN
                                  REWRITE_TAC[ARITH_RULE `12 + 1 = 13`] THEN
                                  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                  DISCH_THEN(SUBST1_TAC o SYM) THEN
                                  AP_THM_TAC THEN AP_TERM_TAC THEN
                                  SUBGOAL_THEN
                                    `SUB_LIST(0, 8) (SUB_LIST(0, 12) (bs7:byte list))
                                       = SUB_LIST(0, 8) bs7 /\
                                     SUB_LIST(8, 4) (SUB_LIST(0, 12) (bs7:byte list))
                                       = SUB_LIST(8, 4) bs7`
                                    STRIP_ASSUME_TAC THENL
                                   [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                    ARITH_TAC; ALL_TAC] THEN
                                  MP_TAC(SPECL [`SUB_LIST(0,12) (bs7:byte list)`;
                                                `8:num`]
                                               RESIDUE4_X_UPDATE) THEN
                                  ASM_REWRITE_TAC[] THEN
                                  REWRITE_TAC[ARITH_RULE `8 + 4 = 12`] THEN
                                  ...
                                  ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                                  DISCH_THEN ACCEPT_TAC;
                                  (* memory a0..a7: rewrite `bytelist (a, 13)`   *)
                                  (* as `bytelist (a, ((0+8)+4)+1)` then chain   *)
                                  (* RESIDUE1_MEMORY_CLOSE atop                  *)
                                  (* RESIDUE4_MEMORY_CLOSE atop                  *)
                                  (* RESIDUE8_MEMORY_CLOSE.                      *)
                                  SUBGOAL_THEN `(13:num) = ((0 + 8) + 4) + 1`
                                    SUBST1_TAC THENL [ARITH_TAC; ALL_TAC] THEN
                                  MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
                                  CONJ_TAC THENL
                                   [MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                                    CONJ_TAC THENL
                                     [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                                      ...
                                      REWRITE_TAC[ADD_CLAUSES] THEN
                                      ASM_REWRITE_TAC[]];
                                    REWRITE_TAC[ADD_CLAUSES; ARITH_RULE `8 + 4 = 12`] THEN ASM_REWRITE_TAC[]];
                                  SUBGOAL_THEN `(13:num) = ((0 + 8) + 4) + 1`
                                    SUBST1_TAC THENL [ARITH_TAC; ALL_TAC] THEN
                                  MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
                                  CONJ_TAC THENL
                                   ...
                                   [MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                                    CONJ_TAC THENL
                                     [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                                      ...
                                      ASM_REWRITE_TAC[]];
                                    REWRITE_TAC[ADD_CLAUSES; ARITH_RULE `8 + 4 = 12`] THEN ASM_REWRITE_TAC[]]];
                                ASM_CASES_TAC `len = 14` THENL
                                 [(* Case len = 14: TBZ at bit 3 NOT taken;       *)
                                  (* falls through to the 8-byte block (24        *)
                                  (* instrs); bit 2 NOT taken falls through to    *)
                                  (* the 4-byte block (24 instrs); bit 1 NOT      *)
                                  (* taken falls through to the 2-byte block (24  *)
                                  (* instrs); bit 0 taken to EOR. 76 ARM steps    *)
                                  (* total. Closure CHAINS three singletons       *)
                                  (* RESIDUE8 + RESIDUE4 + RESIDUE2: per-buffer   *)
                                  (* X-update applies RESIDUE2_X_UPDATE with n=12 *)
                                  (* to consume the outer CRC32CH halfword steps, *)
                                  (* then RESIDUE4_X_UPDATE with bs := SUB_LIST   *)
                                  (* (0,12) bs and n=8 to collapse the middle     *)
                                  (* CRC32CW chain (using SUB_LIST_SUB_LIST),     *)
                                  (* then RESIDUE8_X_UPDATE with bs := SUB_LIST   *)
                                  (* (0,8) bs and n=0 to collapse the inner       *)
                                  (* CRC32CX chain (using SUB_LIST_IDEMPOTENT).   *)
                                  (* Memory-close composes RESIDUE2_MEMORY_CLOSE  *)
                                  (* atop RESIDUE4_MEMORY_CLOSE atop RESIDUE8_    *)
                                  (* MEMORY_CLOSE. Pre-staging uses BYTES64_FROM_ *)
                                  (* BYTELIST (n=0), BYTES32_FROM_BYTELIST (n=8), *)
                                  (* and BYTES16_FROM_BYTELIST (n=12).            *)
                                  UNDISCH_TAC `len = 14` THEN DISCH_THEN SUBST_ALL_TAC THEN
                                  SUBGOAL_THEN
                                    `LENGTH (bs0:byte list) = 14 /\
                                     LENGTH (bs1:byte list) = 14 /\
                                     LENGTH (bs2:byte list) = 14 /\
                                     LENGTH (bs3:byte list) = 14 /\
                                     LENGTH (bs4:byte list) = 14 /\
                                     LENGTH (bs5:byte list) = 14 /\
                                     LENGTH (bs6:byte list) = 14 /\
                                     LENGTH (bs7:byte list) = 14`
                                    STRIP_ASSUME_TAC THENL
                                   [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                  ENSURES_INIT_TAC "s0" THEN
                                  (* Pre-stage 8 bytes64 (n=0), 8 bytes32 (n=8) and*)
                                  ...
                                  SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                           ...
                                           ARITH_RULE `8 <= 64`] THEN
                                  REPEAT CONJ_TAC THENL
                                   [(* X8: chain RESIDUE2_X_UPDATE (n=12) for outer*)
                                    (*   CRC32CH halfword, then RESIDUE4_X_UPDATE      *)
                                    (*   (bs := SUB_LIST(0,12) bs0, n=8) for       *)
                                    (*   middle CRC32CW chain via SUB_LIST_SUB_    *)
                                    (*   LIST, then RESIDUE8_X_UPDATE              *)
                                    (*   (bs := SUB_LIST(0,8) bs0, n=0) for inner  *)
                                    ...
                                    MP_TAC(SPECL [`bs0:byte list`; `12:num`]
                                                 RESIDUE2_X_UPDATE) THEN
                                    REWRITE_TAC[ARITH_RULE `12 + 2 = 14`] THEN
                                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                                    AP_THM_TAC THEN AP_TERM_TAC THEN
                                    AP_THM_TAC THEN AP_TERM_TAC THEN
                                    ...
                                    (* memory a0..a7: rewrite `bytelist (a, 13)`   *)
                                    (* as `bytelist (a, ((0+8)+4)+1)` then chain   *)
                                    (* RESIDUE2_MEMORY_CLOSE atop                  *)
                                    (* RESIDUE4_MEMORY_CLOSE atop                  *)
                                    (* RESIDUE8_MEMORY_CLOSE.                      *)
                                    SUBGOAL_THEN `(14:num) = ((0 + 8) + 4) + 2`
                                      SUBST1_TAC THENL [ARITH_TAC; ALL_TAC] THEN
                                    MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
                                    CONJ_TAC THENL
                                     [MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                                     ...
                                     [MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                                      CONJ_TAC THENL
                                       [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                                        ...
                                        ASM_REWRITE_TAC[]];
                                      REWRITE_TAC[ADD_CLAUSES; ARITH_RULE `8 + 4 = 12`] THEN ASM_REWRITE_TAC[]]];
                                  ASM_CASES_TAC `len = 15` THENL
                                   [(* Case len = 15: TBZ at bit 3 NOT taken;       *)
                                    (* falls through to the 8-byte block (24        *)
                                    (* instrs); bit 2 NOT taken falls through to    *)
                                    (* the 4-byte block (24 instrs); bit 1 NOT      *)
                                    (* taken falls through to the 2-byte block (24  *)
                                    (* instrs); bit 0 NOT taken falls through to    *)
                                    (* the 1-byte block (24 instrs). 100 ARM steps. *)
                                    (* Closure CHAINS four singletons RESIDUE8 +    *)
                                    (* RESIDUE4 + RESIDUE2 + RESIDUE1: per-buffer   *)
                                    (* X-update applies RESIDUE1_X_UPDATE with n=14 *)
                                    (* to consume the outer CRC32CB byte step, then *)
                                    (* RESIDUE2_X_UPDATE with bs := SUB_LIST(0,14)  *)
                                    (* bs and n=12 to collapse the CRC32CH halfword *)
                                    (* (using SUB_LIST_SUB_LIST), then RESIDUE4_X_  *)
                                    (* UPDATE with bs := SUB_LIST(0,12) bs and n=8  *)
                                    (* (using SUB_LIST_SUB_LIST), then RESIDUE8_X_  *)
                                    (* UPDATE with bs := SUB_LIST(0,8) bs and n=0   *)
                                    (* (using SUB_LIST_IDEMPOTENT). Memory-close    *)
                                    (* composes RESIDUE1_MEMORY_CLOSE atop RESIDUE2 *)
                                    (* _MEMORY_CLOSE atop RESIDUE4_MEMORY_CLOSE     *)
                                    (* atop RESIDUE8_MEMORY_CLOSE. Pre-staging uses *)
                                    (* BYTES64 (n=0), BYTES32 (n=8), BYTES16 (n=12),*)
                                    (* and BYTES8 (n=14).                           *)
                                    UNDISCH_TAC `len = 15` THEN DISCH_THEN SUBST_ALL_TAC THEN
                                    SUBGOAL_THEN
                                      `LENGTH (bs0:byte list) = 15 /\
                                       LENGTH (bs1:byte list) = 15 /\
                                       LENGTH (bs2:byte list) = 15 /\
                                       LENGTH (bs3:byte list) = 15 /\
                                       LENGTH (bs4:byte list) = 15 /\
                                       LENGTH (bs5:byte list) = 15 /\
                                       LENGTH (bs6:byte list) = 15 /\
                                       LENGTH (bs7:byte list) = 15`
                                      STRIP_ASSUME_TAC THENL
                                     [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                    ENSURES_INIT_TAC "s0" THEN
                                    (* Pre-stage 8 BYTES64 (n=0), 8 BYTES32 (n=8),   *)
                                    (* 8 BYTES16 (n=12), 8 BYTES8 (n=14) reads at s0.*)
                                    MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                   `bs0:byte list`; `0:num`]
                                                  BYTES64_FROM_BYTELIST) THEN
                                    ANTS_TAC THENL
                                    ...
                                    DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                   `bs0:byte list`; `14:num`]
                                                  BYTES8_FROM_BYTELIST) THEN
                                    ANTS_TAC THENL
                                     [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                    DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                   `bs1:byte list`; `14:num`]
                                                  BYTES8_FROM_BYTELIST) THEN
                                    ANTS_TAC THENL
                                     [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                    DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a2:int64`; `s0:armstate`;
                                                   `bs2:byte list`; `14:num`]
                                                  BYTES8_FROM_BYTELIST) THEN
                                    ANTS_TAC THENL
                                     [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                    DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a3:int64`; `s0:armstate`;
                                                   `bs3:byte list`; `14:num`]
                                                  BYTES8_FROM_BYTELIST) THEN
                                    ANTS_TAC THENL
                                     [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                    DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a4:int64`; `s0:armstate`;
                                                   `bs4:byte list`; `14:num`]
                                                  BYTES8_FROM_BYTELIST) THEN
                                    ANTS_TAC THENL
                                     [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                    DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a5:int64`; `s0:armstate`;
                                                   `bs5:byte list`; `14:num`]
                                                  BYTES8_FROM_BYTELIST) THEN
                                    ANTS_TAC THENL
                                     [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                    DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a6:int64`; `s0:armstate`;
                                                   `bs6:byte list`; `14:num`]
                                                  BYTES8_FROM_BYTELIST) THEN
                                    ANTS_TAC THENL
                                     [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                    DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a7:int64`; `s0:armstate`;
                                                   `bs7:byte list`; `14:num`]
                                                  BYTES8_FROM_BYTELIST) THEN
                                    ANTS_TAC THENL
                                     [ASM_REWRITE_TAC[] THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                    DISCH_TAC THEN
                                    ARM_STEPS_TAC CRC32C_OCTO_ZERO_FILL_XOR_EXEC (1--100) THEN
                                    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
                                    SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                             DIMINDEX_32; DIMINDEX_64;
                                             ARITH_RULE `8 <= 32`; ARITH_RULE `16 <= 32`;
                                             ...
                                             ARITH_RULE `8 <= 64`] THEN
                                    REPEAT CONJ_TAC THENL
                                     [(* X8: chain RESIDUE1 (n=14), RESIDUE2 (n=12),   *)
                                      (* RESIDUE4 (n=8), RESIDUE8 (n=0).               *)
                                    AP_TERM_TAC THEN
                                    MP_TAC(SPECL [`bs0:byte list`; `14:num`]
                                                 RESIDUE1_X_UPDATE) THEN
                                    REWRITE_TAC[ARITH_RULE `14 + 1 = 15`] THEN
                                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                                    AP_THM_TAC THEN AP_TERM_TAC THEN
                                    SUBGOAL_THEN
                                      `SUB_LIST(0, 12) (SUB_LIST(0, 14) (bs0:byte list))
                                         = SUB_LIST(0, 12) bs0 /\
                                       SUB_LIST(12, 2) (SUB_LIST(0, 14) (bs0:byte list))
                                         = SUB_LIST(12, 2) bs0`
                                      STRIP_ASSUME_TAC THENL
                                     [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                      ARITH_TAC; ALL_TAC] THEN
                                    MP_TAC(SPECL [`SUB_LIST(0,14) (bs0:byte list)`;
                                                  `12:num`]
                                                 RESIDUE2_X_UPDATE) THEN
                                    ASM_REWRITE_TAC[] THEN
                                    REWRITE_TAC[ARITH_RULE `12 + 2 = 14`] THEN
                                    ...
                                    DISCH_THEN ACCEPT_TAC;
                                      (* X9 *)
                                    AP_TERM_TAC THEN
                                    MP_TAC(SPECL [`bs1:byte list`; `14:num`]
                                                 RESIDUE1_X_UPDATE) THEN
                                    REWRITE_TAC[ARITH_RULE `14 + 1 = 15`] THEN
                                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                                    AP_THM_TAC THEN AP_TERM_TAC THEN
                                    SUBGOAL_THEN
                                      `SUB_LIST(0, 12) (SUB_LIST(0, 14) (bs1:byte list))
                                         = SUB_LIST(0, 12) bs1 /\
                                       SUB_LIST(12, 2) (SUB_LIST(0, 14) (bs1:byte list))
                                         = SUB_LIST(12, 2) bs1`
                                      STRIP_ASSUME_TAC THENL
                                     [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                      ARITH_TAC; ALL_TAC] THEN
                                    MP_TAC(SPECL [`SUB_LIST(0,14) (bs1:byte list)`;
                                                  `12:num`]
                                                 RESIDUE2_X_UPDATE) THEN
                                    ASM_REWRITE_TAC[] THEN
                                    REWRITE_TAC[ARITH_RULE `12 + 2 = 14`] THEN
                                    ...
                                    DISCH_THEN ACCEPT_TAC;
                                      (* X10 *)
                                    AP_TERM_TAC THEN
                                    MP_TAC(SPECL [`bs2:byte list`; `14:num`]
                                                 RESIDUE1_X_UPDATE) THEN
                                    REWRITE_TAC[ARITH_RULE `14 + 1 = 15`] THEN
                                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                                    AP_THM_TAC THEN AP_TERM_TAC THEN
                                    SUBGOAL_THEN
                                      `SUB_LIST(0, 12) (SUB_LIST(0, 14) (bs2:byte list))
                                         = SUB_LIST(0, 12) bs2 /\
                                       SUB_LIST(12, 2) (SUB_LIST(0, 14) (bs2:byte list))
                                         = SUB_LIST(12, 2) bs2`
                                      STRIP_ASSUME_TAC THENL
                                     [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                      ARITH_TAC; ALL_TAC] THEN
                                    MP_TAC(SPECL [`SUB_LIST(0,14) (bs2:byte list)`;
                                                  `12:num`]
                                                 RESIDUE2_X_UPDATE) THEN
                                    ASM_REWRITE_TAC[] THEN
                                    REWRITE_TAC[ARITH_RULE `12 + 2 = 14`] THEN
                                    ...
                                    DISCH_THEN ACCEPT_TAC;
                                      (* X11 *)
                                    AP_TERM_TAC THEN
                                    MP_TAC(SPECL [`bs3:byte list`; `14:num`]
                                                 RESIDUE1_X_UPDATE) THEN
                                    REWRITE_TAC[ARITH_RULE `14 + 1 = 15`] THEN
                                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                                    AP_THM_TAC THEN AP_TERM_TAC THEN
                                    SUBGOAL_THEN
                                      `SUB_LIST(0, 12) (SUB_LIST(0, 14) (bs3:byte list))
                                         = SUB_LIST(0, 12) bs3 /\
                                       SUB_LIST(12, 2) (SUB_LIST(0, 14) (bs3:byte list))
                                         = SUB_LIST(12, 2) bs3`
                                      STRIP_ASSUME_TAC THENL
                                     [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                      ARITH_TAC; ALL_TAC] THEN
                                    MP_TAC(SPECL [`SUB_LIST(0,14) (bs3:byte list)`;
                                                  `12:num`]
                                                 RESIDUE2_X_UPDATE) THEN
                                    ASM_REWRITE_TAC[] THEN
                                    REWRITE_TAC[ARITH_RULE `12 + 2 = 14`] THEN
                                    ...
                                    DISCH_THEN ACCEPT_TAC;
                                      (* X12 *)
                                    AP_TERM_TAC THEN
                                    MP_TAC(SPECL [`bs4:byte list`; `14:num`]
                                                 RESIDUE1_X_UPDATE) THEN
                                    REWRITE_TAC[ARITH_RULE `14 + 1 = 15`] THEN
                                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                                    AP_THM_TAC THEN AP_TERM_TAC THEN
                                    SUBGOAL_THEN
                                      `SUB_LIST(0, 12) (SUB_LIST(0, 14) (bs4:byte list))
                                         = SUB_LIST(0, 12) bs4 /\
                                       SUB_LIST(12, 2) (SUB_LIST(0, 14) (bs4:byte list))
                                         = SUB_LIST(12, 2) bs4`
                                      STRIP_ASSUME_TAC THENL
                                     [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                      ARITH_TAC; ALL_TAC] THEN
                                    MP_TAC(SPECL [`SUB_LIST(0,14) (bs4:byte list)`;
                                                  `12:num`]
                                                 RESIDUE2_X_UPDATE) THEN
                                    ASM_REWRITE_TAC[] THEN
                                    REWRITE_TAC[ARITH_RULE `12 + 2 = 14`] THEN
                                    ...
                                    DISCH_THEN ACCEPT_TAC;
                                      (* X13 *)
                                    AP_TERM_TAC THEN
                                    MP_TAC(SPECL [`bs5:byte list`; `14:num`]
                                                 RESIDUE1_X_UPDATE) THEN
                                    REWRITE_TAC[ARITH_RULE `14 + 1 = 15`] THEN
                                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                                    AP_THM_TAC THEN AP_TERM_TAC THEN
                                    SUBGOAL_THEN
                                      `SUB_LIST(0, 12) (SUB_LIST(0, 14) (bs5:byte list))
                                         = SUB_LIST(0, 12) bs5 /\
                                       SUB_LIST(12, 2) (SUB_LIST(0, 14) (bs5:byte list))
                                         = SUB_LIST(12, 2) bs5`
                                      STRIP_ASSUME_TAC THENL
                                     [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                      ARITH_TAC; ALL_TAC] THEN
                                    MP_TAC(SPECL [`SUB_LIST(0,14) (bs5:byte list)`;
                                                  `12:num`]
                                                 RESIDUE2_X_UPDATE) THEN
                                    ASM_REWRITE_TAC[] THEN
                                    REWRITE_TAC[ARITH_RULE `12 + 2 = 14`] THEN
                                    ...
                                    DISCH_THEN ACCEPT_TAC;
                                      (* X14 *)
                                    AP_TERM_TAC THEN
                                    MP_TAC(SPECL [`bs6:byte list`; `14:num`]
                                                 RESIDUE1_X_UPDATE) THEN
                                    REWRITE_TAC[ARITH_RULE `14 + 1 = 15`] THEN
                                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                                    AP_THM_TAC THEN AP_TERM_TAC THEN
                                    SUBGOAL_THEN
                                      `SUB_LIST(0, 12) (SUB_LIST(0, 14) (bs6:byte list))
                                         = SUB_LIST(0, 12) bs6 /\
                                       SUB_LIST(12, 2) (SUB_LIST(0, 14) (bs6:byte list))
                                         = SUB_LIST(12, 2) bs6`
                                      STRIP_ASSUME_TAC THENL
                                     [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                      ARITH_TAC; ALL_TAC] THEN
                                    MP_TAC(SPECL [`SUB_LIST(0,14) (bs6:byte list)`;
                                                  `12:num`]
                                                 RESIDUE2_X_UPDATE) THEN
                                    ASM_REWRITE_TAC[] THEN
                                    REWRITE_TAC[ARITH_RULE `12 + 2 = 14`] THEN
                                    ...
                                    DISCH_THEN ACCEPT_TAC;
                                      (* X15 *)
                                    AP_TERM_TAC THEN
                                    MP_TAC(SPECL [`bs7:byte list`; `14:num`]
                                                 RESIDUE1_X_UPDATE) THEN
                                    REWRITE_TAC[ARITH_RULE `14 + 1 = 15`] THEN
                                    ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
                                    DISCH_THEN(SUBST1_TAC o SYM) THEN
                                    AP_THM_TAC THEN AP_TERM_TAC THEN
                                    SUBGOAL_THEN
                                      `SUB_LIST(0, 12) (SUB_LIST(0, 14) (bs7:byte list))
                                         = SUB_LIST(0, 12) bs7 /\
                                       SUB_LIST(12, 2) (SUB_LIST(0, 14) (bs7:byte list))
                                         = SUB_LIST(12, 2) bs7`
                                      STRIP_ASSUME_TAC THENL
                                     [CONJ_TAC THEN MATCH_MP_TAC SUB_LIST_SUB_LIST THEN
                                      ARITH_TAC; ALL_TAC] THEN
                                    MP_TAC(SPECL [`SUB_LIST(0,14) (bs7:byte list)`;
                                                  `12:num`]
                                                 RESIDUE2_X_UPDATE) THEN
                                    ASM_REWRITE_TAC[] THEN
                                    REWRITE_TAC[ARITH_RULE `12 + 2 = 14`] THEN
                                    ...
                                    ANTS_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
                                    DISCH_THEN ACCEPT_TAC;
                                      (* memory a0: rewrite `bytelist (a, 15)` as     *)
                                      (* `bytelist (a, (((0+8)+4)+2)+1)` then chain    *)
                                      (* RESIDUE1_MEMORY_CLOSE atop RESIDUE2 atop      *)
                                      (* RESIDUE4 atop RESIDUE8.                       *)
                                    SUBGOAL_THEN `(15:num) = (((0 + 8) + 4) + 2) + 1`
                                      SUBST1_TAC THENL [ARITH_TAC; ALL_TAC] THEN
                                    MATCH_MP_TAC RESIDUE1_MEMORY_CLOSE THEN
                                    CONJ_TAC THENL
                                     [MATCH_MP_TAC RESIDUE2_MEMORY_CLOSE THEN
                                       ...
                                       [MATCH_MP_TAC RESIDUE4_MEMORY_CLOSE THEN
                                        CONJ_TAC THENL
                                         [MATCH_MP_TAC RESIDUE8_MEMORY_CLOSE THEN
                                          ...
                                          REWRITE_TAC[ADD_CLAUSES] THEN
                                          ASM_REWRITE_TAC[]];
                                        REWRITE_TAC[ADD_CLAUSES; ARITH_RULE `8 + 4 = 12`] THEN
                                        ASM_REWRITE_TAC[]];
                                      REWRITE_TAC[ADD_CLAUSES; ARITH_RULE `8 + 4 = 12`;
                                                  ARITH_RULE `12 + 2 = 14`] THEN
                                      ASM_REWRITE_TAC[]];
                                      (* memory a1 *)
                                    SUBGOAL_THEN `(15:num) = (((0 + 8) + 4) + 2) + 1`
                                      SUBST1_TAC THENL [ARITH_TAC; ALL_TAC] THEN
                     ...
                     [(* Case residue = 12: TBZ at bit 3 not-taken; runs the 8-byte *)
                      ...
                      REPEAT CONJ_TAC THENL
                       [AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE12_X_UPDATE THEN
                        ...
                        MATCH_MP_TAC RESIDUE12_MEMORY_CLOSE THEN
                        ASM_REWRITE_TAC[]];
                      ASM_CASES_TAC `residue = 10` THENL
                       [(* Case residue = 10: TBZ at bit 3 not-taken; runs 8-block, *)
                        (* TBZ at bit 2 taken; TBZ at bit 1 not-taken; runs 2-block,*)
                        (* TBZ at bit 0 taken. 52 instructions total (24 + 24 + 4   *)
                        (* TBZs). Closure composes RESIDUE10_X_UPDATE (which         *)
                        (* collapses the 8-byte CRC32CX chain followed by 2-byte    *)
                        (* CRC32CH chain into crc32c_bytes 0xFFFFFFFF bs) and        *)
                        ...
                        SUBGOAL_THEN
                          `LENGTH (bs0:byte list) = 16 * iters + 10 /\
                           LENGTH (bs1:byte list) = 16 * iters + 10 /\
                           LENGTH (bs2:byte list) = 16 * iters + 10 /\
                           LENGTH (bs3:byte list) = 16 * iters + 10 /\
                           LENGTH (bs4:byte list) = 16 * iters + 10 /\
                           LENGTH (bs5:byte list) = 16 * iters + 10 /\
                           LENGTH (bs6:byte list) = 16 * iters + 10 /\
                           LENGTH (bs7:byte list) = 16 * iters + 10`
                          STRIP_ASSUME_TAC THENL
                         [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                        ENSURES_INIT_TAC "s0" THEN
                        MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                       `bs0:byte list`; `16 * iters`]
                                      RESIDUE10_BYTELIST_TO_BYTES64) THEN
                        ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                        MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                       `bs1:byte list`; `16 * iters`]
                                      RESIDUE10_BYTELIST_TO_BYTES64) THEN
                        ...
                        MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                       `bs0:byte list`; `16 * iters`]
                                      RESIDUE10_BYTELIST_TO_BYTES16) THEN
                        ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                        MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                       `bs1:byte list`; `16 * iters`]
                                      RESIDUE10_BYTELIST_TO_BYTES16) THEN
                        ...
                        SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                 ...
                                 ARITH_RULE `8 <= 64`] THEN
                        REPEAT CONJ_TAC THENL
                         [AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE10_X_UPDATE THEN
                          ASM_REWRITE_TAC[];
                          AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE10_X_UPDATE THEN
                          ASM_REWRITE_TAC[];
                          AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE10_X_UPDATE THEN
                          ...
                          AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE10_X_UPDATE THEN
                          ASM_REWRITE_TAC[];
                          SUBGOAL_THEN `len = 16 * iters + 10` SUBST1_TAC THENL
                           [ASM_ARITH_TAC; ALL_TAC] THEN
                          MATCH_MP_TAC RESIDUE10_MEMORY_CLOSE THEN
                          ASM_REWRITE_TAC[];
                          SUBGOAL_THEN `len = 16 * iters + 10` SUBST1_TAC THENL
                           [ASM_ARITH_TAC; ALL_TAC] THEN
                          MATCH_MP_TAC RESIDUE10_MEMORY_CLOSE THEN
                          ...
                          MATCH_MP_TAC RESIDUE10_MEMORY_CLOSE THEN
                          ASM_REWRITE_TAC[]];
                        ASM_CASES_TAC `residue = 9` THENL
                         [(* Case residue = 9: TBZ at bit 3 not-taken; runs        *)
                          (* 8-block (LDR X / CRC32CX^8 / STR XZR), TBZ at bit 2   *)
                          (* taken; TBZ at bit 1 taken; TBZ at bit 0 not-taken;    *)
                          (* runs 1-block (LDRB / CRC32CB / STRB WZR). 52          *)
                          (* instructions total. Closure composes RESIDUE9_X_UPDATE*)
                          (* (which collapses the 8-byte CRC32CX chain followed by *)
                          (* a 1-byte CRC32CB step into crc32c_bytes 0xFFFFFFFF bs)*)
                          (* and RESIDUE9_MEMORY_CLOSE (which composes the int64 + *)
                          (* int8 zero-fills atop the prefix-bytelist zero into    *)
                          (* bytelist (a, k+9) zero).                              *)
                          (* Validated by kernel-proof load (s034, 2026-05-27).    *)
                          UNDISCH_TAC `residue = 9` THEN
                          DISCH_THEN SUBST_ALL_TAC THEN
                          SUBGOAL_THEN
                            `LENGTH (bs0:byte list) = 16 * iters + 9 /\
                             LENGTH (bs1:byte list) = 16 * iters + 9 /\
                             LENGTH (bs2:byte list) = 16 * iters + 9 /\
                             LENGTH (bs3:byte list) = 16 * iters + 9 /\
                             LENGTH (bs4:byte list) = 16 * iters + 9 /\
                             LENGTH (bs5:byte list) = 16 * iters + 9 /\
                             LENGTH (bs6:byte list) = 16 * iters + 9 /\
                             LENGTH (bs7:byte list) = 16 * iters + 9`
                            STRIP_ASSUME_TAC THENL
                           [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                          ENSURES_INIT_TAC "s0" THEN
                          MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                         `bs0:byte list`; `16 * iters`]
                                        RESIDUE9_BYTELIST_TO_BYTES64) THEN
                          ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                          MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                         `bs1:byte list`; `16 * iters`]
                                        RESIDUE9_BYTELIST_TO_BYTES64) THEN
                          ...
                          MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                         `bs0:byte list`; `16 * iters`]
                                        RESIDUE9_BYTELIST_TO_BYTES8) THEN
                          ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                          MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                         `bs1:byte list`; `16 * iters`]
                                        RESIDUE9_BYTELIST_TO_BYTES8) THEN
                          ...
                          SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                   ...
                                   ARITH_RULE `8 <= 64`] THEN
                          REPEAT CONJ_TAC THENL
                           [AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE9_X_UPDATE THEN
                            ASM_REWRITE_TAC[];
                            AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE9_X_UPDATE THEN
                            ASM_REWRITE_TAC[];
                            AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE9_X_UPDATE THEN
                            ...
                            AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE9_X_UPDATE THEN
                            ASM_REWRITE_TAC[];
                            SUBGOAL_THEN `len = 16 * iters + 9` SUBST1_TAC THENL
                             [ASM_ARITH_TAC; ALL_TAC] THEN
                            ...
                            MATCH_MP_TAC RESIDUE9_MEMORY_CLOSE THEN
                            ASM_REWRITE_TAC[]];
                          ASM_CASES_TAC `residue = 6` THENL
                           [(* Case residue = 6: TBZ at bit 3 taken; TBZ at bit 2  *)
                            (* not-taken; runs 4-block (LDR W / CRC32CW^4 /        *)
                            (* STR WZR), TBZ at bit 1 not-taken; runs 2-block      *)
                            (* (LDRH / CRC32CH^2 / STRH WZR), TBZ at bit 0 taken.  *)
                            (* 52 instructions total (4 TBZs + 24 + 24). Closure   *)
                            (* composes RESIDUE6_X_UPDATE (which collapses the     *)
                            (* 4-byte CRC32CW chain followed by 2-byte CRC32CH     *)
                            (* chain into crc32c_bytes 0xFFFFFFFF bs) and          *)
                            (* RESIDUE6_MEMORY_CLOSE (which composes the int32 +   *)
                            (* int16 zero-fills atop the prefix-bytelist zero into *)
                            (* bytelist (a, k+6) zero).                            *)
                            (* Validated by kernel-proof load (s035, 2026-05-27).  *)
                            UNDISCH_TAC `residue = 6` THEN
                            DISCH_THEN SUBST_ALL_TAC THEN
                            SUBGOAL_THEN
                              `LENGTH (bs0:byte list) = 16 * iters + 6 /\
                               LENGTH (bs1:byte list) = 16 * iters + 6 /\
                               LENGTH (bs2:byte list) = 16 * iters + 6 /\
                               LENGTH (bs3:byte list) = 16 * iters + 6 /\
                               LENGTH (bs4:byte list) = 16 * iters + 6 /\
                               LENGTH (bs5:byte list) = 16 * iters + 6 /\
                               LENGTH (bs6:byte list) = 16 * iters + 6 /\
                               LENGTH (bs7:byte list) = 16 * iters + 6`
                              STRIP_ASSUME_TAC THENL
                             [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                            ENSURES_INIT_TAC "s0" THEN
                            MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                           `bs0:byte list`; `16 * iters`]
                                          RESIDUE6_BYTELIST_TO_BYTES32) THEN
                            ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                            MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                           `bs1:byte list`; `16 * iters`]
                                          RESIDUE6_BYTELIST_TO_BYTES32) THEN
                            ...
                            MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                           `bs0:byte list`; `16 * iters`]
                                          RESIDUE6_BYTELIST_TO_BYTES16) THEN
                            ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                            MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                           `bs1:byte list`; `16 * iters`]
                                          RESIDUE6_BYTELIST_TO_BYTES16) THEN
                            ...
                            SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                     ...
                                     ARITH_RULE `8 <= 64`] THEN
                            REPEAT CONJ_TAC THENL
                             [AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE6_X_UPDATE THEN
                              ASM_REWRITE_TAC[];
                              AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE6_X_UPDATE THEN
                              ASM_REWRITE_TAC[];
                              AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE6_X_UPDATE THEN
                              ...
                              AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE6_X_UPDATE THEN
                              ASM_REWRITE_TAC[];
                              SUBGOAL_THEN `len = 16 * iters + 6` SUBST1_TAC THENL
                               [ASM_ARITH_TAC; ALL_TAC] THEN
                              MATCH_MP_TAC RESIDUE6_MEMORY_CLOSE THEN
                              ASM_REWRITE_TAC[];
                              SUBGOAL_THEN `len = 16 * iters + 6` SUBST1_TAC THENL
                               [ASM_ARITH_TAC; ALL_TAC] THEN
                              MATCH_MP_TAC RESIDUE6_MEMORY_CLOSE THEN
                              ...
                              MATCH_MP_TAC RESIDUE6_MEMORY_CLOSE THEN
                              ASM_REWRITE_TAC[]];
                            ASM_CASES_TAC `residue = 5` THENL
                             [(* Case residue = 5: TBZ at bit 3 taken; TBZ at bit 2  *)
                              (* not-taken; runs 4-block (LDR W / CRC32CW^4 /        *)
                              (* STR WZR), TBZ at bit 1 taken; TBZ at bit 0 not-     *)
                              (* taken; runs 1-block (LDRB / CRC32CB / STRB WZR).    *)
                              (* 52 instructions total (4 TBZs + 24 + 24). Closure   *)
                              (* composes RESIDUE5_X_UPDATE (which collapses the     *)
                              (* 4-byte CRC32CW chain followed by 1-byte CRC32CB     *)
                              (* step into crc32c_bytes 0xFFFFFFFF bs) and           *)
                              (* RESIDUE5_MEMORY_CLOSE (which composes the int32 +   *)
                              (* int8 zero-fills atop the prefix-bytelist zero into  *)
                              (* bytelist (a, k+5) zero).                            *)
                              (* Validated by kernel-proof load (s036, 2026-05-27).  *)
                              UNDISCH_TAC `residue = 5` THEN
                              DISCH_THEN SUBST_ALL_TAC THEN
                              SUBGOAL_THEN
                                `LENGTH (bs0:byte list) = 16 * iters + 5 /\
                                 LENGTH (bs1:byte list) = 16 * iters + 5 /\
                                 LENGTH (bs2:byte list) = 16 * iters + 5 /\
                                 LENGTH (bs3:byte list) = 16 * iters + 5 /\
                                 LENGTH (bs4:byte list) = 16 * iters + 5 /\
                                 LENGTH (bs5:byte list) = 16 * iters + 5 /\
                                 LENGTH (bs6:byte list) = 16 * iters + 5 /\
                                 LENGTH (bs7:byte list) = 16 * iters + 5`
                                STRIP_ASSUME_TAC THENL
                               [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                              ENSURES_INIT_TAC "s0" THEN
                              MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                             `bs0:byte list`; `16 * iters`]
                                            RESIDUE5_BYTELIST_TO_BYTES32) THEN
                              ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                              MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                             `bs1:byte list`; `16 * iters`]
                                            RESIDUE5_BYTELIST_TO_BYTES32) THEN
                              ...
                              MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                             `bs0:byte list`; `16 * iters`]
                                            RESIDUE5_BYTELIST_TO_BYTES8) THEN
                              ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                              MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                             `bs1:byte list`; `16 * iters`]
                                            RESIDUE5_BYTELIST_TO_BYTES8) THEN
                              ...
                              SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                       ...
                                       ARITH_RULE `8 <= 64`] THEN
                              REPEAT CONJ_TAC THENL
                               [AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE5_X_UPDATE THEN
                                ASM_REWRITE_TAC[];
                                AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE5_X_UPDATE THEN
                                ASM_REWRITE_TAC[];
                                AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE5_X_UPDATE THEN
                                ...
                                AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE5_X_UPDATE THEN
                                ASM_REWRITE_TAC[];
                                SUBGOAL_THEN `len = 16 * iters + 5` SUBST1_TAC THENL
                                 [ASM_ARITH_TAC; ALL_TAC] THEN
                                MATCH_MP_TAC RESIDUE5_MEMORY_CLOSE THEN
                                ASM_REWRITE_TAC[];
                                SUBGOAL_THEN `len = 16 * iters + 5` SUBST1_TAC THENL
                                 [ASM_ARITH_TAC; ALL_TAC] THEN
                                MATCH_MP_TAC RESIDUE5_MEMORY_CLOSE THEN
                                ...
                                MATCH_MP_TAC RESIDUE5_MEMORY_CLOSE THEN
                                ASM_REWRITE_TAC[]];
                              ASM_CASES_TAC `residue = 3` THENL
                               [(* Case residue = 3: TBZ at bit 3 taken; TBZ at bit 2  *)
                                (* taken; TBZ at bit 1 not-taken; runs 2-block         *)
                                (* (LDRH / CRC32CH^2 / STRH WZR), TBZ at bit 0         *)
                                (* not-taken; runs 1-block (LDRB / CRC32CB / STRB      *)
                                (* WZR). 52 instructions total (4 TBZs + 24 + 24).    *)
                                (* Closure composes RESIDUE3_X_UPDATE (which collapses *)
                                (* the 2-byte CRC32CH chain followed by 1-byte CRC32CB *)
                                (* step into crc32c_bytes 0xFFFFFFFF bs) and           *)
                                ...
                                DISCH_THEN SUBST_ALL_TAC THEN
                                SUBGOAL_THEN
                                  `LENGTH (bs0:byte list) = 16 * iters + 3 /\
                                   LENGTH (bs1:byte list) = 16 * iters + 3 /\
                                   LENGTH (bs2:byte list) = 16 * iters + 3 /\
                                   LENGTH (bs3:byte list) = 16 * iters + 3 /\
                                   LENGTH (bs4:byte list) = 16 * iters + 3 /\
                                   LENGTH (bs5:byte list) = 16 * iters + 3 /\
                                   LENGTH (bs6:byte list) = 16 * iters + 3 /\
                                   LENGTH (bs7:byte list) = 16 * iters + 3`
                                  STRIP_ASSUME_TAC THENL
                                 [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                ENSURES_INIT_TAC "s0" THEN
                                MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                               `bs0:byte list`; `16 * iters`]
                                              RESIDUE3_BYTELIST_TO_BYTES16) THEN
                                ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                               `bs1:byte list`; `16 * iters`]
                                              RESIDUE3_BYTELIST_TO_BYTES16) THEN
                                ...
                                MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                               `bs0:byte list`; `16 * iters`]
                                              RESIDUE3_BYTELIST_TO_BYTES8) THEN
                                ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                               `bs1:byte list`; `16 * iters`]
                                              RESIDUE3_BYTELIST_TO_BYTES8) THEN
                                ...
                                SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                         ...
                                         ARITH_RULE `8 <= 64`] THEN
                                REPEAT CONJ_TAC THENL
                                 [AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE3_X_UPDATE THEN
                                  ASM_REWRITE_TAC[];
                                  AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE3_X_UPDATE THEN
                                  ASM_REWRITE_TAC[];
                                  AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE3_X_UPDATE THEN
                                  ...
                                  AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE3_X_UPDATE THEN
                                  ASM_REWRITE_TAC[];
                                  SUBGOAL_THEN `len = 16 * iters + 3` SUBST1_TAC THENL
                                   [ASM_ARITH_TAC; ALL_TAC] THEN
                                  MATCH_MP_TAC RESIDUE3_MEMORY_CLOSE THEN
                                  ASM_REWRITE_TAC[];
                                  SUBGOAL_THEN `len = 16 * iters + 3` SUBST1_TAC THENL
                                   [ASM_ARITH_TAC; ALL_TAC] THEN
                                  MATCH_MP_TAC RESIDUE3_MEMORY_CLOSE THEN
                                  ...
                                  MATCH_MP_TAC RESIDUE3_MEMORY_CLOSE THEN
                                  ASM_REWRITE_TAC[]];
                                ASM_CASES_TAC `residue = 14` THENL
                                 [(* Case residue = 14: TBZ at bit 3 not-taken; runs   *)
                                  (* 8-block, TBZ at bit 2 not-taken; runs 4-block,    *)
                                  (* TBZ at bit 1 not-taken; runs 2-block, TBZ at bit  *)
                                  (* 0 taken. 76 instructions total (4 TBZs + 24 +    *)
                                  (* 24 + 24). Closure composes RESIDUE14_X_UPDATE     *)
                                  (* (which collapses the 8-byte CRC32CX, 4-byte       *)
                                  (* CRC32CW and 2-byte CRC32CH chain into             *)
                                  (* crc32c_bytes 0xFFFFFFFF bs) and                   *)
                                  (* RESIDUE14_MEMORY_CLOSE (which composes the int64  *)
                                  (* + int32 + int16 zero-fills atop the prefix-      *)
                                  (* bytelist zero into bytelist (a, k+14) zero).      *)
                                  UNDISCH_TAC `residue = 14` THEN
                                  DISCH_THEN SUBST_ALL_TAC THEN
                                  SUBGOAL_THEN
                                    `LENGTH (bs0:byte list) = 16 * iters + 14 /\
                                     LENGTH (bs1:byte list) = 16 * iters + 14 /\
                                     LENGTH (bs2:byte list) = 16 * iters + 14 /\
                                     LENGTH (bs3:byte list) = 16 * iters + 14 /\
                                     LENGTH (bs4:byte list) = 16 * iters + 14 /\
                                     LENGTH (bs5:byte list) = 16 * iters + 14 /\
                                     LENGTH (bs6:byte list) = 16 * iters + 14 /\
                                     LENGTH (bs7:byte list) = 16 * iters + 14`
                                    STRIP_ASSUME_TAC THENL
                                   [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                  ENSURES_INIT_TAC "s0" THEN
                                  MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                 `bs0:byte list`; `16 * iters`]
                                                RESIDUE14_BYTELIST_TO_BYTES64) THEN
                                  ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                  MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                 `bs1:byte list`; `16 * iters`]
                                                RESIDUE14_BYTELIST_TO_BYTES64) THEN
                                  ...
                                  MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                 `bs0:byte list`; `16 * iters`]
                                                RESIDUE14_BYTELIST_TO_BYTES32) THEN
                                  ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                  MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                 `bs1:byte list`; `16 * iters`]
                                                RESIDUE14_BYTELIST_TO_BYTES32) THEN
                                  ...
                                  MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                 `bs0:byte list`; `16 * iters`]
                                                RESIDUE14_BYTELIST_TO_BYTES16) THEN
                                  ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                  MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                 `bs1:byte list`; `16 * iters`]
                                                RESIDUE14_BYTELIST_TO_BYTES16) THEN
                                  ...
                                  SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                           ...
                                           ARITH_RULE `8 <= 64`] THEN
                                  REPEAT CONJ_TAC THENL
                                   [AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE14_X_UPDATE THEN
                                    ASM_REWRITE_TAC[];
                                    AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE14_X_UPDATE THEN
                                    ASM_REWRITE_TAC[];
                                    AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE14_X_UPDATE THEN
                                    ...
                                    AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE14_X_UPDATE THEN
                                    ASM_REWRITE_TAC[];
                                    SUBGOAL_THEN `len = 16 * iters + 14` SUBST1_TAC THENL
                                     [ASM_ARITH_TAC; ALL_TAC] THEN
                                    MATCH_MP_TAC RESIDUE14_MEMORY_CLOSE THEN
                                    ASM_REWRITE_TAC[];
                                    SUBGOAL_THEN `len = 16 * iters + 14` SUBST1_TAC THENL
                                     [ASM_ARITH_TAC; ALL_TAC] THEN
                                    MATCH_MP_TAC RESIDUE14_MEMORY_CLOSE THEN
                                    ...
                                    MATCH_MP_TAC RESIDUE14_MEMORY_CLOSE THEN
                                    ASM_REWRITE_TAC[]];
                                  ASM_CASES_TAC `residue = 15` THENL
                                   [(* Case residue = 15: TBZ at bit 3 not-taken; runs *)
                                    (* 8-block, TBZ at bit 2 not-taken; runs 4-block,  *)
                                    (* TBZ at bit 1 not-taken; runs 2-block, TBZ at    *)
                                    (* bit 0 not-taken; runs 1-block. 100 instructions *)
                                    (* total (4 TBZs + 24 + 24 + 24 + 24). Closure     *)
                                    (* composes RESIDUE15_X_UPDATE (which collapses    *)
                                    (* the 8-byte CRC32CX, 4-byte CRC32CW, 2-byte      *)
                                    (* CRC32CH and 1-byte CRC32CB chain into           *)
                                    (* crc32c_bytes 0xFFFFFFFF bs) and                 *)
                                    (* RESIDUE15_MEMORY_CLOSE (which composes the      *)
                                    (* int64 + int32 + int16 + int8 zero-fills atop    *)
                                    (* the prefix-bytelist zero into bytelist          *)
                                    (* (a, k+15) zero).                                *)
                                    UNDISCH_TAC `residue = 15` THEN
                                    DISCH_THEN SUBST_ALL_TAC THEN
                                    SUBGOAL_THEN
                                      `LENGTH (bs0:byte list) = 16 * iters + 15 /\
                                       LENGTH (bs1:byte list) = 16 * iters + 15 /\
                                       LENGTH (bs2:byte list) = 16 * iters + 15 /\
                                       LENGTH (bs3:byte list) = 16 * iters + 15 /\
                                       LENGTH (bs4:byte list) = 16 * iters + 15 /\
                                       LENGTH (bs5:byte list) = 16 * iters + 15 /\
                                       LENGTH (bs6:byte list) = 16 * iters + 15 /\
                                       LENGTH (bs7:byte list) = 16 * iters + 15`
                                      STRIP_ASSUME_TAC THENL
                                     [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                    ENSURES_INIT_TAC "s0" THEN
                                    MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                   `bs0:byte list`; `16 * iters`]
                                                  RESIDUE15_BYTELIST_TO_BYTES64) THEN
                                    ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                   `bs1:byte list`; `16 * iters`]
                                                  RESIDUE15_BYTELIST_TO_BYTES64) THEN
                                    ...
                                    MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                   `bs0:byte list`; `16 * iters`]
                                                  RESIDUE15_BYTELIST_TO_BYTES32) THEN
                                    ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                   `bs1:byte list`; `16 * iters`]
                                                  RESIDUE15_BYTELIST_TO_BYTES32) THEN
                                    ...
                                    MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                   `bs0:byte list`; `16 * iters`]
                                                  RESIDUE15_BYTELIST_TO_BYTES16) THEN
                                    ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                   `bs1:byte list`; `16 * iters`]
                                                  RESIDUE15_BYTELIST_TO_BYTES16) THEN
                                    ...
                                    MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                   `bs0:byte list`; `16 * iters`]
                                                  RESIDUE15_BYTELIST_TO_BYTES8) THEN
                                    ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                    MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                   `bs1:byte list`; `16 * iters`]
                                                  RESIDUE15_BYTELIST_TO_BYTES8) THEN
                                    ...
                                    SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                             ...
                                             ARITH_RULE `8 <= 64`] THEN
                                    REPEAT CONJ_TAC THENL
                                     [AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE15_X_UPDATE THEN
                                      ASM_REWRITE_TAC[];
                                      AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE15_X_UPDATE THEN
                                      ASM_REWRITE_TAC[];
                                      AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE15_X_UPDATE THEN
                                      ...
                                      AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE15_X_UPDATE THEN
                                      ASM_REWRITE_TAC[];
                                      SUBGOAL_THEN `len = 16 * iters + 15` SUBST1_TAC THENL
                                       [ASM_ARITH_TAC; ALL_TAC] THEN
                                      MATCH_MP_TAC RESIDUE15_MEMORY_CLOSE THEN
                                      ASM_REWRITE_TAC[];
                                      SUBGOAL_THEN `len = 16 * iters + 15` SUBST1_TAC THENL
                                       [ASM_ARITH_TAC; ALL_TAC] THEN
                                      MATCH_MP_TAC RESIDUE15_MEMORY_CLOSE THEN
                                      ...
                                      MATCH_MP_TAC RESIDUE15_MEMORY_CLOSE THEN
                                      ASM_REWRITE_TAC[]];
                                    ASM_CASES_TAC `residue = 7` THENL
                                     [(* Case residue = 7: TBZ at bit 3 taken; TBZ at    *)
                                      (* bit 2 not-taken; runs 4-block (LDR W /          *)
                                      (* CRC32CW^4 / STR WZR), TBZ at bit 1 not-taken;   *)
                                      (* runs 2-block (LDRH / CRC32CH^2 / STRH WZR),     *)
                                      (* TBZ at bit 0 not-taken; runs 1-block (LDRB /    *)
                                      (* CRC32CB / STRB WZR). 76 instructions total      *)
                                      (* (4 TBZs + 24 + 24 + 24). Closure composes       *)
                                      (* RESIDUE7_X_UPDATE (which collapses the 4-byte   *)
                                      (* CRC32CW, 2-byte CRC32CH and 1-byte CRC32CB      *)
                                      (* chain into crc32c_bytes 0xFFFFFFFF bs) and       *)
                                      (* RESIDUE7_MEMORY_CLOSE (which composes the       *)
                                      (* int32 + int16 + int8 zero-fills atop the        *)
                                      (* prefix-bytelist zero into bytelist (a, k+7)     *)
                                      (* zero).                                          *)
                                      UNDISCH_TAC `residue = 7` THEN
                                      DISCH_THEN SUBST_ALL_TAC THEN
                                      SUBGOAL_THEN
                                        `LENGTH (bs0:byte list) = 16 * iters + 7 /\
                                         LENGTH (bs1:byte list) = 16 * iters + 7 /\
                                         LENGTH (bs2:byte list) = 16 * iters + 7 /\
                                         LENGTH (bs3:byte list) = 16 * iters + 7 /\
                                         LENGTH (bs4:byte list) = 16 * iters + 7 /\
                                         LENGTH (bs5:byte list) = 16 * iters + 7 /\
                                         LENGTH (bs6:byte list) = 16 * iters + 7 /\
                                         LENGTH (bs7:byte list) = 16 * iters + 7`
                                        STRIP_ASSUME_TAC THENL
                                       [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                      ENSURES_INIT_TAC "s0" THEN
                                      MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                     `bs0:byte list`; `16 * iters`]
                                                    RESIDUE7_BYTELIST_TO_BYTES32) THEN
                                      ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                      MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                     `bs1:byte list`; `16 * iters`]
                                                    RESIDUE7_BYTELIST_TO_BYTES32) THEN
                                      ...
                                      MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                     `bs0:byte list`; `16 * iters`]
                                                    RESIDUE7_BYTELIST_TO_BYTES16) THEN
                                      ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                      MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                     `bs1:byte list`; `16 * iters`]
                                                    RESIDUE7_BYTELIST_TO_BYTES16) THEN
                                      ...
                                      MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                     `bs0:byte list`; `16 * iters`]
                                                    RESIDUE7_BYTELIST_TO_BYTES8) THEN
                                      ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                      MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                     `bs1:byte list`; `16 * iters`]
                                                    RESIDUE7_BYTELIST_TO_BYTES8) THEN
                                      ...
                                      SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                               ...
                                               ARITH_RULE `8 <= 64`] THEN
                                      REPEAT CONJ_TAC THENL
                                       [AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE7_X_UPDATE THEN
                                        ASM_REWRITE_TAC[];
                                        AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE7_X_UPDATE THEN
                                        ASM_REWRITE_TAC[];
                                        AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE7_X_UPDATE THEN
                                        ...
                                        AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE7_X_UPDATE THEN
                                        ASM_REWRITE_TAC[];
                                        SUBGOAL_THEN `len = 16 * iters + 7` SUBST1_TAC THENL
                                         [ASM_ARITH_TAC; ALL_TAC] THEN
                                        MATCH_MP_TAC RESIDUE7_MEMORY_CLOSE THEN
                                        ASM_REWRITE_TAC[];
                                        SUBGOAL_THEN `len = 16 * iters + 7` SUBST1_TAC THENL
                                         [ASM_ARITH_TAC; ALL_TAC] THEN
                                        MATCH_MP_TAC RESIDUE7_MEMORY_CLOSE THEN
                                        ...
                                        MATCH_MP_TAC RESIDUE7_MEMORY_CLOSE THEN
                                        ASM_REWRITE_TAC[]];
                                      ASM_CASES_TAC `residue = 11` THENL
                                       [(* Case residue = 11: TBZ at bit 3 not-taken;     *)
                                        (* runs 8-block (LDR X / CRC32CX^8 / STR XZR),    *)
                                        (* TBZ at bit 2 taken; TBZ at bit 1 not-taken;    *)
                                        (* runs 2-block (LDRH / CRC32CH^2 / STRH WZR),    *)
                                        (* TBZ at bit 0 not-taken; runs 1-block (LDRB /   *)
                                        (* CRC32CB / STRB WZR). 76 instructions total     *)
                                        (* (4 TBZs + 24 + 24 + 24). Closure composes      *)
                                        (* RESIDUE11_X_UPDATE (which collapses the        *)
                                        (* 8-byte CRC32CX, 2-byte CRC32CH and 1-byte      *)
                                        (* CRC32CB chain into crc32c_bytes 0xFFFFFFFF bs) *)
                                        (* and RESIDUE11_MEMORY_CLOSE (which composes     *)
                                        (* the int64 + int16 + int8 zero-fills atop the   *)
                                        (* prefix-bytelist zero into bytelist (a, k+11)   *)
                                        (* zero).                                         *)
                                        UNDISCH_TAC `residue = 11` THEN
                                        DISCH_THEN SUBST_ALL_TAC THEN
                                        SUBGOAL_THEN
                                          `LENGTH (bs0:byte list) = 16 * iters + 11 /\
                                           LENGTH (bs1:byte list) = 16 * iters + 11 /\
                                           LENGTH (bs2:byte list) = 16 * iters + 11 /\
                                           LENGTH (bs3:byte list) = 16 * iters + 11 /\
                                           LENGTH (bs4:byte list) = 16 * iters + 11 /\
                                           LENGTH (bs5:byte list) = 16 * iters + 11 /\
                                           LENGTH (bs6:byte list) = 16 * iters + 11 /\
                                           LENGTH (bs7:byte list) = 16 * iters + 11`
                                          STRIP_ASSUME_TAC THENL
                                         [REPEAT CONJ_TAC THEN ASM_ARITH_TAC; ALL_TAC] THEN
                                        ENSURES_INIT_TAC "s0" THEN
                                        MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                       `bs0:byte list`; `16 * iters`]
                                                      RESIDUE11_BYTELIST_TO_BYTES64) THEN
                                        ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                        MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                       `bs1:byte list`; `16 * iters`]
                                                      RESIDUE11_BYTELIST_TO_BYTES64) THEN
                                        ...
                                        MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                       `bs0:byte list`; `16 * iters`]
                                                      RESIDUE11_BYTELIST_TO_BYTES16) THEN
                                        ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                        MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                       `bs1:byte list`; `16 * iters`]
                                                      RESIDUE11_BYTELIST_TO_BYTES16) THEN
                                        ...
                                        MP_TAC(ISPECL [`a0:int64`; `s0:armstate`;
                                                       `bs0:byte list`; `16 * iters`]
                                                      RESIDUE11_BYTELIST_TO_BYTES8) THEN
                                        ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
                                        MP_TAC(ISPECL [`a1:int64`; `s0:armstate`;
                                                       `bs1:byte list`; `16 * iters`]
                                                      RESIDUE11_BYTELIST_TO_BYTES8) THEN
                                        ...
                                        SIMP_TAC[WORD_ZX_ZX; DIMINDEX_8; DIMINDEX_16;
                                                 ...
                                                 ARITH_RULE `8 <= 64`] THEN
                                        REPEAT CONJ_TAC THENL
                                         [AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE11_X_UPDATE THEN
                                          ASM_REWRITE_TAC[];
                                          AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE11_X_UPDATE THEN
                                          ASM_REWRITE_TAC[];
                                          AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE11_X_UPDATE THEN
                                          ...
                                          AP_TERM_TAC THEN MATCH_MP_TAC RESIDUE11_X_UPDATE THEN
                                          ASM_REWRITE_TAC[];
                                          SUBGOAL_THEN `len = 16 * iters + 11` SUBST1_TAC THENL
                                           [ASM_ARITH_TAC; ALL_TAC] THEN
