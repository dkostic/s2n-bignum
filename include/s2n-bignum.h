...
// SHA-256 multi-block processing (scalar, no SHA-256 hardware extensions, ARM only)
// Inputs state[8], data[64*num_blocks], num_blocks, K[64]; output state[8]
extern void sha256_block_data_order_nohw(uint32_t state[S2N_BIGNUM_STATIC 8],const uint8_t *data,uint64_t num_blocks,const uint32_t k[S2N_BIGNUM_STATIC 64]);

// SHA-256 multi-block processing (scalar, optimised: shifted-register EOR
// fusion + fused Maj), no SHA-256 hardware extensions, ARM only
// Inputs state[8], data[64*num_blocks], num_blocks, K[64]; output state[8]
extern void sha256_block_data_order_nohw2(uint32_t state[S2N_BIGNUM_STATIC 8],const uint8_t *data,uint64_t num_blocks,const uint32_t k[S2N_BIGNUM_STATIC 64]);

// SHA-256 multi-block processing (scalar, optimised: nohw2 fusions +
// message-schedule/compression interleaving), no SHA-256 hardware
// extensions, ARM only
// Inputs state[8], data[64*num_blocks], num_blocks, K[64]; output state[8]
extern void sha256_block_data_order_nohw4(uint32_t state[S2N_BIGNUM_STATIC 8],const uint8_t *data,uint64_t num_blocks,const uint32_t k[S2N_BIGNUM_STATIC 64]);

// SHA-256 multi-block processing (scalar, optimised: nohw4 +
// message-schedule window resident in registers, eliminating the
// 256-byte stack scratch), no SHA-256 hardware extensions, ARM only
// Inputs state[8], data[64*num_blocks], num_blocks, K[64]; output state[8]
...
// SHA-512 multi-block processing (hardware-accelerated, ARM only)
// Inputs state[8], data[128*num_blocks], num_blocks, K[80]; output state[8]
extern void sha512_block_data_order_hw(uint64_t state[S2N_BIGNUM_STATIC 8],const uint8_t *data,uint64_t num_blocks,const uint64_t k[S2N_BIGNUM_STATIC 80]);

// SHA-512 multi-block processing (scalar fallback, ARM only)
// Inputs state[8], data[128*num_blocks], num_blocks, K[80]; output state[8]
extern void sha512_block_data_order_nohw(uint64_t state[S2N_BIGNUM_STATIC 8],const uint8_t *data,uint64_t num_blocks,const uint64_t k[S2N_BIGNUM_STATIC 80]);

// SHA-512 multi-block processing (scalar fallback, optimised; ARM only)
// Inputs state[8], data[128*num_blocks], num_blocks, K[80]; output state[8]
extern void sha512_block_data_order_nohw2(uint64_t state[S2N_BIGNUM_STATIC 8],const uint8_t *data,uint64_t num_blocks,const uint64_t k[S2N_BIGNUM_STATIC 80]);

// SHA-512 multi-block processing (scalar fallback, cyclic-state; ARM only)
// Inputs state[8], data[128*num_blocks], num_blocks, K[80]; output state[8]
extern void sha512_block_data_order_nohw3(uint64_t state[S2N_BIGNUM_STATIC 8],const uint8_t *data,uint64_t num_blocks,const uint64_t k[S2N_BIGNUM_STATIC 80]);
