...
let CONGBOUND_BARRED_X86 = prove
 (`!a a' l u.
  ...
  (*** Somewhat simplify the goal using some rewrites we can generate ***)
  ...
  (*** Rewrite with more definitions to transform more stuff to integers ***)

  REWRITE_TAC[WORD_MUL_IMODULAR; imodular; IVAL_WORD_ISHR] THEN
  SIMP_TAC[IVAL_WORD_SX; DIMINDEX_32; DIMINDEX_16; ARITH] THEN
  CONV_TAC WORD_REDUCE_CONV THEN

  (*** Split off and prove inner "ival(iword x) = x" by bound reasoning ***)
  ...
  (*** Split off and prove "ival(iword x) = x" for whole expression ***)
  ...
  (*** The congruence part is now trivial ***)

  ASM_REWRITE_TAC[INTEGER_RULE
   `(a - x * p:int == a') (mod p) <=> (a == a') (mod p)`] THEN

  (*** The rest can be done by integer arithmetic given bound on a ***)
